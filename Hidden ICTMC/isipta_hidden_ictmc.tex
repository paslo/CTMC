...
Let $\mathbb{P}_{\rateset,\mathcal{M}}$ be an ICTMC, consider any $u,v\in\mathcal{U}$, any $f\in\gambles(\states_v)$, and, for all $t_i\in u$, any $g_{t_i}\in\gambles(\states_{t_i})$ such that $g_{t_i}\geq 0$. Let $G:\reals\to\reals$ be a function defined for all $\mu\in\reals$, as
\begin{equation*}

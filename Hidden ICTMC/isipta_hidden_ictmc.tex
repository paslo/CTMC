...
\mathbb{E}_{P_\states}\bigl[g(X_u)\bigl(f(X_v) - \mu\bigr)\bigr] &= \sum_{x_{u\cup v}\in\states_{u\cup v}} P_\states(X_{u\cup v}=x_{u\cup v})g(x_u)\bigl(f(x_v) - \mu\bigr) \\
 ...
 % &= \sum_{x_{u}\in\states_u^0}\sum_{x_{v\setminus u}\in\states_{v\setminus u}} P_\states(X_u=x_u,X_{v\setminus u}=x_{v\setminus u})g(x_u)\bigl(f(x_v) - \mu\bigr) \\
 % &\quad\quad + \sum_{x_{u}\in\states_{u}\setminus \states_u^0}\sum_{x_{v\setminus u}\in\states_{v\setminus u}}P_\states(X_u=x_u,X_{v\setminus u}=x_{v\setminus u})g(x_u)\bigl(f(x_v) - \mu\bigr) \\
 % &= \sum_{x_{u}\in\states_u^0}\sum_{x_{v\setminus u}\in\states_{v\setminus u}} 0g(x_u)\bigl(f(x_v) - \mu\bigr) \\
 % &\quad\quad + \sum_{x_{u}\in\states_{u}\setminus \states_u^0}\sum_{x_{v\setminus u}\in\states_{v\setminus u}} P_\states(X_u=x_u,X_{v\setminus u}=x_{v\setminus u})0\bigl(f(x_v) - \mu\bigr)= 0\,.
\end{align*}
\end{proof}
...
\end{corollary}
\begin{proof}
Trivial consequence of Lemma~\ref{lemma:general_regular_extension} %and~\ref{lemma:state_expectation_doesnt_depend_on_outputs}, and
...
\psi(y_t\vert x_t) = \lim_{i\to+\infty}\frac{P_{\observs\vert\states}(O_t^i\vert x_t)}{\lambda(O_t^i)}\,,
\end{equation*}
as claimed. Because this holds for all $t\in u$, it follows from the above that *** doesn't this require some extra steps (such as the factorisation of the Lebesgue integral)? ***
\begin{equation*}
...
\end{align*}
However, for any $i\in\nats$, since $P_2\in\mathcal{M}$, there is some $P\in\mathcal{Z}$ such that %$\mathbb{E}_P[P_{\observs\vert\states}(O_0^i\vert X_0)]>0$, and hence
%\begin{align*}
...
%and, since also 
\begin{equation*}
P(Y_0\in O_0^i)=\mathbb{E}_{P_\states}[P_{\observs\vert\states}(O_0^i\vert X_0)]=\mathbb{E}_{P_\states}[P_{\observs\vert\states}(O_0^i\vert X_0)]=P_{\observs\vert\states}(O_0^i\vert \overline{x})>0,
\end{equation*}
and so
\begin{align*}
...
Then, as we know from that example, $\mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u]$ exists for all $P\in\mathcal{Z}$ *** HUH??? WHY? ***, and furthermore
\begin{equation*}

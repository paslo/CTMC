...
Now choose any $y_u\in\observs_u$, any $t\in u$, and choose a sequence $\{O_t^i\}_{i\in\nats}$ of open balls in $\observs$ that are centered on, and shrink to, $y_t$. Fix any $x_u\in\states_u$. Then if $\phi(\cdot\vert x_t)$ is continuous at $y_t$, it can be shown that
...
\end{equation*}
provided that each $\phi(y_t\vert x_t)$ satisfies Equation~\eqref{eq:density_is_limit}; continuity of every $\phi(\cdot \vert x_t)$ at the respective $y_t$ is a sufficient condition.

...
The reason that we here choose to update the model using natural extension, rather than regular extension, is that the latter leads to some interpretational difficulties. In contrast, natural extension yields the intuitive limit interpretation\footnote{Proposition~\ref{prop:GBR_for_densities_is_limit_if_continuous} does not in general hold when we modify it and Definition~\ref{def:reg_ext_densities} to work with regular extension, i.e. when $\lexp[\phi_u(y_u\vert X_u)]=0$ but $\uexp[\phi_u(y_u\vert X_u)]>0$. Nevertheless, if one does not mind losing this interpretation, we may note that the updated model that is so defined is still numerically well-behaved and computable; it still satisfies, \emph{mutatis mutandis}, Proposition~\ref{prop:GBR_for_densities_lower_zero}. In any case, a sufficient condition to obtain $\lexp[\phi_u(y_u\vert X_u)]>0$ is that $\phi_u(\cdot\vert X_u)$ is strictly positive on $\observs$; again, this should arguably hold for most practical applications.}: 
...
Let $\mathbb{P}_{\rateset,\mathcal{M}}$ be an ICTMC, consider any $u,v\in\mathcal{U}$, any $f\in\gambles(\states_v)$, and, for all $t_i\in u$, any $g_{t_i}\in\gambles(\states_{t_i})$ such that $g_{t_i}\geq 0$. 
Consider the function $G: \reals\to\reals$ that is given, for all $\mu\in\reals$, by $G(\mu)\coloneqq \lexp\left[\left(\prod_{i=0}^n g_{t_i}(X_{t_i})\right)\bigl(f(X_v) - \mu\bigr)\right]$.
...
Consider the set
\begin{equation*}
\mathcal{P} \coloneqq \left\{P\in\mathcal{Z}\,:\,\text{$\mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u]$ exists}\right\}
\end{equation*}
Let the updated imprecise model be defined by
\begin{equation*}
\underline{\mathbb{E}}_{\mathcal{Z}}^\mathrm{L}[f(X_v)\,\vert\,Y_u=y_u] \coloneqq \inf\left\{ \mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u]\,:\,P\in\mathcal{P} \right\}\,,
\end{equation*}
whenever $\mathcal{P}\neq\emptyset$, and leave it undefined, otherwise.

Then, if $\mathcal{P}\neq\emptyset$, it holds that
\begin{equation*}
\underline{\mathbb{E}}_{\mathcal{Z}}^\mathrm{L}[f(X_v)\,\vert\,Y_u=y_u] = \lim_{i\to+\infty} \underline{\mathbb{E}}_{\mathcal{Z}}[f(X_v)\,\vert\,Y_u\in O_u^i]\,.
\end{equation*}
\end{proposition}
\begin{proof}
This uses the following claim:
\newline
\newline
{\bf Claim 1:~}\emph{If $\mathcal{P}\neq\emptyset$ then $\uexp[P_{\observs\vert\states}(O_u^i\vert X_u)]>0$ for all $i\in\nats$. (Pretty sure that this is true).}
\newline
\newline
For notational brevity, let $\mu_i\coloneqq \underline{\mathbb{E}}_{\mathcal{Z}}[f(X_v)\,\vert\,Y_u\in O_u^i]$ for all $i\in\nats$. Then, by the above claim, $\{\mu_i\}_{i\in\nats}$ is well-defined, and furthermore $\mu_i\in[\min f, \max f]$ for all $i\in\nats$.

Let $\{\mu_{i_k}\}_{k\in\nats}$ be any convergent subsequence of $\{\mu_i\}_{i\in\nats}$; the Bolzano-Weierstrass theorem implies the existence of at least one such subsequence. Let $\mu_{k_*}\coloneqq \lim_{k\to+\infty}\mu_{i_k}$.

Fix any $P\in\mathcal{P}$. Then, for every $k\in\nats$, we have
\begin{equation*}
\mu_{i_k} \leq \mathbb{E}_P[f(X_v)\,\vert\,Y_u\in O_u^{i_k}]\,.
\end{equation*}
Because this holds for all $k\in\nats$, we must have
\begin{equation*}
\mu_{k_*} = \lim_{k\to+\infty}\mu_{i_k} \leq \lim_{k\to+\infty} \mathbb{E}_P[f(X_v)\,\vert\,Y_u\in O_u^{i_k}] = \mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u]\,,
\end{equation*}
and therefore clearly also for every $\epsilon>0$ we have
\begin{equation*}
\mu_{k_*} - \epsilon \leq \mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u] - \epsilon\,.
\end{equation*}
Furthermore, for every $\epsilon>0$, there is some $P\in\mathcal{P}$ such that
\begin{equation*}
\mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u] - \epsilon < \underline{\mathbb{E}}_{\mathcal{Z}}^\mathrm{L}[f(X_v)\,\vert\,Y_u=y_u]\,.
\end{equation*}
Combining with the above, we find
\begin{equation*}
\mu_{k_*} - \epsilon \leq \mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u] - \epsilon < \underline{\mathbb{E}}_{\mathcal{Z}}^\mathrm{L}[f(X_v)\,\vert\,Y_u=y_u]\,.
\end{equation*}
Because the $\epsilon>0$ was arbitrary, we conclude
\begin{equation*}
\mu_{k_*} \leq \underline{\mathbb{E}}_{\mathcal{Z}}^\mathrm{L}[f(X_v)\,\vert\,Y_u=y_u]\,.
\end{equation*}
To establish equality, we next prove the inequality in the other direction.

Choose any sequence $\{\epsilon_k\}_{k\in\nats}\to 0^+$, and fix any $k\in\nats$. Then, there is some $P_k\in\mathcal{Z}$ such that $\mathbb{E}_{P_k}[P_{\observs\vert\states}(O_u^i\vert X_u)]>0$, and
\begin{equation*}
\mathbb{E}_{P_k}[f(X_v)\,\vert\,Y_u\in O_u^{i_k}] - \epsilon_k < \mu_{i_k}\,.
\end{equation*}
Furthermore, clearly, $\abs{\mu_{i_k} - \mathbb{E}_{P_k}[f(X_v)\,\vert\,Y_u\in O_u^{i_k}]} < \epsilon_k$. Repeating this construction for every $k\in\nats$, we obtain a sequence $\{ \mathbb{E}_{P_k}[f(X_v)\,\vert\,Y_u\in O_u^{i_k}] \}_{k\in\nats}$. We now first show that this sequence also converges to $\mu_{k_*}$. To this end, choose any $\epsilon>0$. Because $\{\mu_{i_k}\}_{k\in\nats}\to\mu_{k_*}$, there is some $n\in\nats$ such that, for all $k>n$, $\abs{\mu_{i_k}-\mu_{k_*}}<\nicefrac{\epsilon}{2}$. Furthermore, because $\{\epsilon_k\}_{k\in\nats}\to 0^+$, there is some $m\in\nats$ such that for all $k>m$, $\epsilon_k<\nicefrac{\epsilon}{2}$. Now choose any $k>\max\{n,m\}$. Then,
\begin{align*}
\abs{\mathbb{E}_{P_k}[f(X_v)\,\vert\,Y_u\in O_u^{i_k}] - \mu_{k_*}} &= \abs{\mathbb{E}_{P_k}[f(X_v)\,\vert\,Y_u\in O_u^{i_k}] - \mu_{i_k} + \mu_{i_k} - \mu_{k_*}}
\end{align*}
\end{proof}

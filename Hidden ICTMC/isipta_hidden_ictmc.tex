...
To make this quantity computable, we now furthermore assume that $P_{\observs\vert\states}$ is $\sigma$-additive on $\Sigma$, and absolutely continuous with respect to Lebesgue measure $\lambda$ on $(\observs,\Sigma)$. For any $u\in\mathcal{U}_\emptyset$, we then let $\phi_u:\observs_u\times\states_u\to\realsnonneg$ be the unique function that is defined, for all $x_u\in\states_u$ and $y_u\in\observs_u$, by
...
In order to update a precise model $P$, we now require that $\mathbb{E}_P[\phi_u(y_u\vert X_u)]>0$. If this property does not hold, the updated expectation is left undefined. If it does hold, we have the following result.
...
The imprecise model will again be updated using regular extension, as follows.
\begin{definition}

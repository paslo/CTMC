...
The remainder of this paper is organised as follows. We introduce our notation in Section~\ref{sec:prelim}, and there also briefly recall the definitions and most important properties of ICTMC's. In Section~\ref{sec:icthmc}, we formally introduce ICTHMC's. Next, in Section~\ref{sec:updating_model}, we discuss the problem of making inferences about the latent process, given some observations; and then, in Section~\ref{sec:inference_algos}, we provide polynomial runtime algorithms to solve this inference problem for some important classes of functions. We then compare and relate these results to other work in the literature, in Section~\ref{sec:related}, and finally close with conclusions and some pointers to future work, in Section~\ref{sec:conclusions}.
...
Since we are working in a continuous-time setting, a \emph{time-point} is an element of $\realsnonneg$, and these are typically denoted $t$ or $s$. We also make extensive use of finite sequences of time points $u\subset\realsnonneg$. These are taken to be ordered, so that they may be written $u=t_0,\ldots,t_n$, for some $n\in\nats$, and such that then $t_i<t_j$ for all $i,j\in\{0,\ldots,n\}$ for which $i< j$. Such sequences are usually denoted $u$ or $v$, and we denote the entire set of them by $\mathcal{U}$. We use set-theoretic notation to operate on them, while preserving the ordering requirement; i.e., we write $u\cup v$ for the ordered union of $u$ and $v$.

Throughout, we consider some fixed, finite state space $\states$. A generic element of $\states$ will be denoted $x$. When consider the state-space at a specific time $t$, we write $\states_t\coloneqq\states$, and $x_t$ denotes a generic state-assignment at this time. When considering multiple time points $u$ simultaneously, we consider the joint state-space $\states_u\coloneqq\prod_{t_i\in u}\states_{t_i}$, a generic element of which is denoted $x_u=(x_{t_0},\ldots,x_{t_n})$.

\subsection{Continuous-Time Stochastic Processes}

** Minimum workable definition?

*** coherent conditional prob

*** rate matrix $\Leftarrow$ needs transition matrix
...
Bla *** maybe just focus on $\wprocesses_{\rateset,\mathcal{M}}$, with $\rateset$ convex and s.s.r.?
\end{definition}

*** Required properties: 

***** iterated lower expectation

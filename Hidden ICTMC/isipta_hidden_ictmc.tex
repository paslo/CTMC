...
Moving on, the imprecise model is here updated using \emph{natural extension}~\citep{Walley:1991vk}:
...
The reason that we here choose to update the model using natural extension, rather than regular extension, is that the latter leads to some interpretational difficulties that are somewhat outside the scope of the present paper\footnote{The condition $P(Y_u\in O_u^i)>0$ in Footnote~\ref{fnote:extra_condition} is not very problematic when focussing on single processes $P$, but we are here considering \emph{sets} of processes, and for a given sequence $\{O_u^i\}_{i\in\nats}$ this condition may hold for some processes in the set, yet fail to hold for others.}. In contrast, natural extension leads to the intuitive limit interpretation:
...
Note that this function $G(\mu)$ in Proposition~\ref{prop:GBR_properties} can therefore behave in two essentially different ways. These correspond to the cases where the observed event has strictly positive probability(/density) for \emph{all} processes in the ICTHMC, and to where it only has positive probability(/density) for \emph{some} of these processes.

However, note that in either case, the function is ``well-behaved'' enough to make finding its maximum root a fairly straightforward task. For instance, a standard bisection algorithm can be straightforwardly applied here; known in this context as Lavine's algorithm~\citep{lavine1991sensitivity}. Other, more efficient, algorithms are presented by, e.g., \citet{cozman1997alternatives}. Also note that, due to Propositions~\ref{prop:GBR_regular} and~\ref{prop:GBR_for_densities_lower_zero}, this maximum root will always be found in the interval $[\min f, \max f]$---this observation might be helpful in practice.
...
\end{equation*}
\end{proposition}
So, in order to evaluate the function of interest, it remains to compute $\lexp\left[g_{t_0}^+(X_{t_0})\right]$. Since $g_{t_0}^+$ is a function on a single time point $t_0$, this can again be done in polynomial time, using Equation~\eqref{eq:unconditional_lower_exp} in Section~\ref{subsec:ICTMC_computations} above.

\section{Conclusions and Future Work}\label{sec:conclusions}

*** brief summary

*** future work: imprecise observables, maximal state decoding, arbitrary functions in the future:

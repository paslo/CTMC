...
\end{proposition}
\begin{proof}
We give a proof by counterexample. To this end, let $\states\coloneqq\{x,\overline{x}\}$ be a binary state-space, and let $\observs\coloneqq \reals$, with $\Sigma$ the Borel $\sigma$-algebra on $\observs$ under the usual topology.

For the sequences of time-points, we choose $u=v=\{0\}$, and we will drop the corresponding time-point subscripts for brevity. Set $y\coloneqq 0$.

The trick will be to choose the measures $P_{\observs\vert\states}(y\vert x)$ and $P_{\observs\vert\states}(y\vert \overline{x})$ so that one of them gives $y$ strictly positive support (i.e. density), while the other assigns zero support to $y$ but positive support (i.e. density) to the region around $y$. So, let $P_{\observs\vert\states}(\cdot\vert x)$ be the standard Normal distribution with standard deviation one and mean equal to $y=0$.

We construct the measure $P_{\observs\vert\states}(y\vert \overline{x})$ in reverse, by constructing $\phi(\cdot\vert\overline{x})$ explicitly. To this end, for every $\tilde{y}\in\observs$, let
\begin{equation*}
\phi(\tilde{y}\vert\overline{x}) \coloneqq \left\{\begin{array}{ll}
\abs{\tilde{y}} & \text{if $\tilde{y}\in[-1,1]$, and} \\
0 & \text{otherwise.}
\end{array}\right.
\end{equation*}
Clearly, $\int_\observs \phi(\tilde{y}\vert\overline{x})\,\mathrm{d}\tilde{y} = 1$, and furthermore $\phi(y\vert\overline{x})=0$. For every $O\in\Sigma$, let $P_{\observs\vert\states}(O\vert\overline{x})$ be defined by
\begin{equation*}
P_{\observs\vert\states}(O\vert\overline{x}) \coloneqq \int_O\phi(\tilde{y}\vert\overline{x})\,\mathrm{d}\tilde{y}\,.
\end{equation*}
For any sequence $\{O^i\}_{i\in\nats}$ of open sets around $y$ that shrink to zero, we then clearly have
\begin{equation*}
\phi(y\vert x) = \frac{1}{\sqrt{2\pi}} = \lim_{i\to+\infty} \frac{P_{\observs\vert\states}(O^i\vert x)}{\lambda(O^i)}\,,
\end{equation*}
and
\begin{equation*}
\phi(y\vert \overline{x}) = 0 = \lim_{i\to+\infty} \frac{P_{\observs\vert\states}(O^i\vert \overline{x})}{\lambda(O^i)}\,,
\end{equation*}
with $\lambda(O^i)$ the Lebesgue measure of $O^i$. Fix any such sequence $\{O^i\}_{i\in\nats}$, and let $\{\lambda_i\}_{i\in\nats}\coloneqq\{\lambda(O^i)\}_{i\in\nats}$ be the corresponding sequence.

Construct the set of initial distribution as $\mathcal{M}\coloneqq\{P,\overline{P}\}$, where $P,\overline{P}$ are probability mass functions on $\states_0$ such that $P(x)=1$, $P(\overline{x})=0$, and $\overline{P}(x)=0$, $\overline{P}(\overline{x})=1$. Let $\rateset$ be any non-empty, bounded, and convex set of rate matrices with separately specified rows, and let $\mathbb{P}_{\rateset,\mathcal{M}}$ be the corresponding ICTMC. Construct $\mathcal{Z}$ from $\mathbb{P}_{\rateset,\mathcal{M}}$ and $(\observs,\Sigma,P_{\observs\vert\states})$.

Clearly, $\uexp[\phi(y\vert X_0)]=\overline{\mathbb{E}}_\mathcal{M}[\phi(y\vert X_0)] = P(x)\phi(y\vert x) + P(\overline{x})\phi(y\vert \overline{x})=P(x)\phi(y\vert x) = \frac{1}{\sqrt{2\pi}}>0$, and so we can apply regular extension.

Let $f\in\gambles(\states_0)$ be defined by $f(x)\coloneqq 1$ and $f(\overline{x})\coloneqq -1$. Then, because $\mathbb{E}_P[\phi(y\vert X_0)] > 0$, by Proposition~\ref{prop:precise_bayes_rule_densities} we have that
\begin{equation*}
\mathbb{E}_P[f(X_0)\,\vert\,Y=y] = \frac{\mathbb{E}_P[f(X_0)\phi(y\vert X_0)]}{\mathbb{E}_P[\phi(y\vert X_0)]} = \frac{f(x)\phi(y\vert x)}{\phi(y\vert x)} = f(x) = 1\,.
\end{equation*}
On the other hand, $\mathbb{E}_{\overline{P}}[\phi(y\vert X_0)]=0$. Hence,
\begin{align*}
\underline{\mathbb{E}}_\mathcal{Z}^\mathrm{R}[f(X_0)\,\vert\, Y=y] &= \inf\left\{ \mathbb{E}_{P'}[f(X_0)\,\vert\,Y=y]\,:\,P'\in\mathcal{Z}, \mathbb{E}_{P'}[\phi_u(y\vert X_0)]>0  \right\} \\
 &= \mathbb{E}_P[f(X_0)\,\vert\,Y=y] \\
 &= 1\,.
\end{align*}
However, for any $i\in\nats$ we have $\mathbb{E}_P[P_{\observs\vert\states}(O^i\vert X_0)]>0$, and hence
\begin{align*}
\mathbb{E}_P[f(X_0)\,\vert\,Y\in O^i] = \frac{\mathbb{E}_P[f(X_0)P_{\observs\vert\states}(O^i\vert X_0)]}{\mathbb{E}_P[P_{\observs\vert\states}(O^i\vert X_0)]} = \frac{f(x)P_{\observs\vert\states}(O^i\vert x)}{P_{\observs\vert\states}(O^i\vert x)} = f(x) = 1\,,
\end{align*}
and, since also $\mathbb{E}_{\overline{P}}[P_{\observs\vert\states}(O^i\vert X_0)]>0$,
\begin{align*}
\mathbb{E}_{\overline{P}}[f(X_0)\,\vert\,Y\in O^i] = \frac{\mathbb{E}_{\overline{P}}[f(X_0)P_{\observs\vert\states}(O^i\vert X_0)]}{\mathbb{E}_{\overline{P}}[P_{\observs\vert\states}(O^i\vert X_0)]} = \frac{f(\overline{x})P_{\observs\vert\states}(O^i\vert \overline{x})}{P_{\observs\vert\states}(O^i\vert \overline{x})} = f(\overline{x}) = -1\,.
\end{align*}
It follows that for every $i\in\nats$, we have $\underline{\mathbb{E}}_\mathcal{Z}[f(X_0)\vert Y\in O^i]=-1$. Hence,
\begin{equation*}
-1 =  \lim_{i\to+\infty} \underline{\mathbb{E}}_\mathcal{Z}[f(X_0)\vert Y\in O^i] \neq \underline{\mathbb{E}}_\mathcal{Z}^\mathrm{R}[f(X_0)\,\vert\, Y=y] = 1\,.
\end{equation*}
\end{proof}

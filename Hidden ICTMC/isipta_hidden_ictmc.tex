...
\end{corollary}
\begin{proof}
Trivial consequence of Lemmas~\ref{lemma:general_regular_extension},~\ref{lemma:state_expectation_doesnt_depend_on_outputs}, and~\ref{lemma:state_lower_expectation_doesnt_depend_on_outputs}.
\end{proof}
...
TODO:
\begin{itemize}
\item Limit corresponds to density (straightforward from Lebesgue differentiation theorem)
\item Limit is independent if continuous and every conditioning event has positive prob
\end{itemize}
\end{proof}
...
The following two proposition prove the properties of the first of the alternative imprecise updating methods suggested in Section~\ref{subsec:uncountable}.
...
\begin{equation*}
\lexp\left[g_{t_0}^+(X_{t_0})\right] = \underline{\mathbb{E}}_{\rateset}\left[\lexp\left[\prod_{j=0}^{n}g_{t_j}(X_{t_j})\,\Bigg\vert\,X_{t_0}\right]\right] = \lexp\left[\prod_{j=0}^{n}g_{t_j}(X_{t_j})\right] = \lexp\left[\prod_{t\in u'}g_{t}(X_{t})\right]\,,
\end{equation*}

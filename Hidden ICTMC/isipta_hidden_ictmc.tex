...
Then, the corresponding ICTMC, denoted $\wprocesses_{\rateset,\mathcal{M}}$, is the set of all \emph{well-behaved} processes $P$ that satisfy $P(X_0)\in\mathcal{M}$, and $\overline{\partial} T_{t,x_u}^t \subseteq \rateset$ for all $t\in\realsnonneg$, all $u\in\mathcal{U}_{<t}$, and $x_u\in\states_u$.
\end{definition}

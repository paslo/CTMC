...
To this end, we write for any function $g:\Sigma_u\to\reals$, any $y_u\in\observs_u$ and $c\in\reals$, that $\lim_{C_u\to y_u}g(C_u)=c$ if, for every sequence of \emph{open} cubes\footnote{Under some appropriate metric and topology on $\observs$} $\{C_i\}_{i\in\nats}$ in $\Sigma_u$ that are centred on $y_u$, and satisfy $\cap_{i\in\nats}C_i=\{y_u\}$ and $C_i\supset C_{i+1}$ for all $i\in\nats$, it holds that $\lim_{i\to\infty}g(C_i)$ exists, and is equal to $c$.

The updated (precise) expectation for any process $P$ will then be defined as
\begin{equation*}
\mathbb{E}_P[f(X_v)\,\vert\,Y_u=y_u] \coloneqq \lim_{C_u\to y_u} \mathbb{E}_P[f(X_v)\,\vert\,Y_u\in C_u]\,,
\end{equation*}
whenever this limit exists, and is left undefined, otherwise.

In order to make this concept practicable, we will henceforth assume that if $\observs$ is uncountable, then it is some set $\observs\subseteq\reals^d$ with the usual topology, for some $d\in\nats$, and that $\Sigma$ is the restriction to $\observs$ of the Borel $\sigma$-algebra on $\reals^d$. We then also assume that $P_{\observs\vert\states}$ is $\sigma$-additive on $\Sigma$, and absolutely continuous with respect to Lebesgue measure $\lambda$ on $(\observs,\Sigma)$.

For any $u\in\mathcal{U}_\emptyset$, we now let $\phi_u:\observs_u\times\states_u\to\realsnonneg$ be the unique function that is defined, for all $x_u\in\states_u$ and $y_u\in\observs_u$, by
\begin{equation}\label{eq:density_is_limit}
\phi_u(y_u\vert x_u) \coloneqq \lim_{C_u\to y_u} \frac{P_{\observs_u\vert\states_u}(C_u\vert x_u)}{\lambda(C_u)}\,,
\end{equation}
whenever this limit exists, and by $\phi_u(y_u\vert x_u)\coloneqq 0$, otherwise, where $\lambda(C_u)$ denotes Lebesgue measure of $C_u\subseteq\observs_u$, and where we assume $\frac{0}{0}=0$. Under the assumptions above, the limit in Equation~\eqref{eq:density_is_limit} can be shown to exist for $\lambda$-almost every $y_u\in\observs_u$; that is, it is guaranteed to exist everywhere outside a Lebesgue null-set. Furthermore, for every $O_u\in\Sigma_u$ and $x_u\in\states_u$, this function $\phi_u$ satisfies
...
\newline
\begin{proof}
Due to the assumptions above, and by the Radon-Nikodym theorem, we can associate with $P_{\observs\vert\states}$ a measurable function $\psi_u:\observs_u\times\states_u\to\realsnonneg$ such that, for all $x_u\in\states_u$ and $O_u\in\Sigma_u$,
\begin{equation*}
...
\begin{equation*}
\psi_u(y_u\vert x_u) = \lim_{C_u\to y_u} \frac{1}{\lambda(C_u)} \int_{C_u} \psi(\tilde{y}_u\vert x_u)\,\mathrm{d}\tilde{y}_u = \lim_{C_u\to y_u} \frac{P_{\observs_u\vert\states_u}(C_u\vert x_u)}{\lambda(C_u)}\,.
\end{equation*}
...
In order to update a precise model $P$, we now require that $\mathbb{E}_P[\phi(y_u\vert X_u)]>0$. If this property does not hold, the updated expectation is left undefined. If it does hold, we have the following result.

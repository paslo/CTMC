...
A probability tree $(\states^*_\Box,\prec,p_{(\cdot)})$ is called a Markov chain if $p_v=p_w$ for all situations $v,w\in\states^*$ for which $\lvert v\rvert=\lvert w\rvert$ and $v_\top=w_\top$.
\end{definition}
\begin{figure}
\centering
\begin{tikzpicture}[xscale=0.9,yscale=0.9]

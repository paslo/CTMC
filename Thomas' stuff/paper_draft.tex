...
\subsection{Connections Between $L_t^s$ and Lower Expectations $\underline{\mathbb{E}}$}
...
As the following result shows, the operator $L_t^s$ induced by a lower transition rate operator $\lrate$ corresponds to a lower bound on the expectation of a function at time $s$, with respect to the set $\processes_\rateset$ of non-Markov processes induced by any set $\rateset$ of which $\lrate$ is the lower envelope.
...
As this previous result showed, $L_t^sf$ is a lower bound on the expectation of a function $f\in\gamblesX$, with respect to a set of non-Markov processes $\processes_\rateset$ induced by some non-empty bounded set of rate matrices $\rateset$. Our next result establishes that this bound is tight if $\rateset$ also has separately specified rows. Specifically, we show that $L_t^sf$ can then be approximated to arbitrary precision by carefully choosing a Markov process $P$ from the set $\mprocesses_\rateset$.
...
\subsection{*** functies met een tijdstip in de toekomst en de rest in het verleden ***}

*** deze moeten nog even netjes opgeschreven worden en wat uitleg ervoor dat dit triviaal gevolg is van eerdere sectie en eigenschappen van verwachtingswaarden ***

*** mogelijk is dit ook zonde van de ruimte, en volstaat het om het alleen in de tekst te beschrijven. we moeten alleen wel naar deze resultaten kunnen verwijzen omdat we ze in de volgende sectie gebruiken... *** 

Hence, we have the following results.
\begin{proposition}
In essentie, voor alle $f\in\gambles(\states^{u\cup\{s\}})$, alle $P\in\processes_\rateset$, en alle $(x_{t_0},\ldots,x_{t_n})\in\states^u$,
\begin{equation*}
\left[L_t^s f\right](x_{t_0},\ldots,x_{t_n}) \leq \mathbb{E}[f(x_{t_0},\ldots,x_{t_n},X_s)\,\vert\,X_{t_0,\ldots,t_n}=x_{t_0,\ldots,t_n}]\,.
\end{equation*}
\end{proposition}
\begin{proof}
...
\begin{corollary}
voor alle $f\in\gambles(\states^{u\cup\{s\}})$,
\begin{equation*}

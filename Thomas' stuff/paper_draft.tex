...
We will in this section formalize the notion of continuous-time stochastic processes. This is done in the framework of full conditional probabilities that we described in Section~\ref{sec:cond_prob}. This framework is used in Section~\ref{sec:finite_add_stoch_proc} to define stochastic processes, and Section~\ref{sec:well_behaved} describes a specific class of stochastic processes, which we call well-behaved, and on which we will largely focus throughout this work. Finally, Section~\ref{sec:dynamics} provides some tools with which we can describe the dynamics of stochastic processes.

\subsection{Finitely additive stochastic processes}\label{sec:finite_add_stoch_proc}

We will use the notion of full conditional probabilities to define continuous-time stochastic processes. These stochastic processes will be defined on a specific subset of the set $\power\times\nonemptypower$ of all (conditional) events, as follows.

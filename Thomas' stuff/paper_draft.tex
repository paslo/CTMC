...
\begin{equation*}
(\text{Healthy}) \rightleftarrows (\text{Sick})
\end{equation*}
...
\end{proposition}
\begin{proof}
*** still need to check this *** {\bf TODO} Surely we can find a reference for this...
\end{proof}
...
\end{proposition}
\begin{proof}
*** still need to check this *** {\bf TODO}
\end{proof}
...
\end{proposition}
\begin{proof}
*** still need to check this *** trivial ***
\end{proof}
...
We finally consider another tool that will be useful later; restrictions of transition matrix systems to some specified interval of the time-line $\realsnonneg$. An interval $\mathbf{I}$ is simply any subset $\mathbf{I}\subseteq\realsnonneg$ that is connected, meaning that for any $t,s\in\mathbf{I}$ such that $t\leq s$, and any $r\in[t,s]$, it holds that $r\in\mathbf{I}$. 

For any transition matrix system $\mathcal{T}$ and any interval $\mathbf{I}\subseteq\realsnonneg$, let $\mathcal{T}^\mathbf{I}$ denote the family of transition matrices $T_t^s$ corresponding to $\mathcal{T}$, for all $t,s\in\mathbf{I}$ such that $t\leq s$.
%\begin{equation*}
...
\end{proposition}
\begin{proof}
*** still need to check this *** Trivial consequence of Definition~\ref{def:trans_mat_system}.
\end{proof}
...
(\text{Healthy}) \rightleftarrows (\text{Sick})
\end{equation*}
Thus, the state-space here is of the form $\states=\{\text{Healthy},\text{Sick}\}$. This models a person periodically becoming sick and recovering after some time. 
...
In general, computing $\lrate f$ is a linear optimization problem. *** some notes about (lack of) difficulty of solving this

\begin{exmp}\label{exmp:numerical_lrate}
**** small example here somewhere

In the specific case of the set $\rateset$ from Example {\bf REF}, it is readily seen that $\lrate f$ is always given by some $Q f$, with $Q\in\rateset$ such that $a\in\{\nicefrac{1}{52}, \nicefrac{3}{52}\}$ and $b\in\{\nicefrac{1}{2},2\}$, using the notation from Equation~\eqref{eq:num_example_rateset_params}. Hence, we only need to consider four possible rate matrices $Q\in\rateset$ to find $\lrate f$ for any given $f\in\gamblesX$. As such, this optimization problem is particularly easy to solve in this case.
\exampleend
\end{exmp}
...
\begin{equation*}
\underline{P}(X_1 = \text{Sick}\,\vert\,X_0=\text{Sick}) \equiv \underline{\mathbb{E}}[\ind{\text{Sick}}(X_1)\,\vert\,X_0=\text{Sick}] \equiv \left[L_0^1\ind{\text{Sick}}\right](\text{Sick})\,.
\end{equation*}
Proposition~\ref{prop:approximation_error_bound} tells us that we can compute this quantity using a fine enough partition of the time interval $[0,1]$. We will be using a maximum error for this computation of $\epsilon\coloneqq 10^{-3}$. Furthermore, the length of the time interval $(s-t)=1$, the function norm $\norm{\ind{\text{Sick}}}=1$, and due to our choice of $\rateset$, we have $\norm{\lrate} = 1$. In order to reach the desired maximum error, we therefore need to subdivide the time interval into at least $n$ steps, where
\begin{equation*}
n = \frac{(s-t)^2\norm{\lrate}^2\norm{\ind{\text{Sick}}}}{\epsilon} = \frac{1^2\cdot 1^2\cdot 1}{10^{-3}} = 10^3\,,
\end{equation*}
resulting in a step size of $\Delta=\nicefrac{(s-t)}{n}=10^{-3}$. We now compute the quantity
\begin{equation*}
\prod_{i=1}^n(I + \Delta\lrate)\ind{\text{Sick}} = \prod_{i=1}^{1000}(I + \Delta\lrate)\ind{\text{Sick}} = \left(\prod_{i=1}^{999}(I + \Delta\lrate)\right)\left((I+\Delta\lrate)\ind{\text{Sick}}\right)\,.
\end{equation*}
We start by computing the right-most factor on the right-hand side of this equation. First compute $\lrate\ind{\text{Sick}}$, as in Example~\ref{exmp:numerical_lrate}:
\begin{align*}
\lrate\ind{\text{Sick}} &= \inf\left\{ Q\ind{\text{Sick}}\,:\,Q\in\rateset \right\} = \inf\left\{ Q \left[\begin{array}{c} 0\\ 1 \end{array}\right]\,:\,Q\in\rateset \right\} \\
...
Let now
\begin{equation*}
g_1 \coloneqq (I + \Delta\lrate)\ind{\text{Sick}} = \ind{\text{Sick}} + \Delta\lrate\ind{\text{Sick}} = \left[\begin{array}{c}0\\1\end{array}\right] + 10^{-3}\cdot\left[\begin{array}{c}\nicefrac{1}{52}\\2\end{array}\right] = \left[\begin{array}{l} 0 + \nicefrac{1}{52}\cdot 10^{-3}\\ 1 + 2\cdot 10^{-3}\end{array}\right]\,.
\end{equation*}
Having completed this first step, we have
\begin{equation*}
\prod_{i=1}^n(I + \Delta\lrate)\ind{\text{Sick}} = \prod_{i=1}^{999}(I + \Delta\lrate)g_1 = \left(\prod_{i=1}^{998}(I + \Delta\lrate)\right)\left((I+\Delta\lrate)g_1\right)\,.
\end{equation*}
We now repeat this process, computing the term $g_2\coloneqq(I+\Delta\lrate)g_1$. After $j$ steps, we then have
\begin{equation*}
\prod_{i=1}^n(I + \Delta\lrate)\ind{\text{Sick}} = \prod_{i=1}^{1000-j}(I + \Delta\lrate)g_j\,.
\end{equation*}
After repeating this process for $n$ steps, we have computed the term
\begin{equation*}
\prod_{i=1}^n(I + \Delta\lrate)\ind{\text{Sick}} = g_{1000} \approx \left[\begin{array}{c}0.0083 \\ 0.1408\end{array}\right]\,,
\end{equation*}
from which we conclude that
\begin{align*}
\underline{P}(X_1 = \text{Sick}\,\vert\,X_0=\text{Sick}) &\equiv \left[L_0^1\ind{\text{Sick}}\right](\text{Sick})
= g_{1000}(\text{Sick}) \pm \epsilon \approx 0.141 \pm 10^{-3}\,,
\end{align*}
...
\begin{align*}
\left[L_1^2f\right](\text{Healthy}) &= \left[L_1^2f(\text{Healthy}, X_2)\right](\text{Healthy}) \\
 &= \left[L_1^2\ind{\text{Healthy}}(X_2)\right](\text{Healthy})\,,\\
 &\text{and,} \\
\left[L_1^2f\right](\text{Sick}) &= \left[L_1^2f(\text{Sick}, X_2)\right](\text{Sick}) \\ 
 &= \left[L_1^2\ind{\text{Sick}}(X_2)\right](\text{Sick})\,.
\end{align*}
Using Proposition~\ref{prop:approximation_error_bound}, the quantities $L_1^2\ind{\text{Healthy}}$ and $L_1^2\ind{\text{Sick}}$ can now be computed using the same procedure outlined in Example~\ref{exmp:single_time_numerical}. We then find that
\begin{equation*}
...
\end{equation*}
In order to compute the lower expectation of interest, we once more apply Proposition~\ref{prop:approximation_error_bound} and the procedure from Example~\ref{exmp:single_time_numerical}, finding that
\begin{equation*}
...
\begin{align*}
\underline{P}_{\rateset}^{\mathrm{W}}(X_1 = X_2\,\vert\,X_0=\text{Healthy}) \approx 0.920,\quad\text{and,}\quad \underline{P}_{\rateset}^{\mathrm{W}}(X_1 = X_2\,\vert\,X_0=\text{Sick}) \approx 0.453\,.
\end{align*}
\exampleend
\end{exmp}
...
%		\If{$m=0$} \Comment{End recursion when just one time point remains.}
%			\State $g\gets g\in\gambles(\states^u)$ \Comment{Create new function $g$ in $\gambles(\states^u)$}
%			\For{$x_u\in\states^u$}
%				\State $g(x_u) \gets$ {\tt ComputeLf}$(\rateset, t_n, s_0, f(x_u,X_{s_0}),\epsilon)$
%			\EndFor
%			\State \Return{$g$} \Comment{Return $g=\left[L_{t_n}^{s_0}f(X_u,X_{s_0})\right](X_u)$\,.}
%		\EndIf
%		\State $w\gets v\setminus {s_m}$ \Comment{Define $w\coloneqq s_0,s_1,\ldots,s_{m-1}$\,.}
%		\For{$x_w\in\states^w$}
%		\EndFor
		\State $w \gets v \setminus s_m$	\Comment{Create $w\coloneqq s_0,s_1,\ldots,s_{m-1}$. If $m=0$ then $w=\emptyset$.}
		\State $r \gets s_{m-1}$ \Comment{Latest time point before $s_m$.}
		\If{$m=0$}
			\State $r \gets t_n$ \Comment{We use the notation $u=t_0,t_1,\ldots,t_n$.}
		\EndIf
		\Statex
		\State $g \gets g \in \gambles(\states^{u\cup w})$ \Comment{Create new function $g\in\gambles(\states^{u\cup w})$.}
		\For{$x_{u\cup w}\in\states^{u\cup w}$} \Comment{Compute all values for $g$.}
			\State $h \gets$ {\tt ComputeLf}$\bigl(\rateset, r, s_m, f(x_{u\cup w}, X_{s_m}),\epsilon\bigr)$ \Comment{Compute $L_r^sf(x_{u\cup w},X_s)\pm\epsilon$.}
			\State $g(x_{u\cup w}) \gets h(x_r)$\Comment{Set $g(x_{u\cup w})\coloneqq \left[L_r^{s_m}f(x_{u\cup w},X_s)\right](x_r)$, where $x_r\in x_{u\cup w}$.}
		\EndFor
		\Statex
		\If{$m=0$} \Comment{Check if no other time points in $v$ remain.}
			\State \Return{$g$} \Comment{If this is the case, we are done. Result is a function in $\gambles(\states^u)$.}
		\Else \Comment{Otherwise, we recurse on the remaining time points.}
			\State \Return{{\tt ComputeLowerExp}$(\rateset, u, w, g, \epsilon)$}
		\EndIf      
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
{\bf TODO} tegenvoorbeeld van vorige functie voor de set $\wmprocesses_\rateset$.
\exampleend
\end{exmp}
...
**** computation can also be brute-forced if state space and number of time points allow for it. Algorithm~\ref{alg:compute_multivar} does this, but this approach is exponential in the size of $v$; specifically the runtime is $O\left(m{\vert\states\vert}^m(s_m-t_0)^2\norm{\lrate}^2\norm{f}\epsilon^{-1}\right)$, with $u=t_0,t_1,\ldots,t_n$ and $v=s_0,s_1,\ldots,s_m$.

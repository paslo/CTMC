...
Then, $\mathcal{T}_*^{[t,s]}$ exists, is a restricted transition matrix system defined on $[t,s]$, and $\mathcal{T}_*^{[t,s]}=\lim_{i\to\infty}\mathcal{T}_i^{[t,s]}$. Furthermore, if for all $i\in\nats$ it holds that $\mathcal{T}_i^{[t,s]}$ is well-behaved, then $\mathcal{T}_*^{[t,s]}$ is also well-behaved.
\end{lemma}
\begin{proof}
...
We finally show that if each $\mathcal{T}_i^{[t,s]}$ is well-behaved, that then $\mathcal{T}_*^{[t,s]}$ is also well-behaved.
\end{proof}

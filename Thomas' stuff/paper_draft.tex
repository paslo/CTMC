...
\begin{equation*}
\mathcal{C}^{\mathrm{SP}*} \coloneqq \left\{(A,C)\,:\,A\in\sigma(\mathcal{A}_{>t}),C\in\mathcal{F}_{\leq t},t\in\realsnonneg\right\}\subset\power\times\nonemptypower\,,
\end{equation*}
where $\sigma(\mathcal{A}_{>t})$ is the $\sigma$-algebra generated by $\mathcal{A}_{>t}$, such that $P^*$ is a coherent conditional probability that is $\sigma$-additive on $\mathcal{C}^{\mathrm{SP}*}$.
\end{proposition}
\begin{proof}

...
{\v{S}kulj}'s uniform grid discretisation method works similarly to our Algorithm~\ref{alg:compute_singlevar}, in that it considers a uniform partition $u=t_0,t_1,\ldots,t_n$ of the time interval $[0,s]$ and, at each time point $t_i$, requires solving a local optimisation problem. In our case, this local computation consists of evaluating $\smash{(I+\Delta\lrate)g=g+\Delta\lrate g}$ for some $g\in\gamblesX$, with $\Delta=\nicefrac{s}{n}$. In the uniform grid discretisation method of {\v{S}kulj}, this local computation consists of evaluating $e^{\Delta Q}g$, where $Q\in\rateset$ needs to be selected in such a way that $Qg=\lrate g$. 

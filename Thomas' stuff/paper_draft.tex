...
A general method for computing the conditional lower expectation in Equation~\eqref{eq:lowerexpmultipletimepoints} is outlined in Algorithm~\ref{alg:compute_multivar}. This algorithm starts by constructing a buffer $w\coloneqq v=s_0,\ldots,s_m$ of the time points for which the operators $L_{t_n}^{s_0},L_{s_1}^{s_2},\ldots,L_{s_{m-1}}^{s_m}$ have not yet been resolved, which initially corresponds to just $v$ (Line 2). We rename the function $f$ to $g_m$ for indexing purposes (Line 3). The algorithm then iteratively resolves the composition of operators in a backward fashion, working from the last time point in $v$ back to the first time point in $v$ (Line 4). One step here amounts to removing the current latest remaining time point $s_i$ from the buffer $w$ (Line 5), and then computing the lower expectation $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[g_i(X_u,X_w,X_{s_i})\vert X_u,X_w]$ using Algorithm~\ref{alg:compute_singlevar_multiple_past}. The result of this computation is a function $g_{(i-1)}\in\gambles(\states_{u\cup w})$. By continuing in this way, the resulting iterative process corresponds to the repeated decomposition and resolution of lower expectations, as explained in the beginning of this section. After all time points $s_m,s_{m-1},\ldots,s_0$ have been resolved, it follows from Corollary~\ref{cor:composition_lower_trans} and Proposition~\ref{prop:approximation_error_bound} that the returned function $g_{(-1)}$ (Line 8) is equal to $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_u,X_v)\vert X_u]\pm\epsilon$.
...
\begin{algorithm}[htb]
  \caption{Numerically compute $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_u,X_v)\,\vert\,X_u]$ for any $f\in\gambles(\states_{u\cup v})$.}
    \label{alg:compute_multivar}
  \begin{algorithmic}[1]
  \vspace{4pt}
    \Require{The lower transition rate operator $\lrate$ of a non-empty, bounded and convex set of rate matrices $\rateset$ with separately specified rows, two sequences of time points $u=t_0,\ldots,t_n$ and $v=s_0,\ldots,s_n$ in $\mathcal{U}_\emptyset$ such that $u<v$, a function $f\in\gambles(\states_{u\cup v})$, and a maximum numerical error $\epsilon\in\realspos$.}\vspace{4pt}
\Ensure{A function $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_u,X_v)\,\vert\,X_u]\pm\epsilon$ in $\gambles(\states_u)$.}
\vspace{-5pt}
	 \Statex
    \Function{ComputeLowerExp}{$\lrate, u,v, f,\epsilon$}
...
In particular, the choice of $\mathcal{M}$ will influence our computations through the corresponding initial lower expectation operator $\underline{\mathbb{E}}_{\mathcal{M}}$, defined by
...
Without loss of generality, we assume that $0\in u$. Whenever this is not the case, it suffices to first replace $f(X_u)$ by its trivial extension $f^*(X_0,X_u)$ to $\states_{0\cup u}$, defined by $f^*(x_0,x_u)\coloneqq f(x_u)$ for all $x_0\in\states$ and $x_u\in\states_u$. This addition of $X_0$ to the domain is merely a notational trick: since $f^*(X_0,X_u)$ is identical to $f(X_u)$, it clearly does not influence the result of the computations. It does however provide us with the formal requirements to apply Theorem~\ref{theorem:decomposition_multivar} and Corollary~\ref{cor:composition_lower_trans}. From the former, we infer that 
...
To the best of our knowledge, the concept of an imprecise continuous-time Markov chain was first introduced in the literature by the work of {\v{S}}kulj~\cite{Skulj:2015cq}. There, $\lrate$ was defined as the lower envelope of a non-empty, closed, convex set of rate matrices $\rateset$ with separately specified rows, and the notion of an imprecise continuous-time Markov chain was then introduced as follows.

For any $f\in\gamblesX$, {\v{S}}kulj starts by considering a time-dependent function $f_s$, which---using our notation---is interpreted as $f_s\coloneqq T_0^s f$, and then requires that $f_0=f$,
\begin{equation*}
\lrate f_s
\leq
D^+f_s\coloneqq
...
\leq\overline{Q} f_s,
\end{equation*}
where $\overline{Q}$ is the upper envelope of $\rateset$, or equivalently,  the conjugate upper transition rate operator of $\lrate$, defined by $\overline{Q}g\coloneqq-\lrate(-g)$ for all $g\in\gamblesX$, and where $D^+f_s$ and $D_-f_s$ are called Dini derivatives. In this way, he mimics the usual differential equation characterisation of a precise homogeneous Markov chain, where the the Dini derivatives would be replaced by derivatives, where the operator $\lrate$ would be replaced by a transition rate matrix $Q$, and where the inequalities would become equalities.

Next, {\v{S}}kulj goes on to define $\underline{f}_s$ and $\overline{f}_s$ as the unique solutions of the differential equations
\begin{equation*}
\frac{d \underline{f}_{\,s}}{d s}=\lrate\,\underline{f}_{\,s}
~~~\text{and}~~~
\frac{d \overline{f}_{\,s}}{d s}=\overline{Q}\,\overline{f}_{\,s},\vspace{8pt}
\end{equation*}
with $\underline{f}_0=\overline{f}_0=f$, and proves that $\underline{f}_s\leq f_s\leq\overline{f}_s$.
...
\underline{\mathbb{E}}^\mathrm{W}_{\rateset,\mathcal{M}}[f(X_0)]
=\underline{\mathbb{E}}_{\mathcal{M}}[f]=\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^{\mathrm{W

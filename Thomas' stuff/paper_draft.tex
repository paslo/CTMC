...
In the imprecise continuous-time Markov chains that we consider, these four conditions are relaxed in the following way. First of all, instead of providing an exact value for the initial probabilities and the rate parameters, we allow ourselves to specify a set of values. For the purpose of this example, we take these sets to be intervals. The other three conditions are then dropped completely, provided that they remain compatible with these intervals. For example, a rate parameter does not need to be constant, but can vary in time in an arbitrary way, as long as it remains within its interval. Similarly, the rate parameters are also allowed to depend on previous states. In fact, they do not even need to exist, since we do not require differentiability either.
\exampleend
\end{exmp}

...
\end{lemma}
\begin{proof}
Since $\emptyset\neq u\subseteq w$, $s\in w$ and $s>u$, there are $j^*,j\in\{0,\dots,m\}$ such that $s=w_j$ and $u\subseteq\{w_0,\dots,w_{j-1}\}$. Let $u_i^\mathrm{c}\coloneqq\{w_0,\dots,w_{j-1}\}\setminus u_i$.

...
In particular, a bettor will specify a price $P(A\vert C)$ for which she is willing to both buy and sell the gamble---with a reward of one currency unit---that event $A$ will come true, given that event $C$ comes true. These gambles are then bought and sold under the proviso that, should event $C$ not happen, then all parties involved are refunded. 
...
Now, because the bettor quoted prices $P(A\vert C)$ for each $(A,C)\in\mathcal{C}$, we also have to consider combinations of gambles. Specifically, after quoting her prices, the bettor can be forced to buy and/or sell any number of gambles in any combination. Her prices $P$ are then said to be \emph{coherent}, as in the definition above, if there exists an outcome $\omega$ for which she does not make a loss. The well-known result from coherent probability theory is then that her prices $P$ are coherent if and only if they satisfy the axioms of probability, i.e., properties~\ref{def:coh_prob_2}-\ref{def:coh_prob_6} above.

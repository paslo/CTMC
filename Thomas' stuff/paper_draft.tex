...
Note that such a sequence $u$ is always a finite set of real-valued time points, meaning that $u\subset\realsnonneg$. As such, we use set-theoretic notation to operate on such sequences, where the preservation of the ordering is implied.  For example, for any $u,v\in\mathcal{U}$, we let $u\cup v$ denote the ordered union of $u$ and $v$. Similarly, for any $s\in\realsnonneg$ and any $u\in\mathcal{U}_{<s}$ with $u=t_0,\ldots,t_n$, we denote with $u\cup\{s\}$ the sequence $t_0,\ldots,t_n,s$.

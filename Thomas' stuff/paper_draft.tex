...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}[1]
  \vspace{2pt}
    \Require{A lower transition rate operator $\lrate$, two time points $t,s\in\realsnonneg$ such that $t\leq s$, a function $f\in\gamblesX$ and a maximum numerical error $\epsilon\in\realspos$.}\vspace{2pt}    
...
\begin{equation*}
\underline{P}(X_1 = X_2\,\vert\, X_0) = \underline{\mathbb{E}}[I_{X_1=X_2}(X_1,X_2)\,\vert\,X_0],\vspace{4pt}
\end{equation*}
with $I_{X_1=X_2}$ the indicator of the event $X_1=X_2$, as defined by
\begin{equation*}
I_{X_1=X_2}(x_1,x_2) \coloneqq
\begin{cases}
1 & \text{if $x_1 = x_2$} \\
0 & \text{if $x_1 \neq x_2$}
\end{cases}
...
For now, in this example, we use the \ictmc~$\smash{\mathbb{P}_{\rateset,\mathcal{M}}^{\mathrm{W}}}$, with $\mathcal{M}$ some arbitrary set of probability mass functions on $\states$. The aim is therefore to compute the conditional lower expectation $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^{\mathrm{W}}[I_{X_1=X_2}(X_1,X_2)\,\vert\,X_0]$. Technically speaking, we cannot do this by means of Corollary~\ref{cor:composition_lower_trans}, because $I_{X_1=X_2}$ does not formally depend on $X_0$. However, in practice, this of course not a problem, because we can trivially regard $I_{X_1=X_2}$ as a function of $X_0$, $X_1$ and $X_2$ whose value remains constant on $X_0$. In order to formalize this, we introduce the function $f(X_0,X_1,X_2)$, defined by
\begin{equation*}
f(x_0,x_1,x_2)\coloneqq I_{X_1=X_2}(x_1,x_2)
=
\begin{cases}
1 & \text{if $x_1 = x_2$} \\
0 & \text{if $x_1 \neq x_2$}
...
\end{equation*}
Since $f(X_0,X_1,X_2)$ is clearly identical to $I_{X_1=X_2}(X_1,X_2)$, our problem now consist in computing $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^{\mathrm{W}}[f(X_0,X_1,X_2)\,\vert\,X_0]$, which, according to Corollary~\ref{cor:composition_lower_trans}, is equal to
\begin{equation}\label{eq:num_example_composition}
\underline{\mathbb{E}}_{\rateset}^\mathrm{W}[f(X_0,X_1,X_2)\,\vert\,X_0] \equiv \left[L_0^1L_1^2f\right](X_0).
\end{equation}
...
\begin{align*}
\underline{P}_{\rateset}^{\mathrm{W}}(X_1 = X_2\,\vert\,X_0=\text{{\tt h}}) \approx 0.920,\quad\text{and,}\quad \underline{P}_{\rateset}^{\mathrm{W}}(X_1 = X_2\,\vert\,X_0=\text{{\tt s}}) \approx 0.453\,.
\end{align*}
\exampleend
\end{exmp}
...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}[1]
  \vspace{2pt}
    \Require{The lower transition rate operator $\lrate$ of a non-empty, bounded and convex set of rate matrices $\rateset$ with separately specified rows, two sets of time points $u,v\in\mathcal{U}_\emptyset$ such that $u< v$, a function $f\in\gambles(\states_{u\cup v})$, and a maximum numerical error $\epsilon\in\realspos$.}\vspace{2pt}
...
%		\Statex
		\For{$i\in\{m,m-1,\ldots,0\}$}	\Comment{Iterate backward over $v=s_0,\ldots,s_m$.}
			...
			\State $g_{(i-1)}\gets ${\textsc{ComputeLuf}}($\rateset,u\cup w,s_i,g_i,\nicefrac{\epsilon}{m}$)  \Comment{Run Algorithm~\ref{alg:compute_singlevar_multiple_past}}
%			\Statex
		\EndFor
%		\Statex

...
The notion of coherence is perhaps easiest to understand in a gambling framework. In this framework, we first consider a set $\mathcal{C}\subseteq\power\times\nonemptypower$ of possible events. Gambles---lottery tickets, if you will---are then bought and sold prior to observing the outcome $\omega$ of a random experiment. 
...
Similarly, for each such ticket that she sold, she has received $P(A\vert C)$ amount of currency in advance, and must pay $\ind{A}(\omega)$ to its buyer. Because this is conditional on event $C$ coming true, her profit after paying is $\ind{C}(\omega)(P(A\vert C) - \ind{A}(\omega))$, per ticket sold. Comparing to the above, it follows that, for any given outcome $\omega$, her resulting profit from buying is equal to her loss from selling, and vice versa.
...
The prices $P$ are now said to be \emph{coherent} if, regardless of this choice of gambles, there exists an outcome $\omega$ in which at least one of the conditioning events $C_i$ came true, and for which she does not make a loss. That is, the prices $P$ are coherent if they \emph{avoid sure loss}. Note that if none of the events $C_i$ come true, i.e. if $\omega\notin \cup_{i=1}^n C_i$, then she would trivially not make a loss because she would get refunded on all the gambles. However, she also has to avoid sure loss if (some) of the conditioning events do come true. 

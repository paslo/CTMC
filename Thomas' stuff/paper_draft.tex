...
For any sequence $\left\{\mathcal{T}_i^{[t,s]}\right\}_{i\in\nats}$ of restricted transition matrix systems, we define the limit---if it exists---as the family $\mathcal{T}_*^{[t,s]}\coloneqq\lim_{i\to\infty}\mathcal{T}_i^{[t,s]}$ of matrices defined for all $q,r\in[t,s]$ as
\begin{equation*}
\presuper{*}T_q^r \coloneqq \lim_{i\to\infty} \presuper{i}T_q^r\,,
\end{equation*}
where, for all $i\in\nats$, it is understood that $\presuper{i}T_q^r\in\mathcal{T}_i^{[t,s]}$.

We have the following results.

\begin{lemma}
Consider any Cauchy sequence $\left\{\mathcal{T}_i^{[t,s]}\right\}_{i\in\nats}$ of restricted transition matrix systems. Then, the limit $\mathcal{T}_*^{[t,s]}\coloneqq\lim_{i\to\infty}\mathcal{T}_i^{[t,s]}$ exists, and is a restricted transition matrix system. Furthermore, if for all $i\in\nats$ it holds that $\mathcal{T}_i^{[t,s]}$ is well-behaved, then $\mathcal{T}_*^{[t,s]}$ is also well-behaved.
\end{lemma}
\begin{proof}
...
Because the sequence $\left\{\mathcal{T}_i^{[t,s]}\right\}_{i\in\nats}$ is Cauchy, it follows from Lemma~{\bf REF} that the limit $\mathcal{T}_*^{[t,s]}$ exists, and is a restricted transition matrix system. Furthermore, because for all $i\in\nats$ it holds that $\mathcal{T}_i^{[t,s]}\in\mathbb{T}_\rateset^{[t,s]}$, we have that each $\mathcal{T}_i^{[t,s]}$ is well-behaved. Therefore, by Lemma~{\bf REF}, $\mathcal{T}_*^{[t,s]}$ is also well-behaved.

It remains to show that $\mathcal{T}_*^{[t,s]}\in\mathbb{T}_\rateset^{[t,s]}$.

...
The first step is to observe that $\nicefrac{1}{3}t\leq\varphi_1(t)\leq\nicefrac{2}{3}t$ and $\varphi_1(t)+\varphi_2(t)=t$ for every $t\in(0,1]$,; we leave this as an exercise. Given this observation, it follows that for all $t\in(0,1]$, there is some $\lambda_t\in[\nicefrac{1}{3},\nicefrac{2}{3}]$ such that $\varphi_1(t)=\lambda_t t$ and $\varphi_2(t)=(1-\lambda_t)t$. Hence, in particular, for every $i\in\nats$, there is some $\lambda_i\in[\nicefrac{1}{3},\nicefrac{2}{3}]$ such that $\varphi_1(\Delta_i)=\lambda_i \Delta_i$ and $\varphi_2(\Delta_i)=(1-\Delta_i)\Delta_i$. Furthermore, because of the Bolzano-Weierstrass theorem, the resulting sequence $\{\delta_i\}_{i\in\nats_0}$ contains a convergent subsequence $\{\delta_{i_k}\}_{k\in\nats_0}$ whose limit $\lambda\coloneqq\lim_{k\to+\infty}\delta_{i_k}$ clearly belongs to $[\nicefrac{1}{3},\nicefrac{2}{3}]$. In the remainder of this example, we will prove that $Q=Q_\lambda$.

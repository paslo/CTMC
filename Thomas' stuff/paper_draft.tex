...
\begin{equation*}
\mathcal{T}^{[t,r]} \otimes \mathcal{T}^{[r,s]} \coloneqq \mathcal{T}^{[t,r]} \cup \mathcal{T}^{[r,s]} \cup \left\{T_\tau^rT_r^{\sigma}\,\Big\vert\,T_{\tau}^r\in\mathcal{T}^{[t,r]},\,T_r^{\sigma}\in\mathcal{T}^{[r,s]},\,\forall\tau\in[t,r],\,\forall\sigma\in[r,s]\right\}\,.
\end{equation*}
\begin{proposition}
...
Because this sequence $\left\{\mathcal{T}_i^{[t,s]}\right\}_{i\in\nats}$ is Cauchy, it follows from Lemma~{\bf REF} that the limit $\mathcal{T}_*^{[t,s]}$ exists, and is a restricted transition matrix system. Furthermore, because for all $i\in\nats$ it holds that $\mathcal{T}_i^{[t,s]}\in\mathbb{T}_\rateset^{[t,s]}$, we have that each $\mathcal{T}_i^{[t,s]}$ is well-behaved. Therefore, by Lemma~{\bf REF}, $\mathcal{T}_*^{[t,s]}$ is also well-behaved. ***** {\bf TODO} actually have to show that it is uniformly well-behaved, but that's clearly true
...
\end{equation}
If $S_\emptyset=\emptyset$, we let $\omega^*$ be any element of $C_0$. Clearly, this path $\omega^*$ will then also satisfy Equation~\eqref{eq:theo:aanelkaarplakken:geqfirstpart}.
...
\}.
\end{equation*}
We first consider the case $S^*\neq\emptyset$. Then for all $i\in S^*$, we have that $(A_i,C_i)\in\mathcal{C}_{x_u^*}$ and therefore, there are $u\subseteq v_i\in\mathcal{U}$ and $x_{v_i\setminus u}\in\states^{v_i\setminus u}$ such that $C_i=(X_u=x_u^*)\cap(X_{v_i\setminus u}=x_{v_i\setminus u})$, which we use to define
\begin{equation*}
C_i^{*}\coloneqq(X_u=x_u^*)\cap(X_{v_i\setminus [0,\max u]}=x_{v_i\setminus [0,\max u]})
\end{equation*}
Since $P_{x_u^*}$ is a stochastic process, it then follows from Corollary~\ref{corol:processiffcoherent} that
\begin{equation*}%\label{eq:theo:aanelkaarplakken:secondpart}
\sup\left\{\sum_{i\in S^*}\lambda_i\ind{C_i^*}(\omega)\bigl(P_{x_u^*}(A_i\vert C_i^*)-\ind{A_i}(\omega)\bigr)~\Bigg\vert~\omega\in C^*\right\}\geq0,
\end{equation*}
...
\end{equation}
If $S^*=\emptyset$, we let $\omega^{**}$ be any element of $C_0$. Clearly, this path $\omega^{**}$ will then also satisfy Equation~\eqref{eq:theo:aanelkaarplakken:geqsecondpart}.
...
(The indicator of $C_i$/$C_i^*$ part holds because $\omega^{***}$ and $\omega^{*}$ agree before $\max u$ and because $C_i\cap E\neq\emptyset$. The indicator of $A_i$ part holds because $A_i$ only depends on time points in or beyond $u$ and because $\omega^{**}$ and $\omega^{***}$ coincide on those time points.)
\begin{equation*}
\sum_{i\in S^*}\lambda_i\ind{C_i}(\omega^{***})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{***})\bigr)
=
\sum_{i\in S^*}\lambda_i\ind{C_i^*}(\omega^{**})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{**})\bigr)\geq0,
\end{equation*}
and (this holds because $C_i$ and $A_i$ only depend on time points $\leq\max u$, and because $\omega^{***}$ and $\omega^{*}$ coincide on those time points)
\begin{equation*}
\sum_{i\in S_\emptyset}\lambda_i\ind{C_i}(\omega^{***})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{***})\bigr)
=
\sum_{i\in S_\emptyset}\lambda_i\ind{C_i}(\omega^{*})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{*})\bigr)\geq0,
\end{equation*}
and, for all $i\in\{1,\dots,n\}\setminus\{S_\emptyset\cup S^*\}$, $\ind{C_i}(\omega^{***})=0$ (this holds because $\omega^{***}(u)=\omega^{*}(u)=x_u^*\neq x_u$ or because $C_i\cap E=\emptyset$ (which, since $\omega^{***}\in E$, implies that $\omega^{***}\notin C_i$)). Therefore, we find that
\begin{equation*}
\sum_{i=1}^n\lambda_i\ind{C_i}(\omega^{***})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{***})\geq0,
\end{equation*}
which, since $\omega^{***}\in C_0$ (STILL NEED TO PROVE THIS!), implies that Equation~\eqref{eq:theo:aanelkaarplakken:coh} holds. Hence, $\tilde{P}$ is coherent.

...
\begin{figure}[t]
\begin{tikzpicture}
      \node[anchor=south west,inner sep=0] (plot) at (0,1) {\includegraphics[width=110mm]{HomogeneousCounterExamplePicture}};
      \begin{scope}[x={(plot.south east)},y={(plot.north west)}]
        \draw (0,0) -> (0,1) ;
        \draw (0,0) -> (1.03,-0.125) ;
        \node[below] at (0,0) {$0$}; 
        \node[below] at (1,0) {$1$};
        \node[below=0pt] at (1,-0.125) {$t$}; 
        \node[left=0pt] at (0,1) {$q$};
        \node at (0.7,0.42) {$\underline{\mathbb{E}}_\rateset^\mathrm{WHM}[f(X_t)\,\vert\,X_0=a]$};
        \node at (0.8,0.15) {$\underline{\mathbb{E}}_\rateset^\mathrm{WM}[f(X_t)\,\vert\,X_0=a]$};
        \node at (0.85,0.07) {$~~~~=\underline{\mathbb{E}}_\rateset^\mathrm{W}[f(X_t)\,\vert\,X_0=a]$};
      \end{scope}
    \end{tikzpicture}

...
Consider any $v\in\mathcal{U}$ and $s\in\reals_{\geq0}$ such that $v<s$ and $u<s$. Then for all $x_v\in\states^v$ and $y\in\states$, we have that
\begin{align*}
P(X_s&=y\vert X_v=x_v)
...
This equality allows us to prove that $P$ is well-behaved and consistent with $\mathcal{Q}$.

We first prove that $P$ is well-behaved. In order to do this, we consider any time sequence $0\leq t_0<\dots<t_{n}<t$ and any set of states $x_{0},\dots,x_{n},x,y\in\states$, and we distinguish between two cases: $t<\max u$ and $t\geq\max u$. If $t<\max u$, we see that $(X_{t+\Delta}=y,(X_t=x, X_{t_0}=x_{0}, \dots, X_{t_n}=x_{n}))\in\mathcal{C}_\emptyset$ for all $\Delta\in(0,\max u-\max v]$ and therefore, since $P$ is an extension of $\tilde{P}$, it follows from Equation~\eqref{eq:theo:aanelkaarplakken:defPtilde} that
\begin{multline*}
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t+\Delta}=y\vert X_t=x, X_{t_0}=x_{0}, \dots, X_{t_n}=x_{n})-\delta_{xy}}\\
=\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P_\emptyset(X_{t+\Delta}=y\vert X_t=x, X_{t_0}=x_{0}, \dots, X_{t_n}=x_{n})-\delta_{xy}}<+\infty,
\end{multline*}
where the final inequality is a consequence of the well-behavedness of $P_\emptyset$. If $t\geq \max u$, it follows from Equation
...
Consider now any $x_u\in\states^u$, and the processes $P_{X_z,x_u}$ and $P_{X_w,x_z,x_u}$ for all $x_z\in\states^z$. Because $\rateset$ is non-empty, bounded, and convex, Theorem~{\bf REF} implies the existence of a process $P_{X_w,X_z,x_u}\in\wprocesses_\rateset$, such that
\begin{align*}
...
\end{align*}
for all $x_z\in\states^z$ and all $x_w\in\states^w$.

Finally, consider an arbitrary $P_{X_u}\in\wprocesses_\rateset$, and the processes $P_{X_w,X_z,x_u}$ for all $x_u\in\states^u$. Theorem~{\bf REF} then implies the existence of some $P_{X_w,X_z,X_u}$ such that, for all $x_u\in\states$,
\begin{align*}

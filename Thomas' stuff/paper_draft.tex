...
This result is trivial if $s=t$. Hence, without loss of generality, we assume that $s>t$, which implies that $u=(t_0,\dots,t_n)$, with $n\in\nats$, $t_0=t$ and $t_n=s$, and that $u^*=(t^*_0,\dots,t^*_{n^*})$, with $n^*\in\nats$, $t^*_0=t$ and $t^*_{n^*}=s$.

Consider now any $u'\in\mathcal{U}_{[t,s]}$ that is finer than $u$ and $u^*$, meaning that the timepoints it consists of contain the timepoints in $u$ and the timepoints in $u^*$. For example, let $u'$ be the ordered union of the timepoints in $u$ and $u^*$.

Then, for all $k\in\{1,\dots,n\}$, there is some sequence $\Delta_{k,i}\geq 0$, $i\in\{1,\dots,n_k\}$, such that $\Delta_k=\sum_{i=1}^{n_k}\Delta_{k,i}=t_k-t_{k-1}$ and
\begin{equation*}
\Phi_{u'}\coloneqq\prod_{k=1}^n\left(\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)\right)
\text{~~and~~}
\Phi_{u}\coloneqq\prod_{k=1}^n(I+\Delta_{k}\lrate),
\end{equation*}
which, because of Lemma~\ref{lemma:differencebetweennested}, implies that $\norm{\Phi_{u'}-\Phi_u}\leq\delta C\norm{\lrate}^2$. 
...
Fix any $\epsilon>0$, let $C\coloneqq s-t$ and choose any $\delta>0$ such that $4\delta C\norm{\lrate}^2<\epsilon$ and $\delta\norm{\lrate}\leq1$ [this is clearly always possible].
% \begin{equation*}
...
\end{equation*}
where the strict inequality follows from Equation~\eqref{eq:theo:convergencelowerbound} and Proposition~\ref{prop:differencebetweenu}.

...
\end{equation*}
Because each of these component transition matrices corresponds to some exponential transition matrix system, this can equivalently be expressed as
\begin{equation*}
...
\end{equation*}
Because $Q_1$ and $Q_2$ were taken to commute, meaning that $Q_1Q_2=Q_2Q_1$, the matrix exponential is well-known~{\bf CITE} to satisfy $e^{Q_1\Delta_1'}e^{Q_2\Delta_2'}=e^{Q_2\Delta_2'}e^{Q_1\Delta_1'}=e^{Q_1\Delta_1' + Q_2\Delta_2'}$, for any $\Delta_1',\Delta_2'\in\realsnonneg$. Therefore, the above sequence can equivalently be written
\begin{align*}
...
\end{equation*}
By focusing only on the subsequence $\presuper{j}T_0^{\Delta_j},\presuper{j+2}T_0^{\Delta_j},\presuper{j+4}T_0^{\Delta_j}$ containing the even elements, it can be seen that, for all $k\in\natswith$,
\begin{equation*}
\presuper{j+2k}T_0^{\Delta_j} = e^{\Delta_j\left(Q_1(1-s_k) + Q_2s_k\right)}\,,
\end{equation*}
with $s_k\coloneqq \sum_{\ell=0}^k \nicefrac{1}{4^\ell} - 1$. Clearly, the limiting behavior of the sequence is therefore governed by the limiting behavior of the above geometric power series, and some arithmetic will show that
\begin{equation*}
...
\end{equation*}
where the first equality follows from the proof of Proposition~\ref{lemma:restricted_trans_mat_system_cauchy_converges}. Similarly, if $j$ was taken to be even instead of odd, then instead $\presuper{*}T_0^{\Delta_j}=e^{\Delta_j(Q_1(\nicefrac{1}{3}) + Q_2(\nicefrac{2}{3}))}$. Note that the mixing proportion between $Q_1$ and $Q_2$ is then reversed.
\exampleend
\end{exmp}
...
Suppose that, for some well-behaved stochastic process $P\in\wprocesses$, for some time $t$ and for some history $x_u$ such that $u<t$, there exist two different sequences $\{\Delta_i^1\}_{i\in\nats}$ and $\{\Delta_i^2\}_{i\in\nats}$ such that $\{\Delta_i^1\}_{i\in\nats}\to0^+$ and $\{\Delta_i^2\}_{i\in\nats}\to0^+$, for which
\begin{equation*}
\lim_{i\to\infty}~~\frac{1}{\Delta_i^1}\left(T_{t,x_u}^{t+\Delta_i^1} - I\right) = Q_1\,,~~\text{and,}~~ \lim_{i\to\infty}~~ \frac{1}{\Delta_i^2}\left(T_{t,x_u}^{t+\Delta_i^2} - I\right) = Q_2\,,
\end{equation*}
...
***** laat outer partial deriv zien voor systeem met verdichtingspunt uit sectie 3. merk op dat dit bestaan van process uit sectie 4.4 bewijst.
...
Let $\rateset$ be an arbitrary non-empty bounded set of rate matrices that has separately specified rows, with corresponding lower transition rate operator $\lrate$, and let $\underline{\mathcal{T}}_{\lrate}$ be the family of lower transition operators corresponding to $\lrate$. Then, for all $s\in\realspos$, all $u\in\mathcal{U}_{<s}$ such that $u\neq\emptyset$, all $x_u\in\states^u$, and all $f\in\gambles(\states^{u\cup\{s\}})$,
\begin{equation*}

...
\geq\frac{n}{2}-2.
\end{equation}
Since $\Delta\in(0,\delta_n]$ is arbitrary, this implies that
\begin{equation*}
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\norm{T_{0}^{\Delta}-I}\geq\frac{n}{2}-2.
\end{equation*}
Finally, since this is true for every $n\in\nats_0$, we infer from Definition~\ref{def:well_behaved_trans_mat_system} that $\mathcal{T}$ is not well-behaved, because the definition fails for $t=0$.
\exampleend
\end{exmp}

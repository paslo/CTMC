...
Then, for all $k\in\{1,\dots,n\}$, there is some sequence $\Delta_{k,i}\geq 0$, $i\in\{1,\dots,n_k\}$, such that $\Delta_k=\sum_{i=1}^{n_k}\Delta_{k,i}$ and
\begin{equation*}
...
Using a completely analogous argument, we find that $\norm{\Phi_{u'}-\Phi_{u^*}}\leq\delta C\norm{\lrate}^2$. Hence, it follows that
\begin{equation*}
\norm{\Phi_{u}-\Phi_{u^*}}
%=

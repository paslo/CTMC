...
\begin{proposition}
A family $\mathcal{T}^{[t,s]}$ is a restricted transition matrix system defined on $[t,s]$ if and only if **** satisfies decomposition, and identity matrix when no time elapses
\end{proposition}
\begin{proof}
*** {\bf TODO}
\end{proof}
...
****** Onderstaande weg, opnieuw doen in termen van compositie van r.t.m.s.
...
\subsection{*** Useful tools for construction of processes in set}
...
%\vert X_w=x_w)
\end{align}
Using this equality, we will now show that, for any $t\geq0$, $v\in\mathcal{U}_{<t}$ and $x_v\in\states^v$, there is some finite index set $I$, a set of non-negative coefficients $(\lambda_i)_{i\in S}$ that sum to one, and a set of stochastic processes $({}^iP\in\wprocesses_\rateset)_{i\in I}$ such that
...
{}^iT_{t,x_u}^{t+\Delta}
\end{equation}
and, similarly, that for any $t>0$, $v\in\mathcal{U}_{<t}$ and $x_v\in\states^v$, there is some finite index set $I$, a set of non-negative coefficients $(\lambda_i)_{i\in S}$ that sum to one, and a set of stochastic processes $({}^iP\in\wprocesses_\rateset)_{i\in I}$ such that
...
Need to add a proof for Equations~\eqref{eq:theo:aanelkaarplakken:convexTleft} and~\eqref{eq:theo:aanelkaarplakken:convexTleft} here.
...
=\sum_{i\in I}\lambda_i\lim_{\Delta\to 0^{+}}\frac{1}{\Delta}\norm{{}^iT_{t,x_u}^{t+\Delta}-I}
<+\infty.
\end{equation*}
...
We end by proving that $P$ is consistent with $\rateset$. Assume \emph{ex absurdo} that $P$ is not consistent with $\rateset$, or equivalently, that there are $t\geq0$, $v\in\mathcal{U}_{<t}$ and $x_v\in\states^v$ such that $\overline{\partial}
{T^t_{t,\,x_u}}\not\subseteq\rateset$. We will show that this leads to a contradiction.
...
As we are about to show, this equality allows us to prove that $P$ is well-behaved and consistent with $\mathcal{Q}$.

In order to prove that $P$ is well-behaved, we consider any (possibly empty) time sequence $v\in\mathcal{U}$, any $x_v\in\states^v$ and any $x,y\in\states$. 

First fix any $t\in\reals_{\geq0}$ such that $t>v$.
We distinguish between two cases: $t<\max u$ and $t\geq\max u$. If $t<\max u$, we see that $(X_{t+\Delta}=y,(X_t=x, X_v=x_v))\in\mathcal{C}_\emptyset$ for all $\Delta\in(0,\max u-\max v]$ and therefore, since $P$ is an extension of $\tilde{P}$, it follows from Equation~\eqref{eq:theo:aanelkaarplakken:defPtilde} that
\begin{multline*}
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)-\delta_{xy}}\\
=\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P_\emptyset(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)-\delta_{xy}}<+\infty,
\end{multline*}
where the final inequality is a consequence of the well-behavedness of $P_\emptyset$. If $t\geq \max u$, then for all $\Delta>0$, it follows from Equation~\eqref{eq:theo:aanelkaarplakken:convexcombo} (with $s\coloneqq t+\Delta$ and $w\coloneqq v\cup t$) that
\begin{align*}
&P(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)\\
&~~~~=
\sum_{x_{u\setminus(v\cup t)}\in\states^{u\setminus(v\cup t)}}
P_{x_u}(X_{t+\Delta}=y\vert X_t=x, 
X_{u\setminus t}=x_{u\setminus t}, X_{v\setminus [0,\max u]}=x_{v\setminus [0,\max u]})\\[-4mm]
...
\begin{align*}
&\abs{P(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)-\delta_{xy}}\\
&\leq
\sum_{x_{u\setminus(v\cup t)}\in\states^{u\setminus(v\cup t)}}
\abs{P_{x_u}(X_{t+\Delta}=y\vert
X_t=x, X_{(u\setminus t)\cup(v\setminus [0,\max u])}=x_{(u\setminus t)\cup(v\setminus [0,\max u])})-\delta_{xy}}\\[-4mm]
&\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad~~~~
P(X_{u\setminus(v\cup t)}=x_{u\setminus(v\cup t)}
\vert X_t=x, X_v=x_v).
\end{align*}
Since $P_{x_u}$ is well-behaved for all $x_u\in\states^u$, this implies that
\begin{equation*}
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)-\delta_{xy}}
<+\infty.
\end{equation*}
Since we have shown this to be true for both cases ($t<\max u$ and $t\geq\max u$), it follows that Equation~\eqref{eq:def:well-behaved:right} is satisfied. 

Next, fix any $t\in\reals_{>0}$ such that $t>v$.
We distinguish between two cases: $t\leq\max u$ and $t>\max u$. If $t\leq\max u$, we see that $(X_{t}=y,(X_{t-\Delta}=x, X_v=x_v))\in\mathcal{C}_\emptyset$ for all $\Delta\in(0,t-\max v)$ and therefore, since $P$ is an extension of $\tilde{P}$, it follows from Equation~\eqref{eq:theo:aanelkaarplakken:defPtilde} that
\begin{multline*}
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t}=y\vert X_{t-\Delta}=x, X_v=x_v)-\delta_{xy}}\\
=\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P_\emptyset(X_{t}=y\vert X_{t-\Delta}=x, X_v=x_v)-\delta_{xy}}<+\infty,
\end{multline*}
where the final inequality is a consequence of the well-behavedness of $P_\emptyset$. If $t>\max u$, then for all $\Delta\in(0,\min\{t-\max v,t-\max u\})$, using a similar argument as before, it follows from Equation~\eqref{eq:theo:aanelkaarplakken:convexcombo} (now with $s\coloneqq t$ and $w\coloneqq v\cup\{t-\Delta\}$) that
\begin{align*}
&\abs{P(X_{t}=y\vert X_{t-\Delta}=x, X_v=x_v)-\delta_{xy}}\\
&\quad\quad\leq
\sum_{x_{u\setminus v}\in\states^{u\setminus v }}
\abs{P_{x_u}(X_{t}=y\vert
X_{t-\Delta}=x, X_{u\cup(v\setminus [0,\max u])}=x_{u\cup(v\setminus [0,\max u])})-\delta_{xy}}\\[-4mm]
...
\vert X_t=x, X_v=x_v),
\end{align*}
which, since $P_{x_u}$ is well-behaved for all $x_u\in\states^u$, implies that
\begin{equation*}
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)-\delta_{xy}}
<+\infty.
\end{equation*}
Since we have shown this to be true for both cases ($t\leq\max u$ and $t>\max u$), it follows that Equation~\eqref{eq:def:well-behaved:left} is satisfied.

Since we have shown that, for any (possibly empty) time sequence $v\in\mathcal{U}$, any $x_v\in\states^v$ and any $x,y\in\states$, Equations~\eqref{eq:def:well-behaved:right} and~\eqref{eq:def:well-behaved:left} are satisfied, it follows from Definition~\ref{def:well-behaved} that $P$ is well-behaved.



*** still need to prove consistency ***
...
**** doing some setup for new approach

\begin{theorem}\label{theorem:decomposition_multivar}
Let $\rateset$ be an arbitrary non-empty, bounded, and convex set of rate matrices, with the corresponding set $\wprocesses_\rateset$ of well-behaved stochastic processes consistent with $\rateset$. Then, for any $t\in\realsnonneg$, any $u\in\mathcal{U}_{<t}$ and $v\in\mathcal{U}_{\geq t}$, any $f\in\gambles(\states^{u\cup v})$, and any $w\subseteq v$,

...
To the best of our knowledge, the concept of an imprecise continuous-time Markov chain was first introduced in the literature by the work of {\v{S}}kulj~\cite{Skulj:2015cq}. There, $\lrate$ was defined as the lower envelope of a set of rate matrices $\rateset$ that has separately specified rows, and the notion of an imprecise continuous-time Markov chain was then introduced in terms of expectations of functions $f\in\gamblesX$. 

In particular, for any $f\in\gamblesX$, {\v{S}}kulj starts by considering a time-dependent function $f_t$, which---using our notation---is equal to $f_t\coloneqq T_0^t f$, and then requires that $f_0=f$,
\begin{equation*}
\lrate f_t
\leq
D^+f_t\coloneqq
\liminf_{\Delta\to0^+}
\frac{f_{t+\Delta}-f_t}{\Delta}
~~~\text{and}~~~
\limsup_{\Delta\to0^+}
\frac{f_{t+\Delta}-f_t}{\Delta}
=\hspace{-1.5pt}\colon D_+f_t
\leq\overline{Q} f_t,
\end{equation*}
where $\overline{Q}$ is the conjugate upper transition rate operator of $\lrate$, defined by $\overline{Q}g\coloneqq-\lrate(-g)$ for all $g\in\gamblesX$, and where $D^+f_t$ and $D_-f_t$ are called Dini derivatives. In this way, he mimics the usual differential equation characterisation of a precise homogeneous Markov chain, where the the Dini derivatives would be replaced by derivatives, and where the $\lrate$ would be replaced by a transition rate matrix $Q$.

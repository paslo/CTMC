...
Consider a stochastic process $P\in\processes$ of which the transition matrices $T_t^s$ are given by Equation~\eqref{eq:ex4:def}. For now, we simply assume that this is possible. A formal proof for the existence of such a process requires some additional machinery, and we therefore postpone it to Examples~\ref{exmp:markov_with_no_deriv} and~\ref{exmp:running:wellbehaved}, where we will construct a well-behaved continuous-time Markov chain $P$ for which this is indeed the case.

For all $k\in\nats_0$, let $\Delta_k\coloneqq2^{-k}$ and $Q_k$ be defined as in Example~\ref{exmp:limit_trans_mat_system_matrices}. It then follows from Equation~\eqref{eq:ex4:finalstep} that
\begin{equation*}
\lim_{k\to\infty}
T_0^{\Delta_{2k}}
=

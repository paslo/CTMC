...
{T^t_{t,\,x_u}}}$ then follows trivially because a union of two non-empty, closed and bounded sets is always non-empty, closed and bounded itself.

In order to prove that $\smash{\overline{\partial}_{+}
{T^t_{t,\,x_u}}}$ is non-empty, we consider an arbitrary sequence $\Delta_k\to0^+$, $k\in\nats$. The corresponding sequence of matrices $Q_k$, $k\in\nats$, defined by
\begin{equation*}
Q_k\coloneqq\frac{1}{\Delta_i}
(T^{t+\Delta_i}_{t,\,x_u}-I)
\text{~~for all $k\in\nats$},
\end{equation*}
is then bounded because of Definition~\ref{def:well-behaved} and therefore, it follows from the Bolzano-Weierstrass theorem that it has a convergent subsequence $Q_{k_i}$, $i\in\nats$, of which we denote the limit by $Q^*$. Hence, we have found a sequence $\Delta_{k_i}\to0^+$, $i\in\nats$, such that $Q_{k_i}\to Q^*$.
...
Next, we prove that $\smash{\overline{\partial}_{+}
{T^t_{t,\,x_u}}}$ is closed. 

*** I have this written down somewhere ***



*** The boundedness of $\smash{\overline{\partial}_{+}
{T^t_{t,\,x_u}}}$ follows ***
\end{proof}
...
~
\norm{
T^{t_{i+1}}_{t_i,\,x_u}-(I+\Delta_{i+1}Q_i)
}<\Delta_{i+1}\epsilon
\end{equation*}
\end{lemma}
\begin{proof}
*** I have this written down somewhere (although roughly) ***
\end{proof}
...
\noindent Furthermore, this set is a complete metric space under the norm $\norm{\cdot}$.
...
Let now $\gambles_{\leq1}(\states)\coloneqq\left\{f\in\gamblesX\,:\,\forall x\in\states, 0\leq f(x)\leq 1\right\}$ be the set of non-negative functions $f\in\gamblesX$ with $\norm{f}\leq 1$. It was previously shown {\bf REF COMPLETE FACTOR} that the metric space $(\underline{\mathcal{T}}_x,d_x)$ is complete under the metric $d_x$, defined for all $\lt_x,\underline{S}_x\in\underline{\mathcal{T}}_x$ by
\begin{equation*}
...
\end{equation*}
from which it follows {\bf REF} that $d_x$ and $d_x^*$ induce the same topology on $\underline{\mathcal{T}}_x$. Hence, because $(\underline{\mathcal{T}}_x, d_x)$ is complete, so is $(\underline{\mathcal{T}}_x,d_x^*)$.

Now, because  {\bf REF COMPLETE PRODUCT} $\underline{\mathcal{T}}$ is a finite product of $\underline{\mathcal{T}}_1,\ldots,\underline{\mathcal{T}}_m$, the metric space $(\underline{\mathcal{T}},D)$ is complete under any metric $D$ that is \emph{conserving} {\bf REF CONSERVING} with respect to $d_1^*,\ldots,d_m^*$. For $D$ to be conserving, it suffices to show that for all $\lt,\underline{S}\in\underline{\mathcal{T}}$,
\begin{equation*}
D(\lt, \underline{S}) = \max_{x\in\states}\bigl\{d_x^*(\lt_x, \underline{S}_x)\bigr\}\,.
\end{equation*}
Now, consider the metric $d$ on $\underline{\mathcal{T}}$ that is induced by $\norm{\cdot}$. We have, for any $\lt,\underline{S}\in\underline{\mathcal{T}}$, 
\begin{align*}
d(\lt,\underline{S}) &= \norm{\lt - \underline{S}} \\
...
*** Metric Spaces, Michael O'Searcoid, Theorem 10.5.1 (complete under preserving), Definition 1.6.2 (preserving w.r.t factor metrics) ***
...
We will next introduce a specific lower transition operator on which we will focus for the remainder of this work. We will assume in this section that we are given some arbitrary lower transition rate operator $\lrate$, and then define the \emph{lower transition operator corresponding to $\lrate$}.

\begin{definition}[Corresponding Lower Transition Operator]\label{def:low_trans}
Consider any $t,s\in\realsnonneg$ such that $t<s$ and let $\lrate$ be an arbitrary lower transition rate operator. The \emph{corresponding lower transition operator} $\lbound_t^s$ is then a map from $\gamblesX$ to $\gamblesX$, defined by
\begin{equation}\label{eq:lowerbound}
\lbound_t^s\coloneqq\lim_{\sigma(u)\to0}\left\{ \prod_{k=1}^n(I+\Delta_k\lrate)\,\Big\vert\,u\in\mathcal{U}_{[t,s]}\right\},
\end{equation}
...
\end{theorem}
\quad\newline\newline
\noindent The remainder of this section serves to prove Theorem~\ref{theo:convergencelowerbound}. The proof basically works as follows. In Lemmas~\ref{lemma:justtheindicator}-\ref{lemma:differencebetweennested}, we first establish bounds on the norm of the difference between various lower transition operators. For any $u\in\mathcal{U}_{[t,s]}$, we then define the operator
\begin{equation*}
\Phi_u\coloneqq\prod_{k=1}^n(I+\Delta_k\lrate)\,,
\end{equation*}
so that for any sequence $u_1,u_2,\ldots,u_n,\ldots$, we can obtain a corresponding sequence $\Phi_{u_1},\Phi_{u_2},\ldots,\Phi_{u_n},\ldots$ of operators. Using the previously established bounds, Corollary~\ref{corol:limitexistsandiscoherent} shows that if we choose $u_1,u_2,\ldots,u_n,\ldots$ such that $\lim_{n\rightarrow\infty}\sigma(u_n)=0$, the corresponding sequence $\Phi_{u_1},\Phi_{u_2},\ldots,\Phi_{u_n},\ldots$ converges, and that its limit point is a lower transition operator. The proof of Theorem~\ref{theo:convergencelowerbound} finally establishes that this limit is furthermore unique, in that the limit point is independent of the choice of $u_1,u_2,\ldots,u_n,\ldots$. Thus, we show that the limit in Equation~\eqref{eq:lowerbound} exists, and that $L_t^s$ is a lower transition operator. 

We begin with some bounds on norms. If they are not given here, the proofs of these results can be found in Appendix~\ref{sec:proof_appendix}.
...
\noindent We now have all the pieces required to prove Theorem~\ref{theo:convergencelowerbound}.
...
\begin{proof}[Proof of Lemma~\ref{lemma:justtheindicator}]
\begin{align*}
...
\begin{proof}[Proof of Lemma~\ref{lemma:justthelinearpart}]
\begin{align*}
...
\begin{proof}[Proof of Lemma~\ref{lemma:differencebetweennested}]
\begin{align*}
&\norm{\prod_{k=1}^n\left(\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)\right)
-
\prod_{k=1}^n(I+\Delta_k\lrate)

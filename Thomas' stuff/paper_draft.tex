...
We next introduce a function that is often seen in the context of continuous-time Markov chains: the \emph{matrix exponential}~\cite{van2006study} $e^{Qt}$ of $Q t$, with $Q$ a rate matrix and $t\in\reals_{\geq0}$. %For any matrix $A$, its matrix exponential is denoted $e^A$. 
...
Consider a rate matrix $Q\in\mathcal{R}$ and any $t\in\realsnonneg$. Then $e^{Qt}$ is a transition matrix.
\end{proposition}

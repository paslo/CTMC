...
\end{proposition}
Note that this result includes as a special case that the set $\mathbb{T}^{\realsnonneg}$ of all (unrestricted) transition matrix systems is complete. The following example illustrates this result for $\mathbf{I}=[0,1]$.
...
We can then construct the following sequence. For $i=0$, we let $\mathcal{T}_{0}^{[0,1]}\coloneqq\mathcal{T}_{Q_2}^{[0,1]}$, and for all $i\in\nats$, we let
\begin{equation*}

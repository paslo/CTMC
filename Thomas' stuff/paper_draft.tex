...
\end{equation*}
One of the main results of this paper is that we can use the operator $L_t^s$ to compute lower expectations of this last form, taken with respect to sets $\processes_\rateset$ of non-Markov chains.

Finally, in Section~\ref{sec:tractability}, we will show that although $L_t^s$ provides us with a way to easily compute such lower expectations, doing so for general functions $f\in\gambles(\states^{u\cup v})$ is still computationally intractable. However, we then provide algorithms to tractably compute lower expectations for rather broad, and practically applicable, subclasses of $\gambles(\states^{u\cup v})$.

\subsection{*** functies met een tijdstip in de toekomst en de rest in het verleden ***}\label{sec:function_single_future_multiple_past}

notatie aanhalen voor restrictie zoals besproken in sectie~\ref{sec:multivar_notation}.

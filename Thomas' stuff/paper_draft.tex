...
*** notation for naturals, reals, ... notation and conventions for matrices ***
...
*** add nice introtext here. apologize for how much machinery we need :P ***
...
*** laat zien dat we met die transitie matrices verwachtingswaarden kunnen uitrekenen:

\begin{proposition}
*** nog even goed verwoorden

Note that for all $x_t\in\states$, we have that
\begin{align*}
...
*** ook doen voor $T_{t,x_u}^s$
\end{proposition}
\begin{proof}
*** trivial ***
\end{proof}

*** hoe kunnen we eigenschappen van die transitie matrices beschrijven? met afgeleides:

\begin{definition}[Directional Partial Derivatives]
*** tralala, we kunnen eigenschappen van transitie matrices van een gegeven process uitdrukken aan de hand van (directionele en partieele) afgeleides:
\begin{equation*}
\partial_{+}{T_{t,\,x_u}^t}
\coloneqq
\lim_{\Delta\to 0^{+}}
...
\end{definition}
\vspace{5pt}
\noindent If these partial derivatives exist, then because of Proposition~\ref{prop:rate_from_stochastic_matrix}, they are guaranteed to belong to $\mathcal{R}$. If they both exists and coincide, we write $\partial{T_{t,\,x_u}^t}$ to denote their common value.
...
\end{equation*}
Before defining this operator of interest, we first give some preliminary results. We start with a bound on the distance between two operators $\Phi_u$ and $\Phi_{u*}$.

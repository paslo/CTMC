...
*** is het mogelijk om zoiets te laten zien? ***
...
\end{equation*}
and furthermore, there does not exist a $\processes^+\subseteq\processes$ with $\processes^+\setminus\wprocesses_{\rateset_{\lrate}}\neq\emptyset$ such that for all $t,s\in\realsnonneg$ such that $t<s$, all $f\in\gamblesX$, and all $x\in\states$,
\begin{equation*}
\left[L_t^sf\right](x) = \inf\left\{\mathbb{E}[f(X_s)\,\vert\,X_t=x]\,:\,P\in\processes^+\right\}\,.
\end{equation*}
\end{theorem}
\begin{proof}
*** Geen idee, is dit zo? even kijken... ***

First statement is trivial.

For the second statement, assume that it is not true. Then, there must be some $\processes^+\subseteq\processes$ such that for some $P\in\processes^+$, $P\notin\wprocesses_{\rateset_{\lrate}}$, and at least, for all $t,s\in\realsnonneg$ such that $t<s$, all $f\in\gamblesX$ and all $x\in\states$,
\begin{equation*}
\left[L_t^sf\right](x) \leq \mathbb{E}[f(X_s)\,\vert\,X_t=x]\,.
\end{equation*}
Because $P\notin\wprocesses_{\rateset_{\lrate}}$, we know that $\smash{\overline{\partial}}T_{t,x_u}^t\nsubseteq\rateset_{\lrate}$.

*** focus on $\smash{\overline{\partial}}_+T_{t,x_u}^t$ for now ***

This implies that either $\smash{\overline{\partial}}_+T_{t,x_u}^t=\emptyset$, or, there is some $Q\in\smash{\overline{\partial}}_+T_{t,x_u}^t$ such that $Q\notin\rateset_{\lrate}$, meaning that there exists a sequence $\{\Delta_i\}_{i\in\nats}\to0$ such that
\begin{equation*}
Q\coloneqq \lim_{i\rightarrow+\infty}\frac{1}{\Delta_i}(T_{t,x_u}^{t+\Delta_i}-I) \notin \rateset_{\lrate}\,.
\end{equation*}
*** in case we need it: also no convergent subsequence with limit in that set ***
Hence, for all $\epsilon>0$ and all $\delta>0$, there must be some $0<\Delta<\delta$, such that
\begin{equation*}
\norm{T_{t,x_u}^{t+\Delta} - (I+\Delta Q)} < \epsilon\,.
\end{equation*}
Hence also
\begin{equation*}
\norm{T_{t,x_u}^{t+\Delta}f - (I+\Delta Q)f} < \epsilon\norm{f}\,.
\end{equation*}
Lala
\begin{equation*}
\norm{T_{t,x_u}^{t+\Delta}f - L_t^{t+\Delta}f} + \norm{L_t^{t+\Delta}f - (I+\Delta Q)f} < \epsilon\norm{f}\,.
\end{equation*}
pomppom
\begin{align*}
\norm{T_{t,x_u}^{t+\Delta}f - L_t^{t+\Delta}f} + \norm{(I+\Delta\lrate)f - (I+\Delta Q)f} + \norm{\Phi_u - (I+\Delta\lrate)} &< \epsilon\norm{f} \\
\norm{T_{t,x_u}^{t+\Delta}f - L_t^{t+\Delta}f} + \Delta\norm{\lrate f -  Qf} + \norm{\Phi_u - (I+\Delta\lrate)} &< \epsilon\norm{f} \\
\end{align*}
\end{proof}

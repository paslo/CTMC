...
In particular, for each event $(A,C)\in\mathcal{C}$ a bettor will specify a price $P(A\vert C)$ for which she is always willing to both buy and sell the gamble that event $A$ will come true, given that event $C$ comes true. Such a gamble yields a reward of one currency unit to its holder---paid by the seller---if the event $A$ occurs. These gambles are then bought and sold under the proviso that, should event $C$ not happen, then all parties involved are refunded. 

Suppose now that the outcome is $\omega$. For each ticket the bettor bought, she will receive one unit of currency if event $A$ came true. Her profit is then $(\ind{A}(\omega) - P(A\vert C))$, per ticket, with negative profit being loss. However, she only receives this profit if event $C$ also came to pass. Hence, her actual profit is $\ind{C}(\omega)(\ind{A}(\omega) - P(A\vert C))$. Note that if $\ind{C}(\omega)=0$, she neither gains nor loses anything. Allowing for fractional tickets, if she has bought a total of $\lambda\in\realsnonneg$ such tickets, her total profit is $\lambda \ind{C}(\omega)(\ind{A}(\omega) - P(A\vert C))$.

Similarly, for each such ticket that she sold, she has received $P(A\vert C)$ currency units in advance, and must now pay $\ind{A}(\omega)$ to its buyer. Because this is conditional on event $C$ coming true, her remaining profit after paying is $\ind{C}(\omega)(P(A\vert C) - \ind{A}(\omega))$, per ticket sold. Comparing to the above, it follows that for any given outcome $\omega$, her resulting profit from buying is equal to her loss from selling, and vice versa.

Now, note that the bettor quoted prices for which she is \emph{always} willing to buy or sell these gambles. Therefore, the bettor can be forced---by an adversarial gambler---to buy and/or sell any number of gambles in any combination. That is, for any $n\in\nats$ and any $i\in\{1,\ldots,n\}$, she can be forced to buy $\lambda_i\in\reals$ gambles on any event $(A_i,C_i)\in\mathcal{C}$ (which is equivalent to selling these gambles, if $\lambda_i$ is negative). After being forced into this combined gamble, her total loss in case of the outcome $\omega$ is then
\begin{equation*}
...
The prices $P$ are now said to be \emph{coherent} if they \emph{avoid sure loss}. That is, if regardless of the combination of gambles, there exists an outcome $\omega$ for which at least one of the events $C_i$ comes true, and for which she does not make a loss. Note that if none of the events $C_i$ come true, i.e. if $\omega\notin \cup_{i=1}^n C_i$, then she would trivially not make a loss because she would get refunded on all the gambles. However, to be coherent, the prices also have to avoid sure loss if (some) of the conditioning events do come true. 

...
\end{equation*}
This first equality follows from the Markov property of the elements in $\wmprocesses$, ensuring that $\smash{\overline{\partial}}T_{t,x_u}^t=\smash{\overline{\partial}}T_t^t$ for all $u\in\mathcal{U}_{<t}$ and all $x_u\in\states^u$. The second equality follows from the Markov property and the homogeneity of the elements of $P\in\whmprocesses$, which, by Proposition~\ref{prop:Q_is_singleton_deriv_for_homogen}, ensures that $\smash{\overline{\partial}}T_t^t=\{Q_P\}$ for all $t\in\realsnonneg$.
...
Let $\lrate_1$ and $\lrate_2$ be the lower transition rate operator that corresponds to $\rateset_1$ and $\rateset_2$, respectively, %We will show that $\lrate_1=\lrate_2$.
...
=[\lrate f](x),
\end{equation*}
where the inequality holds because $\lrate$ is the lower envelope of $\rateset_1$. Since this is true for all $x\in\states$, we find that $Qf\geq\lrate f$. Since this is true for all $f\in\gamblesX$, we find that $Q\geq\lrate$. Since this is true for all $Q\in\rateset^*$, it follows that $\rateset^*\subseteq\rateset_{\lrate}$. Hence, if we let $\lrate^*$ be the lower transition rate operator that corresponds to $\rateset^*$, we find that $\lrate^*\geq\lrate$. Similarly, since $\rateset_1$ is clearly a subset of $\rateset^*$, we find that $\lrate^*\leq\lrate_1$. Since $\lrate_1=\lrate$, we conclude that $\lrate^*=\lrate$.

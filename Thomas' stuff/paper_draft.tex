...
For any $k\in\{1,\dots,n\}$, consider a sequence $\Delta_{k,i}>0$, $i\in\{1,\dots,n_k\}$, and let $\Delta_k\coloneqq\sum_{i=1}^{n_k}\Delta_{k,i}$. Let $\Delta\coloneqq\sum_{k=1}^n\Delta_k$ and let $\alpha\coloneqq\max_{k=1}^n\Delta_k$. Then for any lower transition rate operator $\lrate$ such that $\alpha\leq\nicefrac{1}{\norm{\lrate}}$:
\begin{equation*}

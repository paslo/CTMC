...
\begin{equation*}
\mathbb{C}_\epsilon\coloneqq \left\{\mathcal{T}_i\in\mathcal{T}_\rateset^{[t,s]}\,:\,i\in\{1,\ldots,N\}\right\}\,,
\end{equation*}
with $N\in\nats$, such that
\begin{equation*}
(\forall \mathcal{T}\in\mathcal{T}_\rateset^{[t,s]})(\exists \mathcal{S}\in \mathbb{C}_\epsilon)\,:\,d(\mathcal{T},\mathcal{S}) < \epsilon\,.
\end{equation*}
...
Now, because $\rateset$ is non-empty, bounded, and convex, due to Lemma~{\bf REF}, there is some $\delta\in\realspos$ such that, for all $P\in\wmprocesses_\rateset$, all $t\in\realsnonneg$, and all $\Delta\in\realspos$ with $\Delta<\delta$,
...
\begin{equation*}
\mathcal{T}_1^{[t_{0},t_1]} \otimes \mathcal{T}_2^{[t_{1},t_2]} \cdots \otimes \mathcal{T}_n^{[t_{n-1},t_n]} \in \mathcal{T}_\rateset^{[t,s]}\,.
\end{equation*}
\end{claim}
...
\end{equation*}
then $\mathbb{C}_\epsilon$ contains a finite number of elements, and, due to Claim~\ref{claim:composition_restricted_trans_mat_sys_in_set}, $\mathbb{C}_\epsilon\subset \mathcal{T}_\rateset^{[t,s]}$.

It remains to show that $\mathbb{C}_\epsilon$ generates an open $\epsilon$-cover of $\mathcal{T}_\rateset^{[t,s]}$. To this end, consider any $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$. We will show that there is some $\mathcal{S}\in \mathbb{C}_{\epsilon}$ such that $d(\mathcal{T},\mathcal{S})<\epsilon$. We start by constructing this $\mathcal{S}$.
...
\begin{equation*}
\mathcal{S}\coloneqq \mathcal{T}_{\widetilde{Q_1}}^{[t_{0},t_1]} \otimes \cdots \otimes \mathcal{T}_{\widetilde{Q_n}}^{[t_{n-1},t_n]} \in \mathbb{C}_\epsilon\,.
\end{equation*}
...
\end{equation*}
with $T_q^r\in\mathcal{T}$ and $S_q^r\in\mathcal{S}$. Because this holds for all $q$ and $r$, it follows that
\begin{equation*}
d(\mathcal{T},\mathcal{S}) = \sup\{\norm{T_q^r - S_q^r}\,:\,q,r\in[t,s], q\leq r\} \leq \epsilon^* < \epsilon\,.
\end{equation*}
Because this holds for any $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$, it follows that $\mathbb{C}_\epsilon$ is a finite, open $\epsilon$-cover of $\mathcal{T}_\rateset^{[t,s]}$. Because the $\epsilon\in\realspos$ was arbitrary, we conclude that $\mathcal{T}_\rateset^{[t,s]}$ is totally bounded.
\end{proof}

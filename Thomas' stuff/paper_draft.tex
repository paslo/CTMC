...
\begin{equation*}
\underline{P}(X_1 = X_2\,\vert\, X_0) = \underline{\mathbb{E}}[f(X_1,X_2)\,\vert\,X_0],\vspace{4pt}
\end{equation*}
with $f$ the indicator of the event $X_1=X_2$, as defined by
\begin{equation*}
f(x_1,x_2) \coloneqq
\begin{cases}
1 & \text{if $x_1 = x_2$} \\
...
\end{equation*}
The value of this lower expectation can---as in Example~\ref{exmp:single_time_numerical}---depend on the particular type of {\ictmc} that we choose to use. In this example, we will consider the \ictmc~$\smash{\mathbb{P}_{\rateset,\mathcal{M}}^{\mathrm{W}}}$, with $\mathcal{M}$ some arbitrary set of probability mass functions on $\states$.

...
%Recall from Section~\ref{sec:trans_mat_systems} that for all $P\in\wmprocesses$, there is a well-behaved transition matrix system $\mathcal{T}_P$ corresponding to this $P$. For any $t,s\in\realsnonneg$ such that $t\leq s$, let $\mathcal{T}_P^{[t,s]}$ be the restriction of $\mathcal{T}_P$ to the interval $[t,s]$. That is, $\mathcal{T}_P^{[t,s]}$ contains all transition matrices $T_q^r\in\mathcal{T}_P$ such that $q,r\in[t,s]$ with $q\leq r$.
...
\end{equation*}
Because for all $i\in\{1,\ldots,n\}$ we have that $\mathcal{T}_i^{[t_{i-1},t_i]}$ is a well-behaved restricted transition matrix system, it follows from Proposition~{\bf REF} that $\mathcal{T}^{[t,s]}$ is a well-behaved restricted transition matrix system defined on $[t,s]$.

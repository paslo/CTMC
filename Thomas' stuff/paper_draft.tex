...
We will now formalize the notion of continuous-time stochastic processes. Next, in Section~\ref{sec:well_behaved}, we describe a specific subclass of stochastic processes, which we call well-behaved, and on which we will largely focus throughout this work. Finally, Section~\ref{sec:dynamics} provides some tools with which we can describe the dynamics of stochastic processes.

We will restrict our definition of continuous-time stochastic processes to \emph{finitely additive} stochastic processes, meaning that the events on which they are defined contain at most a finite---but arbitrary---number of time points. This simplifies considerably the subsequent analysis, and we will see shortly that, when required, the extension to $\sigma$-additive processes can always be made. 

These finitely additive stochastic processes will be defined on a specific subset of the set $\power\times\nonemptypower$ of all (conditional) events, as follows.
...
Because $\rateset$ has separately specified rows, there must therefore be some $x\in\states$ such that $Q(x,\cdot)\notin\rateset_x$, where $\rateset_x\coloneqq\{Q'(x,\cdot)\,:\,Q'\in\rateset\}$. Because $\rateset$ is non-empty, bounded, closed and convex, it follows that $\rateset_x$ is also non-empty, bounded, closed, and convex. 

Note also that we can interpret $\rateset_x\subset\reals^m$ as a subset of the vector space $\reals^m$, where $m$ is the size of $\states$. Let $\overline{\rateset}_x$ denote the complement of $\rateset_x$ in $\reals^m$. Because $\rateset_x$ is closed, it follows that $\overline{\rateset}_x$ is open. Because $Q(x,\cdot)\notin\rateset_x$, and because $Q(x,\cdot)\in\reals^m$, it follows that $Q(x,\cdot)\in\overline{\rateset}_x$. Because $\overline{\rateset}_x$ is open, there must therefore be some $\epsilon\in\realspos$ such that the open ball $\mathcal{B}(Q(x,\cdot),\epsilon)$ satisfies $\mathcal{B}(Q(x,\cdot),\epsilon)\subset\overline{\rateset}_x$. We therefore have that $\mathcal{B}(Q(x,\cdot),\epsilon)\cap\rateset_x=\emptyset$. In summary, $\mathcal{B}(Q(x,\cdot),\epsilon)$ and $\rateset_x$ are non-empty, convex, and disjoint subsets of $\reals^m$, and furthermore, $\rateset_x$ is closed, and $\mathcal{B}(Q(x,\cdot),\epsilon)$ is open.

Therefore, by the hyperplane separation theorem\footnote{That is, by one of them.} ({\bf REF}), there must be some $f\in\gamblesX$ and some $c\in\reals$, such that, because $Q(x,\cdot)\in \mathcal{B}(Q(x,\cdot),\epsilon)$, it holds that $Q(x,\cdot)f = [Qf](x) < c$, and, for all $Q'(x,\cdot)\in\rateset_x$, that $Q'(x,\cdot)f = [Q'f](x) \geq c$.

This clearly implies that, for all $Q'\in\rateset$, it holds that $[Qf](x) < [Q'f](x)$. In turn, this implies the existence of some $\epsilon'\in\realspos$ for which $[Qf](x) + \epsilon' < [Q'f](x)$, for all $Q'\in\rateset$. Because $\lrate$ is the lower envelope of $\rateset$, by Equation~\eqref{eq:correspondinglowertrans}, we therefore find that $[Qf](x) < [\lrate f](x)$. Hence, we have found a $f\in\gamblesX$ and a $x\in\states$, such that for some $Q\in\rateset_{\lrate}$, it holds that $[Qf](x) < [\lrate f](x)$. By Equation~\eqref{eq:dominatingratematrices}, this is a contradiction, and hence $\rateset=\rateset_{\lrate}$.
\end{proof}

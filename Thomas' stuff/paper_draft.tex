...
\begin{abstract}
Continuous-time Markov chains are mathematical models used to describe the evolution of discrete-state dynamical systems under stochastic uncertainty, and have found widespread applications in many fields. Nevertheless, they rely on a number of assumptions that need not be realistic for the domain of application---like the ability to provide exact numerical parameter assessments, or the eponymous Markov property---to make working with these models computationally tractable. In this work, we generalize this model class to \emph{imprecise continuous-time Markov chains} (\ictmc's), which robustifies such models with respect to both uncertain numerical parameter assessments and possibly incorrect assumptions of time-homogeneity, Markovian probabilistic independence, and differentiability of the state-transition probabilities.

An \ictmc~is a \emph{set} of ``precise'' continuous-time discrete-state stochastic processes, and rather than computing expected values of functions, we seek to compute \emph{lower expectations}---lower envelopes over the expectations induced by such a set of ``precise'' models. Note that, in contrast to e.g. Bayesian methods, all the elements of such a set are treated on equal grounds; we do not consider a distribution over this set. Together with the conjugate notion of \emph{upper expectation}, such assessments can then intuitively be interpreted as providing best- and worst-case scenarios with respect to all models in such a set. 

We provide in this work a formalism to describe continuous-time discrete-state stochastic processes, that does not require us to make any of the above-mentioned simplifying assumptions. This formalism is then used to exactly characterize set-valued \ictmc's, and we investigate their properties. The concept of lower expectation is given an alternative operator-theoretic characterization, by means of a \emph{lower transition operator}, and its properties are investigated. We use this lower transition operator to derive tractable (polynomial runtime-complexity w.r.t. the maximum numerical error) algorithms to compute lower expectations for functions defined on the state space at any finite number of time points.
\end{abstract}
...
We call each of these three sets 
 an \emph{imprecise continuous-time Markov chain}, and abbreviate this as \ictmc.\footnote{For the set $\wprocesses_{\rateset,\mathcal{M}}$, one might wonder why we choose to call it a \emph{Markov} chain, since, at first sight, it does not satisfy any Markov property. The reason for this choice will become clear in Section~\ref{subsec:ictmc_types}.} 
...
Lower and upper probabilities also have analogous definitions. However, these too do not need to be studied separately, because they correspond to special cases of lower and upper expectations. For example, for any $u\in\mathcal{U}$, $x_u\in\states^u$ and $A\in\mathcal{A}_u$, we have that
\begin{align*}
...
*** ADD REFERENCES FOR FIRST APPROACH! ***
...
\begin{exmp}
**** Ok, got it down numerically. Consider a ternary-state model, with $\states\coloneqq \{1,2,3\}$, take
\begin{equation*}
...
\end{equation*}
and let $f\in\gamblesX$ be defined as $f\coloneqq [0~1~2]^\top$.
...
\begin{figure}[H]\label{fig:homogeneousCounterExample}
\includegraphics[width=\textwidth]{HomogeneousCounterExamplePicture}
\caption{Plot of the induced set of expected values $\mathbb{E}_P[f(X_t)\,\vert\,X_0=2]$, corresponding to all $P\in\whmprocesses_{\rateset}$ obtained as we vary $\lambda\in[0.01,0.5]$. The horizontal axis denotes the time-points $[0,100]$. Observe that the lower expectation with respect to this set is first given by choosing $\lambda=0.5$. This changes around the time-point $t\approx 6.6$, after which the minimizing value of $\lambda$ becomes a (changing) internal point of the interval $[0.01,0.5]$. The orange line corresponds to the lower expectation with respect to the set $\wmprocesses_\rateset$, which also includes non-homogeneous Markov chains. It is clear that the lower expectations $\underline{\mathbb{E}}_\rateset^\mathrm{WHM}[f(X_t)\,\vert\,X_0=2]$ and $\underline{\mathbb{E}}_\rateset^\mathrm{WM}[f(X_t)\,\vert\,X_0=2]$ are equal when $t<6.6$, but then diverge as time evolves further.}
\end{figure}
\exampleend
\end{exmp}
...
Having introduced lower expectations that corresponds to the set of processes that may not be homogeneous and that may not even satisfy the Markov property, one might wonder how to compute this, either numerically or for analytical purposes. Indeed, one of the aims of this paper is to provide methods with which such lower expectations can be tractably computed.

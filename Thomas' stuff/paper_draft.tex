...
In Sections~\ref{sec:stochastic_processes} and~\ref{sec:cont_time_markov_chains}, we formalized stochastic processes, and provided ways to characterize them. We now turn to the field of \emph{imprecise probability}~\cite{Walley:1991vk} {\bf CITE CITE}, and will formalize in this section the notion of \emph{imprecise continuous-time Markov chains}. Here, rather than look at a single stochastic process $P\in\processes$, we instead consider jointly some \emph{set} of processes $\mathcal{P}\subseteq\processes$. We start by looking at how such sets can be described.
...
Therefore, we also introduce the notion of consistency of initial distributions. To this end, let $\mathcal{M}$ be an arbitrary set of probability mass functions on $\states$. We will then say that a process $P$ is \emph{consistent with $\mathcal{M}$ in initial distribution}, whenever $P(X_0)\in\mathcal{M}$. We now focus on sets of processes that are jointly consistent with some given $\rateset$ and $\mathcal{M}$.

However, rather than look at the set of \emph{all} processes consistent with such $\rateset$ and $\mathcal{M}$, we will for the sake of generality and ease of notation consider the consistent subset of a given set of processes $\mathcal{P}\subseteq\processes$. The usefulness of this notational convention will be made clear momentarily.
...
\subsection{Lower Expectations for \ictmc's} 
...
Rather than give a separate example of this method, we will illustrate Algorithm~\ref{alg:compute_singlevar_multiple_past} in the following example, where we use Corollary~\ref{cor:composition_lower_trans} for computing a function defined on multiple time points.
...
**** THE NEXT TWO LEMMAS STILL NEED SOME INTRO. ALSO NOT SURE IF THIS LOCATION IS OPTIMAL, STILL NEED TO CHECK

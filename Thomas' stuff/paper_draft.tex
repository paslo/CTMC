...
We provide in this work a formalism that lets us describe continuous-time discrete-state stochastic processes, that does not require us to make any of the above-mentioned simplifying assumptions. This formalism is then used to exactly characterize set-valued \ictmc's, and we investigate their properties. The concept of lower expectation is given an alternative operator-theoretic characterization, by means of a \emph{lower transition operator}, and its properties are investigated. We use this lower transition operator to derive tractable (polynomial runtime-complexity) algorithms to compute lower expectations for functions defined on the state space at an arbitrary (but finite) number of time points.
\end{abstract}

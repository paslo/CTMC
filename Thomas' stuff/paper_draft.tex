...
*** blabla, this was well-behaved, but the next one is not ***

\begin{exmp}
Consider any rate matrix $Q\in\mathcal{R}$ such that $\norm{Q}=1$ and, for all $i\in\nats_0$, let $Q_i\coloneqq iQ$ and let $\mathcal{T}_i$ be defined as in Example~\ref{exmp:limit_trans_mat_system}. Since $\{Q_i\}_{i\in\nats_0}$ satisfies the conditions of Example~\ref{exmp:limit_trans_mat_system} with $c=1$, the sequence $\{\mathcal{T}_i\}_{i\in\nats_0}$ then has a limit $\mathcal{T}\coloneqq \lim_{i\to\infty}\mathcal{T}_i$ in $\mathbb{T}$. The aim of this example is to show that, despite the fact that we know from Example~\ref{exmp:limit_trans_mat_system} that each of the transition matrix systems $\mathcal{T}_i$, $i\in\nats_0$, is well-behaved, this is not true for the limit $\mathcal{T}$.
...
\\
&=i-2^{-i}i^2-2\\
&\geq i-\frac{1}{2}i-2=\frac{i}{2}-2\geq \frac{n}{2}-2
\end{align*}
...
\end{equation*}
where, for every $\lambda\in[\nicefrac{1}{3},\nicefrac{2}{3}]$, $Q_\lambda\coloneqq\lambda Q_1+(1-\lambda)Q_2$ as in Example~\ref{exmp:markov_with_no_deriv}.
...
*** dit wordt versimpeld tot: kijk, er is inderdaad een proces (zelfs een Markov proces) dat $\mathcal{T}$ als zijn transitiematrixsysteem heeft, en herhalen dat we later bewijzen dat het well-behaved is ***
...
\begin{exmp}
Example~\ref{exmp:notwellbehaved}

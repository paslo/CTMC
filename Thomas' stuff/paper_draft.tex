...
Let $Q_1,Q_2\in\mathcal{R}$ be two commuting rate matrices---for example, let $Q_1\in\mathcal{R}$ be an arbitrary rate matrix and let $Q_2\coloneqq\lambda Q_1$, with $\lambda\in\realsnonneg$---and consider again the sequence $\{\mathcal{T}_i\}_{i\in\nats}$ from Example~\ref{exmp:limit_trans_mat_system}, which, as explained in that example, converges to a limit in $\mathbb{T}$. 

 and $Q_i\coloneqq Q_1$ if $i$ is odd and $Q_i\coloneqq Q_2$ if $i$ is even

Now denote this limit by $\mathcal{T}_*\coloneqq \lim_{i\to\infty}\mathcal{T}_i$. As we will show in this example, for any $\Delta\in(0,1]$, the transition matrix that corresponds to the transition matrix system $\mathcal{T}_*$ is then equal to

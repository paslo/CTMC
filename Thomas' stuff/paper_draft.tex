...
Let $w=\{s_0,s_1,\dots,s_m\}\subset\reals_{\geq0}$ be a finite set of time points, with $m\in\nats_0$, such that $s_0<s_1<\dots<s_m$.
...
\right\}
\end{equation*}
such that, for any $j\in\{0,\dots,m\}$, $u=\{s_0,\dots,s_{j-1}\}$ and $x_u\in\states^u$, $P_w(\cdot\,\vert X_u=x_u)$ is a probability mass function on $\states$. Then $P_w$ is a coherent conditional probability on $\mathcal{C}_w$.
\end{lemma}
\begin{proof}

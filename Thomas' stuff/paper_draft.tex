...
\Ensure{An approximation of the function $\underline{\mathbb{E}}_\rateset^\mathrm{W}[f(X_u,X_v)\,\vert\,X_u]$ in $\gambles(\states^u)$.}
    ...
    \Function{ComputeLowerExp}{$\rateset, u,v, f,\epsilon$}
		...
		\For{$i\in\{m,m-1,\ldots,0\}$}	\Comment{Running index of latest remaining time point.}
			\State $w \gets w\setminus s_i$ \Comment{Update remaining time points. If $i=0$ then $w=\emptyset$.}
			...
			\EndIf
			\Statex
			\State $g_i \gets g_i \in \gambles(\states^{u\cup w})$ \Comment{Create new function $g_i\in\gambles(\states^{u\cup w})$.}
			\For{$x_{u\cup w}\in\states^{u\cup w}$} \Comment{Compute all values for $g_i$.}
				\Statex \Comment{Set $g_i(x_{u\cup w})\coloneqq \left[L_r^{s_i}f(x_{u\cup w},X_{s_i})\right](x_r)$, where $x_r\in x_{u\cup w}$.}
				\State $h \gets$ {\tt ComputeLf}$\bigl(\rateset, r, s_i, f(x_{u\cup w}, X_{s_i}),\epsilon\bigr)$ %\Comment{Compute $L_r^sf(x_{u\cup w},X_s)\pm\epsilon$.}
				\State $g_i(x_{u\cup w}) \gets h(x_r)$ 
			\EndFor
%			\Statex
		\EndFor
		\Statex
		\State \Return{$g_0$}\Comment{Result is a function in $\gambles(\states^u)$.}
    \EndFunction
  \end{algorithmic}
\end{algorithm}

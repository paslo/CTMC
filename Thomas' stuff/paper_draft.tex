...
Consider any closed interval $\mathbf{I}\subseteq\realsnonneg$, and let $\mathcal{T}^{\mathbf{I}}$ be a family of transition matrices $T_t^s$ that is defined for all $t,s\in\mathbf{I}$ with $t\leq s$. Then $\mathcal{T}^{\mathbf{I}}$ is a restricted transition matrix system defined on $\mathbf{I}$ if and only if, for all $t,r,s\in\mathbf{I}$ with $t\leq r\leq s$, it holds that $T_t^s = T_t^rT_r^s$ and $T_t^t=I$.
\end{proposition}
...
Note however that this is not problematic for our purposes. In particular, suppose that we are only ever interested in conditional lower expectations with respect to $\mathcal{P}$. It should be clear that such conditioning makes the lower expectation invariant to the initial distribution, and hence we can just ignore the fact that $\mathcal{P}$ contains many processes that differ only in their initial distribution. This will be illustrated in Example~\ref{exmp:singleton_set_only_different_initial} below.

However, suppose that we \emph{are} interested in unconditional lower expectations. We distinguish two cases: either we consider a (precise) initial distribution $p$ and restrict the set $\mathcal{P}$ such that $P(X_0)=p(\cdot)$ for all $P\in\mathcal{P}$, or, we consider an imprecise initial distribution represented by a set $\mathcal{M}$ of probability mass functions on $\states$, and restrict $\mathcal{P}$ such that it contains exactly those processes whose initial distributions $P(X_0)$ are in this set $\mathcal{M}$.

In the first case, where we consider a precise initial distribution $p$, the unconditional lower expectation $\underline{\mathbb{E}}[\,\cdot\,]$ simply reduces to
\begin{align*}
\underline{\mathbb{E}}[\,\cdot\,] = \inf\{\mathbb{E}_P[\,\cdot\,]\,:\,P\in\mathcal{P}\} &= \inf\left\{\sum_{x\in\states}\mathbb{E}_P[\,\cdot\,\vert\,X_0=x]P(X_0=x)\,:\,P\in\mathcal{P}\right\}\\
 &= \inf\left\{\sum_{x\in\states}\mathbb{E}_P[\,\cdot\,\vert\,X_0=x]p(x)\,:\,P\in\mathcal{P}\right\} \\
 &\geq \sum_{x \in \states} \inf\{\mathbb{E}_P[\,\cdot\,\vert\,X_0=x]\,:\,P\in\mathcal{P}\} p(x) \\
 &= \sum_{x \in \states} \underline{\mathbb{E}}[\,\cdot\,\vert\,X_0=x]p(x)
\end{align*}

***** PROBLEEMPJE: IK HAD IN MN HOOFD ZITTEN DAT DIE ONGELIJKHEID EEN GELIJKHEID ZOU WORDEN. DIT KAN DUIDELIJK ALS $\mathcal{P}$ GROOT (FLEXIBEL) GENOEG IS, MAAR IS EEN BEETJE LASTIG OM DAT HIER TE FORMALISEREN.
...
\begin{exmp}\label{exmp:singleton_set_only_different_initial}
...
\begin{align*}
\underline{\mathbb{E}}[f(X_s)\,\vert\,X_{0}=x_0] &= \inf\left\{\mathbb{E}_P[f(X_s)\,\vert\,X_{0}=x_0]\,:\,P\in\mathcal{P}\right\} \\
...
Therefore, even though different processes may be consistent with the same set $\rateset$, their difference need not be relevant. In particular, this is the case whenever different processes have the same conditional expectation. However, when $\rateset$ is not singleton, the collection of processes consistent with $\rateset$ actually starts to exhibit differences that lead to different conditional expectations, and that are therefore relevant.
...
T_{\min\textbf{I}}^{\max\textbf{I}} &\text{ if $t<\min\textbf{I}$ and $\max\textbf{I}<s$}\\
T_t^{\max\textbf{I}}
&\text{ if $t\in\textbf{I}$ and $\max\textbf{I}<s$}\\
I &\text{ if $\max \mathbf{I}<t$}
\end{cases}

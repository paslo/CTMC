...
As in any modeling task, simplifying assumptions are made when we choose to model a problem using a continuous-time Markov chain. %We would argue however that in many cases, these assumptions are grounded more in pragmatism than in informed consideration of the underlying system. F
...
As a result of all these assumptions, continous-time Markov chains can be described using convenient analytic expressions. 


However, we would argue that in many cases, these assumptions are not realistic, and that they are grounded more in pragmatism than in informed consideration of the underlying system.
Therefore, we consider in this paper \emph{imprecise continuous-time Markov chains}, which can be used to model the same systems that are typicallly modelled with a continous-time Markov chain, but in a more robust way, without the need to impose the many simplifying  assumptions that we have just discussed. The following provides a motivating toy example that will return throughout this work. Although the model in this example is clearly too simple to be of practical use, it does already illustrate that the simplifying assumptions above are clearly unrealistic.

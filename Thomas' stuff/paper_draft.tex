...
The following additional properties can easily be shown to follow from \ref{def:coh_prob_1}--\ref{def:coh_prob_4}; see Appendix~\ref{app:prelim} for a proof. For all $A\in\power$ and all $C,D\in\nonemptypower$:
\vspace{5pt}
...
\begin{proposition}\label{prop:fullcoherent}
Let $P$ be a real-valued map from $\power\times\nonemptypower$ to $\reals$. Then $P$ is a full conditional probability if and only if it is a coherent conditional probability.
\end{proposition}


\begin{theorem}\label{theo:coherentextendable}
Let $P$ be a real-valued map from $\mathcal{C}\subseteq\power\times\nonemptypower$ to $\reals$. Then $P$ can be extended to a full conditional probability if and only if it is a coherent conditional probability.
\end{theorem}
...
\begin{proof}[Proof of \ref{def:coh_prob_8}-\ref{def:coh_prob_6}]
*** moet dit nog opschrijven ***
\end{proof}
...
First assume that $P$ can be extended to a full conditional probability $P^*$. Proposition~\ref{prop:fullcoherent} then implies that $P^*$ is a coherent conditional probability, and therefore, since $P$ is the restriction of $P^*$ to $\mathcal{C}$, it clearly follows from Definition~\ref{def:coherence} that $P$ is a coherent conditional probability.

Conversely, if $P$ is a coherent conditional probability, it follows from Reference~\cite[Theorem 4]{regazzini1985finitely} *** DO YOU HAVE A PDF OF THIS??? *** that $P$ can be extended to a coherent conditional probability $P^*$ on $\power\times\nonemptypower$, which, because of Proposition~\ref{prop:fullcoherent}, is a full conditional probability.
\end{proof}

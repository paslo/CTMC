...
Recall from the definition of the lower envelope $\lrate$ of a given set $\rateset$ of rate matrices, *** blabla, is benaderbaar als ssr.
...
**** blabla, paar decompositie lemmas

\begin{lemma}\label{lemma:weirddecomposition}
Consider any $P\in\wprocesses_\rateset$, any $t,s\in\realsnonneg$ such that $t<s$, any $u\in\mathcal{U}_{<t}$ and $x_u\in\states_u$ and any sequence $t=t_0<t_1<\cdots<t_n=s$, with $n\in\nats$.
...
**** kan het onderstaande niet direct via recursieve lemma?

...
As we will show in this example, for any $t\in(0,1]$, the transition matrix from $0$ to $t$ that corresponds to the transition matrix system $\mathcal{T}_*$ is then equal to
\vspace{4pt}
\begin{equation}\label{eq:ex4:def}
\presuper{*}T_0^t=e^{Q_1\varphi_1(t)+Q_2\varphi_2(t)},
\end{equation}
with
...
\begin{align*}
\norm{\presuper{*}T_0^t-\presuper{k}T_0^t}
&=
\norm{\presuper{*}T_0^{\delta_{i+1}}e^{Q_i(t-\delta_{i+1})}-\presuper{k}T_0^{\delta_{i+1}}e^{Q_i(t-\delta_{i+1})}}\\
&\leq
...
=\delta_k\norm{Q_1-Q_2},
\end{align*}
where the first equality follows from Equation~\eqref{eq:ex4:recursekandstar}, the second equality holds because $Q_1$ and $Q_2$ commute, and the two inequalities follow from Lemmas~\ref{lemma:recursive} and~\ref{lemma:differencebetweenexponentials} in the appendix. Hence, we find that $\norm{\presuper{*}T_0^t-\presuper{k}T_0^t}\leq\delta_k\norm{Q_1-Q_2}$. Since this is true for any $k\geq i$, it follows that $\lim_{k\to+\infty}\presuper{k}T_0^t=\presuper{*}T_0^t$. Therefore, because $\mathcal{T}_*\coloneqq \lim_{i\to\infty}\mathcal{T}_i$, we can conclude that $\presuper{*}T_0^t$ indeed corresponds to $\mathcal{T}_*$.
...
Hence, for all $i\in\nats_0$, Equation~\eqref{eq:ex4:def} now tells us that $\presuper{*}T_0^{\Delta_i}=e^{Q_\lambda\Delta_i}$.

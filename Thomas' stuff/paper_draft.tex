...
In this work, continuous-time stochastic processes were defined within the framework of full conditional probabilities, and using the notion of coherent conditional probabilities. Using the concept of well-behaved stochastic processes, we introduced a way to characterize the dynamics of such stochastic processes by means of their outer partial derivatives---without imposing differentiability assumptions on the conditional state-transition probabilities of these processes. We have seen that by imposing the Markov property on such stochastic processes, we derive continuous-time Markov chains. Under the further assumption of time-homogeneity of the state-transition probabilities, we have re-derived the well known time-homogeneous continuous-time Markov chains, which are those stochastic processes whose matrix of conditional state-transition probabilities $T_t^s$ is given by the matrix exponential $e^{Q(s-t)}$, for some rate matrix $Q\in\mathcal{R}$.

Using these definitions, we have defined \emph{imprecise continuous-time Markov chains} as sets of stochastic processes which are in a specific sense consistent with a given set of rate matrices $\rateset$. In particular, an \ictmc~is a set of stochastic processes whose outer partial derivatives are contained within such a set $\rateset$. We focused in this work on three specific types of \ictmc, namely the sets $\wprocesses_{\rateset}, \wmprocesses_{\rateset}$, and $\whmprocesses_{\rateset}$, which are the sets of all stochastic processes consistent with $\rateset$ and in addition, cumulatively satisfying well-behavedness, the Markov property, and time-homogeneity. We have investigated closure-properties of such sets of stochastic processes, in particular closure under recombination of known elements.

The concept of \emph{lower expectation} for \ictmc's was defined as the infimum expected value with respect to the set of expected values of some function $f$, induced by an \ictmc. *** verschillende moeilijkheid voor verschillende ictmcs, decompositie eigenschap, alternatieve karakterisatie, introductie $\lrate$ en canonical verzameling, definitie $L$, eigenschappen $L$, gebruik van $L$ voor uitrekenen, algoritmes voor uitrekenen $L$ en uitbreiding arbitraire functies, kleinste en grootste verzameling waarvoor ie werkt

...
\end{equation}
Assume \emph{ex absurdo} that this statement is not true. Then this implies that there are $1\leq i<j\leq m$ such that either $r_i-\delta_{r_i}>r_j-\delta_{r_j}$ or $r_i+\delta_{r_i}>r_j+\delta_{r_j}$. If $r_i-\delta_{r_i}>r_j-\delta_{r_j}$, then since $i<j$ implies that $r_i<r_j$, it follows that $\delta_{r_j}>\delta_{r_i}+r_j-r_i>\delta_{r_i}$ and therefore, that $r_j+\delta_{r_j}>r_i+\delta_{r_i}$. 
Hence, we find that $U_{r_i}\subseteq U_{r_j}$. Since $C^*$ was taken to be a minimal cover, this is a contradiction.
Similarly, If $r_i+\delta_{r_i}>r_j+\delta_{r_j}$, then since $i<j$ implies that $r_i<r_j$, it follows that $\delta_{r_i}>\delta_{r_j}+r_j-r_i>\delta_{r_j}$ and therefore, that $r_i-\delta_{r_i}<r_j-\delta_{r_i}$. Hence, we find that $U_{r_j}\subseteq U_{r_i}$. Since $C^*$ was taken to be a minimal cover, this is again a contradiction. From these two contradictions, it follows that Equation~\eqref{eq:orderingofbounds} is indeed true.
...
Since $r_k\in[t,s]$, this implies that $\r_k+\delta_{r_k}>s$.


Then $r_k+\delta_{r_k}\notin U_{r_{k+1}}$ and, because of Equation~\eqref{eq:orderingofbounds}, $r_k+\delta_{r_k}\notin U_{r_{i}}$  

For all $i\in\{k$


For all $i\in\{1,\dots, k-1\}$, it then follows from Equation~\eqref{eq:orderingofbounds} that $r_i+\delta_{r_i}\leq r_k+\delta_{r_k}$, which implies that $r_k+\delta_{r_k}\notin U_{r_i}$. 

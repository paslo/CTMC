...
On the one hand, {\v{S}kulj} defines an imprecise continuous-time Markov chain as a random process $X\colon\reals\to\states$ whose transition rate matrix is an unspecified function $Q_t\in\rateset$ \cite[Definition~1]{Skulj:2015cq}, without any further restrictions on $Q_t$---such as, for example, continuity. However, he does not provide a definition for $Q_t$, nor for the notion of a random process, and in that sense, this first definition of an imprecise continuous-time Markov chain is rather informal. Furthermore, the definition also does not make explicit whether the random process in question should satisfy a Markov property. However, given the use of the transition rate matrix $Q_t$, it seems to us that the Markov property is intended to be imposed implicitly.

On the other hand, more formally, for any $f\in\gamblesX$, {\v{S}}kulj considers a time-dependent function $f_s$, which---using our notation---is interpreted as $f_s\coloneqq T_0^s f$, and then requires that $f_0=f$,

...
*** I have skipped this for now... I have quite some questions and suggestions for this section ***
...
We have in Example~\ref{exmp:limit_trans_mat_system_matrices} constructed a transition matrix system $\mathcal{T}$ as the limit of a sequence $\{\mathcal{T}_i\}_{i\in\nats}$ of transition matrix systems. We there claimed that $\mathcal{T}$ was well-behaved, and have repeatedly used this property for illustration in Examples~REF, REF, and REF. However, up until this point, no proof of this fact was given.

Therefore, we will in this example illustrate the use of Theorem~\ref{theorem:restricted_transmatsystem_space_compact_if_Q_closed} to prove that $\mathcal{T}$ is indeed well-behaved. To this end, let $Q_1,Q_2\in\mathcal{R}$ be the two commuting rate matrices from Example~\ref{exmp:limit_trans_mat_system_matrices}, and let $\rateset$ be the convex hull of $\{Q_1,Q_2\}$. Then, clearly, $\rateset$ is a non-empty, bounded, convex and closed set of rate matrices.

Because the sequence $\{\mathcal{T}_i\}_{i\in\nats}$ is constructed as in Equation REF in Example~\ref{exmp:limit_trans_mat_system}, it follows from Propositions CONCAT and EXPONENTIAL that, for all $i\in\nats$, $\mathcal{T}_i$ is a well-behaved transition matrix system. Furthermore, due to Theorem MARKOV FROM TRANSMAT, there is some $P_i\in\wmprocesses$ such that $\mathcal{T}_{P_i}=\mathcal{T}_i$.

**** my comments about the outer partials for $P_i$ got removed...

See also Lemma REF to confirm that $P_i\in\wmprocesses_{\rateset}$.

Finally, we note that due to Equation SEQUENCEDEF and Proposition CONCAT, for all $i\in\nats$, $\mathcal{T}_i^{[\nicefrac{1}{2},\infty)}=\mathcal{T}_{Q_0}^{[\nicefrac{1}{2},\infty)}$, and therefore that also $\mathcal{T}^{[\nicefrac{1}{2},\infty)}=\mathcal{T}_{Q_0}^{[\nicefrac{1}{2},\infty)}$. Since we know from Proposition REF that $\mathcal{T}_{Q_0}^{[\nicefrac{1}{2},\infty)}$ is well-behaved, it suffices to show that $\mathcal{T}^{[0,\nicefrac{1}{2}]}$ is well-behaved since, due to Proposition CONCAT, this would imply that $\mathcal{T}=\mathcal{T}^{[0,\nicefrac{1}{2}]}\otimes \mathcal{T}_{Q_0}^{[\nicefrac{1}{2},\infty)}$ is well-behaved, which is what we intent to show.

Now, observe that because $P_i\in\wmprocesses_\rateset$ and $\mathcal{T}_{P_i}=\mathcal{T}_i$, it must hold that also $\mathcal{T}_i^{[0,\nicefrac{1}{2}]}\in\mathbb{T}_\rateset^{[0,\nicefrac{1}{2}]}$. Therefore, since the sequence $\{\mathcal{T}_i^{[0,\nicefrac{1}{2}]}\}_{i\in\nats}$ is in $\mathbb{T}_\rateset^{[0,\nicefrac{1}{2}]}$, Theorem COMPACT implies the existence of a convergent subsequence $\{\mathcal{T}_{i_k}^{[0,\nicefrac{1}{2}]}\}_{k\in\nats}$ of which the limit $\mathcal{T}_*^{[0,\nicefrac{1}{2}]}$ also belongs to $\mathbb{T}_\rateset^{[0,\nicefrac{1}{2}]}$.

**** Blabla, $\mathcal{T}_*^{[0,\nicefrac{1}{2}]}=\mathcal{T}^{[0,\nicefrac{1}{2}]}$ omdat deelrij.

Because $\mathcal{T}^{[0,\nicefrac{1}{2}]}\in\mathbb{T}_\rateset^{[0,\nicefrac{1}{2}]}$, this implies the existence of some $P\in\wmprocesses_\rateset$ with transition matrix system $\mathcal{T}_P$, such that $\mathcal{T}_P^{[0,\nicefrac{1}{2}]}=\mathcal{T}^{[0,\nicefrac{1}{2}]}$. Since $P\in\wmprocesses_\rateset$ is well-behaved, it follows from Theorem REF that $\mathcal{T}_P$ is well-behaved and therefore, that $\mathcal{T}^{[0,\nicefrac{1}{2}]}=\mathcal{T}_P^{[0,\nicefrac{1}{2}]}$ is well-behaved. As mentioned above, it now follows from Proposition CONCAT that $\mathcal{T}=\mathcal{T}^{[0,\nicefrac{1}{2}]}\otimes \mathcal{T}_{Q_0}^{[\nicefrac{1}{2},\infty)}$ is also well-behaved, as claimed.

%Furthermore, since $\{\mathcal{T}_i\}_{i\in\nats}\to\mathcal{T}$, due to the definition of the metric $d$ in Equation REF, we clearly also have $\{\mathcal{T}_i^{[0,\nicefrac{1}{2}]}\}_{i\in\nats}\to \mathcal{T}^{[0,\nicefrac{1}{2}]}$. **** can also use total-boundedness here to find the sequence explicitly

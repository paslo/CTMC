...
\end{equation}
If $S^*=\emptyset$, we let $\omega^*$ be any element of $C_0$. Clearly, this path $\omega^*$ will then also satisfy Equation~\eqref{eq:theo:aanelkaarplakken:geqfirstpart}.
...
\}.
\end{equation*}
We first consider the case $S^{**}\neq\emptyset$. Then for all $i\in S^{**}$, we have that $(A_i,C_i)\in\mathcal{C}_{x_u^*}$ and therefore, it follows that there are $u\subseteq v_i\in\mathcal{U}$ and $x_{v_i\setminus u}\in\states^{v_i\setminus u}$ such that
\begin{equation*}
...
\end{equation}
and $C_i^{*}\coloneqq(X_{v_i\cap [0,\max u]}=x_{v_i\cap [0,\max u]})$. Since $P_{x_u^*}$ is a stochastic process, it then follows from Corollary~\ref{corol:processiffcoherent} that

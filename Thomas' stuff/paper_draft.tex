...
Further on in this example, we will establish closed-form expression for some of the transition matrices $T_t^s$ that correspond to this limit $\mathcal{T}$, but first, let us prove that $\mathcal{T}$ is well-behaved.


*** Fix any $r\in[t,s]$ and consider any $\Delta>0$. Then due to Lemma~\ref{lemma:uniformelywellbehaved} (STILL NEED TO FIX THE FORWARD DEPENDENCIES IN THAT LEMMA), for all $i\in\nats$:
\begin{equation*}
\norm{T_r^{r+\Delta}-I}

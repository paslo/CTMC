...
In order to prove that $P$ is well-behaved, we consider any (possibly empty) time sequence $v\in\mathcal{U}$, any $x_v\in\states^v$ and any $x,y\in\states$. First fix any $t\in\reals_{\geq0}$ such that $t>v$.
...
<+\infty.
\end{equation*}
Since we have shown this to be true for both cases ($t<\max u$ and $t\geq\max u$), it follows that Equation~\eqref{eq:def:well-behaved:right} is satisfied. Next, fix any $t\in\reals_{>0}$ such that $t>v$.

*** still need to finish this ***

for any (possibly empty) time sequence $v\in\mathcal{U}$, any $x_v\in\states^v$ and any $x,y\in\states$:
\begin{equation*}
(\forall t\in\reals_{\geq0}
\colon t\geq v)~~
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t+\Delta}=y\vert X_t=x, X_v=x_v)-\delta_{xy}}<+\infty

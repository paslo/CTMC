...
\vspace{4pt}
\end{equation*}
As we know from Propositions~\ref{prop:systemQ} and~\ref{prop:concat_restr_trans_mat_systems_is_system}, transition matrix systems that are constructed in this way are always well-behaved. For example, the transition matrix systems $\mathcal{T}_i$, $i\in\nats_0$, that we defined in Equation~\eqref{eq:def:sequenceinexample3} are all of this form.

\begin{proposition}\label{prop:finite_different_rate_matrix_has_process}
*** er bestaat een proces dat stuksgewijs is ***
\end{proposition}
...
Consider a non-empty bounded set of rate matrices $\rateset$ and a non-empty set $\mathcal{M}$ of probability mass functions on $\states$ and a set of stochastic processes $\mathcal{P}\subseteq\processes$. Then, the \emph{subset of $\mathcal{P}$ consistent with} $\rateset$ \emph{and} $\mathcal{M}$ is denoted $\mathcal{P}_{\rateset,\mathcal{M}}$, and defined as
\begin{equation*}
...
In Sections~\ref{sec:stochastic_processes} and~\ref{sec:cont_time_markov_chains}, we introduced the sets $\wprocesses, \wmprocesses,$ and $\whmprocesses$ of well-behaved stochastic processes with different qualitative properties. In particular, $\wprocesses$ is the set of \emph{all} well-behaved stochastic processes. $\wmprocesses$ is a subset of $\wprocesses$, the elements of which satisfy the Markov property. $\whmprocesses$ is the set of time-homogeneous Markov chains, which is therefore a subset of $\wmprocesses$. We will use the notational convention from Definition~\ref{def:consistent_process_set} to focus on \ictmc's which also have these different qualitative properties, as follows.
...
\end{itemize}
Following Definition~\ref{def:consistent_process_set}, we will write $\wprocesses_{\rateset}$ when we take $\mathcal{M}$ to be the set of \emph{all} probability mass functions on $\states$ (and similarly for these other sets). 
\end{definition}

Because the sets $\wprocesses, \wmprocesses,$ and $\whmprocesses$ are subsets of each other, it should be clear that this also holds for the different types of \ictmc's introduced above.
...
%*** {\bf TODO}, but trivial.
%\end{proof}
Observe furthermore that the qualitative differences between these sets simplify the notion of consistency from Definition~\ref{def:consistent_process}. In particular, it holds that
\begin{equation*}
...
The following example further illustrates the difference between these different types of \ictmc's.

\begin{exmp}\label{example:rateset_not_singleton}
Let $Q_1,Q_2\in\mathcal{R}$ be two rate matrices such that $Q_1\neq Q_2$, and consider the set $\rateset\coloneqq \{Q_1, Q_2\}$. Let furthermore $\mathcal{M}\coloneqq\{p\}$, with $p$ an arbitrary probability mass function on $\states$.

The set $\whmprocesses_{\rateset,\mathcal{M}}$ then contains exactly two processes. In particular, these are the well-behaved time-homogeneous Markov chains $P_1,P_2\in\whmprocesses$ for which $P_1(X_0)=P_2(X_0)=p$, $Q_{P_1}=Q_1$, and $Q_{P_2}=Q_2$, where $Q_{P_1},Q_{P_2}$ are the two rate matrices that characterize $P_1$ and $P_2$, respectively, as in Theorem~\ref{theo:homogeneoushasQ}. The transition matrices of $P_1$ and $P_2$ are therefore given, respectively, by ${}^1T_t^s=e^{Q_1(s-t)}$ and ${}^2T_t^s=e^{Q_2(s-t)}$. Put differently, the families of transition matrices of these processes are given by the exponential transition matrix systems $\mathcal{T}_{Q_1}$ and $\mathcal{T}_{Q_2}$, respectively.

Furthermore, since $\whmprocesses_{\rateset,\mathcal{M}}\subseteq \wmprocesses_{\rateset,\mathcal{M}}$, these time-homogeneous processes also satisfy $P_1,P_2\in\wmprocesses_{\rateset,\mathcal{M}}$. However, $\wmprocesses_{\rateset,\mathcal{M}}$ also contains different processes, which are not time-homogeneous and therefore not in $\whmprocesses_{\rateset,\mathcal{M}}$. 

For instance, for any $r\in\realsnonneg$, there will be some $P_r\in\wmprocesses_{\rateset,\mathcal{M}}$ such that, for all $t<r$, $\overline{\partial}T_t^t=\{Q_1\}$ and, for all $t>r$, $\overline{\partial}T_t^t=\{Q_2\}$. Therefore, $P_r$ will be ``piecewise'' time-homogeneous on the intervals $[0,r]$ and $[r,+\infty)$, and its family of transition matrices $\mathcal{T}_{P_r}$ will be given by the transition matrix system $\mathcal{T}_{Q_1}^{[0,r]}\otimes\mathcal{T}_{Q_2}^{[r,+\infty)}$. It is readily verified that this $P_r$ therefore satisfies $\overline{\partial}T_r^r=\{Q_1,Q_2\}$, since in this case $\overline{\partial}_-T_r^r=\{Q_1\}$ and $\overline{\partial}_+T_r^r=\{Q_2\}$.
...
In turn, all these elements of $\wmprocesses_{\rateset,\mathcal{M}}$ are also in $\wprocesses_{\rateset,\mathcal{M}}$, but this latter set contains more complicated processes still. For instance, for any $u\in\mathcal{U}$ and any $x_u,y_u\in\states_u$ such that $x_u\neq y_u$, there will be some $P\in\wprocesses_{\rateset,\mathcal{M}}$ such that, for all $t>u$, $\smash{\overline{\partial}}T_{t,x_u}^t=\{Q_1\}$ and $\smash{\overline{\partial}}T_{t,y_u}^t=\{Q_2\}$. Therefore, for all $s>t>u$, the history-dependent transition matrices of this $P$ will be given by $T_{t,x_u}^s=e^{Q_1(s-t)}$ and $T_{t,y_u}^s=e^{Q_2(s-t)}$.
...
We conclude this section with some notes about closure properties of these different \ictmc's, which provide powerful tools for existence proofs. In particular, we focus on closure properties under recombination of known elements---colloquially, the ``piecing together'' of two or more processes to construct a new process that belongs to the same \ictmc.

The example above already suggests how to do this for $\wmprocesses_{\rateset,\mathcal{M}}$. Using the transition matrix systems $\mathcal{T}_{P_1}, \mathcal{T}_{P_2}$ of any two $P_1,P_2\in\wmprocesses_{\rateset,\mathcal{M}}$, we can construct a new transition matrix system $\mathcal{T}\coloneqq \mathcal{T}_{P_1}^{[0,t]}\otimes \mathcal{T}_{P_2}^{[t,+\infty)}$, for any $t\in\realsnonneg$. Theorem~\ref{theo:uniqueMarkovchain} then guarantees that there exists a Markov chain $P$ for which $\mathcal{T}_P=\mathcal{T}$ and such that $P(X_0)\in\mathcal{M}$. It is straightforward to verify that, because $P_1,P_2\in\wmprocesses_{\rateset,\mathcal{M}}$, also $P\in\wmprocesses_{\rateset,\mathcal{M}}$; we leave this as an exercise for the reader.

Now, clearly, a similar procedure is impossible for $\whmprocesses_{\rateset,\mathcal{M}}$, since the combination of two processes would make the resultant one lose the homogeneity property which is required to be an element of this set.

However, for the most general set $\wprocesses_{\rateset,\mathcal{M}}$, it turns out to be possible to recombine elements in a history-dependent way, under the assumption that $\rateset$ is convex. That is, under this assumption it is possible, for fixed time points $u\in\mathcal{U}$, to choose for every history $x_u\in\states_u$ a different process $P_{x_u}\in\wprocesses_{\rateset,\mathcal{M}}$, and to recombine these into a process $P$ that agrees with these $P_{x_u}$ conditional on the specific history $x_u$. Furthermore, the distribution on the time points $u$ can be chosen to agree with any element $P_{\emptyset}\in \wprocesses_{\rateset,\mathcal{M}}$. The following result then guarantees that this new process $P$ also belongs to $\wprocesses_{\rateset,\mathcal{M}}$.
...
From a practical point of view, after having specified a (precise) stochastic process, one is typically interested in the expected value of some function of interest. Similarly, in this work, our main object of consideration will be the \emph{lower expectation} with respect to the \ictmc's that we introduced in the previous section.
...
As specific cases, we will denote the lower expectation with respect to $\wprocesses_{\rateset,\mathcal{M}}$ as $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^{\mathrm{W}}$, with respect to $\wmprocesses_{\rateset,\mathcal{M}}$ as $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^{\mathrm{WM}}$, and with respect to  $\whmprocesses_{\rateset,\mathcal{M}}$ as $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^{\mathrm{WHM}}$. Following Definition~\ref{def:consistent_process_set}, we will write $\underline{\mathbb{E}}_{\rateset}^{\mathrm{W}}$ when we take $\mathcal{M}$ to be the set of \emph{all} probability mass functions on $\states$ (and similarly for these other lower expectations).
\end{definition}
...
First of all, it can be shown that if $\rateset$ is a non-empty, bounded, convex and closed set of rate matrices, then the conditional lower expectations $\underline{\mathbb{E}}_{\rateset}^\mathrm{W}, \underline{\mathbb{E}}_{\rateset}^\mathrm{WM},$ and $\underline{\mathbb{E}}_{\rateset}^\mathrm{WHM}$ are actually minima---rather than infima---because they are always reached by some element of their corresponding \ictmc. However, the proof of this claim is extremely involved and requires some machinery that is rather outside the scope of this paper; we intend to publish these and related results separately in future work.

Moving on, we noted in Section~\ref{subsec:types_ictmc} that the sets $\whmprocesses_{\rateset,\mathcal{M}}, \wmprocesses_{\rateset,\mathcal{M}},$ and $\wprocesses_{\rateset,\mathcal{M}}$ are nested subsets of each other. As an immediate consequence, their corresponding lower expectations provide (lower) bounds for each other.
...
For now, we first provide a useful property of the lower expectation $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}$ for $\wprocesses_{\rateset,\mathcal{M}}$. To this end, consider a function $f\in\gambles(\states_{u\cup v\cup w})$ defined on an arbitrary number of time points $u,v,w\in\mathcal{U}$. It follows from the basic properties of expectation that for any process $P\in\wprocesses_{\rateset,\,\mathcal{M}}$, the expectation $\mathbb{E}_P$ of such a function satisfies,
\begin{equation*}
...
This property stated by Theorem~\ref{theorem:decomposition_multivar} relies on the fact that the elements of $\wprocesses_{\rateset,\mathcal{M}}$ can have history-dependent probabilities, which is why it does not in general hold for $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{WM}$ or $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{WHM}$. Because, as mentioned above, this property does simplify the process of computing the lower expectation $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}$, this suggests that the qualitative differences between our various \ictmc's make some of these lower expectations harder to compute than others. In fact, we will show in Section~\ref{sec:funcs_multi_time_points} that $\wprocesses_{\rateset\,,\mathcal{M}}$ is in some sense the easiest set to do this for, exactly because of Theorem~\ref{theorem:decomposition_multivar}.

Furthermore, it---ironically---turns out that computing lower expectations for the set $\whmprocesses_{\rateset\,,\mathcal{M}}$ is in general much harder than for the sets $\wmprocesses_{\rateset\,,\mathcal{M}}$ or $\wprocesses_{\rateset\,,\mathcal{M}}$. The problem is, essentially, that while homogeneous Markov chains are easy to work with numerically, the set $\whmprocesses_{\rateset\,,\mathcal{M}}$ does not provide enough ``degrees of freedom'' to make the optimization problem involved in computing $\underline{\mathbb{E}}^\mathrm{WHM}_{\rateset\,,\mathcal{M}}$ tractable. The following serves as an illustration. 
...
We start by generalizing transition matrices $T$ to lower transition operators $\lt$.
...
Such lower transition operators furthermore satisfy the following properties---see Reference~\cite{DeBock:2016} for a proof. For any lower transition operator $\lt$ and any two non-negatively homogeneous operators $A,B$ from $\gamblesX$ to $\gamblesX$,
...
For any two lower transition operators $\lt,\underline{S}\in\underline{\mathcal{T}}$, their composition is also a lower transition operator  $\left(\lt\,\underline{S}\right)\in\underline{\mathcal{T}}$.
\end{proposition}
\begin{proof}
...
\end{equation*}
We call this map $\underline{\mathbb{E}}$ the \emph{lower envelope} of $\mathcal{M}$, and it should be clear that this map computes a lower expectation with respect to $\mathcal{M}$. Furthermore, note that for any $p\in\mathcal{M}$, the quantity $\sum_{x\in\states}p(x)f(x)$ is bounded from below by $\min\{f(y):y\in\states\}$. Because this holds for any $p\in\mathcal{M}$, it follows that $\underline{\mathbb{E}}$ is also bounded below by this minimum operator. Furthermore, it is easily verified that $\underline{\mathbb{E}}$ is super-additive and non-negatively homogeneous, due to the properties of the $\inf$ operator. Hence, by the definition cited above, this lower expectation operator corresponding to $\mathcal{M}$ is a coherent lower prevision on $\gamblesX$. 

Our consideration of the coherent lower prevision $\lt_x$  above is essentially the same idea, but \emph{without} considering explicitly any link to sets $\mathcal{M}$ of probability mass functions. Suffice it to say that, for any coherent lower prevision, such a set $\mathcal{M}$ will always exist {\bf CITE}. It should furthermore be noted that the qualifier ``coherent'' here has connections to the notion of \emph{coherence} that we mentioned in Section~\ref{sec:cond_prob}. We refer to {\bf CITE CITE CITE} for further discussion on this.

Because for any lower transition operator $\lt$ and any $x\in\states$, the map $\lt_x$ is a coherent lower prevision, it follows that the lower transition operator $\lt$ can be seen as a vector of coherent lower previsions. Furthermore, as mentioned above, each coherent lower prevision $\lt_x$ has some set $\mathcal{M}_{\lt_x}$ of probability mass functions of which $\lt_x$ computes the lower envelope {\bf CITE}. Therefore, and since the set $\mathcal{M}_{\lt}\coloneqq \times_{x\in\states}\mathcal{M}_{\lt_x}$ can be interpreted as a set of transition matrices---the elements $T\in\mathcal{M}_{\lt}$ are row-stochastic, with $T(x,\cdot)\in\mathcal{M}_{\lt_x}$ for all $x\in\states$---the operator $\lt$ can intuitively be seen as a lower envelope operator over this set. That is, for every lower transition operator $\lt$, there is some set $\mathcal{M}_{\lt}$ of transition matrices, such that for any $f\in\gamblesX$,
\begin{equation*}
\lt f = \inf\{ Tf\,:\, T\in\mathcal{M}_{\lt} \}\,.
\end{equation*}
Due to the correspondence between transition matrices and conditional expectation operators, as noted by Remark~\ref{remark:expectationT}, it should therefore be clear that lower transition operators are an intuitive starting point for alternative characterizations of lower expectations for \ictmc's. In particular, what remains is to find the specific lower transition operator $\lt$, whose set of dominating transition matrices $\mathcal{M}_{\lt}$ corresponds to the set of transition matrices induced by a given \ictmc; the remainder of this section will provide the machinery required to do this.
...
We conclude with a powerful result about the set $\underline{\mathcal{T}}$ of all lower transition operators; this set is a complete metric space under our usual norm.
...
\subsection{Lower Transition Rate Operators and Sets of Rate Matrices}\label{sec:connections_rate}
...
The first reason that this specific generalization is of interest, is that it preserves the relation between transition matrices and rate matrices, as stated by Propositions~\ref{prop:stochastic_from_rate_matrix} and~\ref{prop:rate_from_stochastic_matrix}. The following two propositions make this explicit in our current setting.
...
\section{\ictmc's and Corresponding Lower Transition Operators}\label{sec:connections}
...
\subsection{Lower Expectations and Corresponding Lower Transition Operators}\label{sec:single_var_lower_exp}
...
In order to establish the connection between lower expectations and lower transition operators $L_t^s$, we first recall that lower expectations are defined with respect to a given \ictmc, which in turn is derived from a given set $\rateset$ of transition rate matrices, as in Definition~\ref{def:process_sets}. In contrast, the lower transition operator $L_t^s$ is derived from a given lower transition rate operator $\lrate$. We have already seen in Section~\ref{sec:connections_rate} that there is a strong connection between sets of rate matrices $\rateset$ and lower transition rate operators $\lrate$. In particular, any set $\rateset$ has a corresponding lower transition rate operator $\lrate$, which computes the lower envelope with respect to $\rateset$, as in Equation~\eqref{eq:correspondinglowertrans}. It is this connection between sets of rate matrices and lower transition rate operators that we will use to establish the connection between $L_t^s$ and lower expectations.
...
Thus, we find that there is indeed a correspondence between the operator $L_t^s$ and lower expectations for \ictmc's.

Furthermore, observe that Corollary~\ref{cor:lower_operator_is_infimum} states that $L_t^sf = \underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_s)\,\vert\,X_t,X_u]$ for \emph{all} histories $u\in\mathcal{U}_{<t}$. Therefore, and since the definition of $L_t^s$ does not depend on this choice of $u$, it follows that
\begin{equation*}
\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_s)\,\vert\,X_t,X_u] = \underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_s)\,\vert\,X_t].
\end{equation*}
This equality explains why we call $\wprocesses_{\rateset,\mathcal{M}}$ an imprecise continuous-time Markov chain. That is, the conditional lower expectation $\underline{\mathbb{E}}_{\rateset,\mathcal{M}}^\mathrm{W}[f(X_s)\,\vert\,X_t,X_u]$ depends only on the state at time $t$---it satisfies the Markov property---\emph{even though} $\wprocesses_{\rateset,\mathcal{M}}$ \emph{contains non-Markov processes}! 

Now, Corollary~\ref{cor:lower_operator_is_infimum} also establishes that the correspondence between $L_t^s$ and lower expectations is not one-to-one. Clearly, $L_t^s$ computes the lower expectation for the different sets $\wmprocesses_\rateset$ and $\wprocesses_\rateset$. Furthermore, we know from Section~\ref{sec:connections_rate} that different sets $\rateset_1$ and $\rateset_2$ may have the same corresponding lower transition rate operator $\lrate$. Hence, whenever this is the case, $L_t^s$ would---assuming the conditions in Corollary~\ref{cor:lower_operator_is_infimum} are met by both $\rateset_1$ and $\rateset_2$---then compute the lower expectation with respect to both $\wmprocesses_{\rateset_1}$ and $\wmprocesses_{\rateset_2}$ (and also, furthermore, for $\wprocesses_{\rateset_1}$ and $\wprocesses_{\rateset_2}$). An obvious question, therefore, would be what the largest set of stochastic processes is for which $L_t^s$ computes the lower expectation.
...
A second conclusion that we can draw from Theorem~\ref{theo:dominating_rate_processes_max_set}, is that it vindicates our choice to focus on \emph{well-behaved} stochastic processes---rather than, say, continuous or differentiable stochastic processes. In particular, since we aim to use $L_t^s$ as a computational tool for lower expectations, the notion of well-behavedness allows us to characterize exactly those sets of stochastic processes for which we can do this. 
...
\subsection{Computing Lower Expectations with Corresponding Lower Transition Operators}\label{subsec:compute_single_var}

We have in the previous section shown that (conditional) lower expectations can be computed using the operator $L_t^s$. However, in order to make these results practicable, we still require a method to numerically compute the function $L_t^sf$. Because we are now interested in numerical methods, we will focus on computing such a quantity $L_t^sf$ within a guaranteed $\epsilon$-error bound. 
...
Finally, this earlier work provided two different algorithms to numerically compute the solution of the differential equation~\eqref{eq:damjans_diff}. The first algorithm presented therein works similarly to our Algorithm~\ref{alg:compute_singlevar}, in that it computes a uniform partition $u=t_0,t_1,\ldots,t_n$ of the time interval $[0,s]$, so that at each step $t_i$, one only has to solve a relatively straightforward linear optimization problem. However, it turns out that our Algorithm~\ref{alg:compute_singlevar} is much more efficient, because the number of steps $n$ that we require in this partition is much smaller. 

For instance, for the numerical example presented at the end of~\cite[Section 4.1]{Skulj:2015cq}, {\v{S}}kulj finds that this partition requires at least $n=6000$ steps. In contrast, due to Proposition~\ref{prop:approximation_error_bound}, Algorithm~\ref{alg:compute_singlevar} requires only $n=100$ steps for that particular example. Similarly, in our Example~\ref{exmp:single_time_numerical} we found that we required $n=1.6\times 10^{4}$ steps. Applying the algorithm from~\cite{Skulj:2015cq}, by comparison, requires that $n\approx 2.2\times 10^9$.

These observations may go some way in explaining a comment made by the authors of Reference~\cite{troffaes2015using}, in which they applied this algorithm from Reference~\cite{Skulj:2015cq} in a practical application. In particular, they observed that the number of steps required to obtain good results, was much lower than was expected from the theoretical bound used to derive their algorithm.

*** nog een opmerking over zijn tweede algorithme, en dat het wat moeilijker bruikbaar is en zn eerste gebruikt.
...
Since $\lim_{i\to\infty}\sigma(u_i)=0$, and because of Propositions~\ref{lemma:normQsmallenough} and~\ref{lemma:compositioncoherence} and property~\ref{LR:normlratefinite}, there is some index $n$ such that the sequence $\Phi_{u_n},\Phi_{u_{n+1}},\ldots$ consists of lower transition operators. Due to Corollary~\ref{corol:cauchy}, this sequence is Cauchy and therefore, because of Proposition~\ref{lemma:completemetricspace}, this sequence has a limit that is also a lower transition operator. Since the limit starting from $n$ and the limit starting from $1$ are identical (initial elements do not influence the limit), we find that the sequence $\left\{\Phi_{u_i}\right\}_{i\in\nats}$ has a limit, and that this limit is a lower transition operator.
\end{proof}
...
First of all, recall from Proposition~\ref{prop:outerderivativebehaveslikelimit} that, for any $\epsilon\in\realspos$ and over a small enough time step $\Delta$, the transition matrix $T_{t,x_u}^{t+\Delta}$ of any stochastic process $P\in\wprocesses_{\rateset,\mathcal{M}}$ can be approximated by $(I+\Delta Q)$ with an error of at most $\Delta \epsilon$, using some rate matrix $Q\in\overline{\partial}T_{t,x_u}^t\subseteq \mathcal{Q}$. The following lemma states that, for any time interval $[t,s]$, there exists a finite partition $u\in\mathcal{U}_{[t,s]}$, $u=t_0,\ldots,t_n$, such that the transition matrices $T_{t_i,x_u}^{t_{i+1}}$ can all be similarly approximated by $(I+\Delta_iQ_i)$, for some $Q_i\in\rateset$ and with $\Delta_i=t_{i+1}-t_i$. 

The reason that this result is not entirely straightforward is the ``small enough'' requirement to approximate $T_{t,x_u}^{t+\Delta}$; this only works for $\Delta<\delta_t$, for some $\delta_t\in\realspos$ that is specific to the time $t$. Because we are now looking at multiple time points simultaneously, we need a guarantee that these $\delta_t$ do not vanish as we change $t$---this is what the following lemma essentially establishes.
...
Roughly speaking, this result establishes that the transition matrix $T_{t,x_u}^s$ of any well-behaved stochastic process $P\in\wprocesses$ can be decomposed into a number of other transition matrices corresponding to this $P$, such that---crucially---these other transition matrices do not depend on the exact way that the decomposition was performed, despite $P$ not necessarily being a Markov process.
...
The final result that we require has to do with the connection between $\rateset$ and $\lrate$. Due to the definition of the lower transition rate operator $\lrate$ corresponding to a given $\rateset$, it holds for all $\Delta\in\realsnonneg$, all $Q\in\rateset$, and all $f\in\gamblesX$ that $(I+\Delta Q)f \geq (I+\Delta\lrate)f$. The following lemma establishes that this inequality extends to compositions of such operators $(I+\Delta Q)$ and $(I+\Delta\lrate)$.
...
For all $i\in\{0,\ldots,n\}$, let $\mathcal{T}_{Q_i}$ denote the transition matrix system corresponding to $Q_i$, as in Definition~\ref{def:systemfromQ}. Then, by Proposition~\ref{prop:nonhomogeneous_in_process_set}, there is some $P\in\wmprocesses_{\rateset,\,\mathcal{M}}$ with transition matrix system $\mathcal{T}_P$, such that
\begin{equation*}

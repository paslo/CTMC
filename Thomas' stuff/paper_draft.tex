...
\lambda & -\lambda & 0 \\
0 & 0 & 0
\end{array}\right] \,:\,\mu\in[0.01,0.5],\lambda\in[0.01,0.5]\right\}\,,
\end{equation*}
let $f\in\gamblesX$ be defined as $f\coloneqq [0~1~2]^\top$, and compute the lower expectation at time $t=10$, conditional on $X_0=2$, to find
\begin{equation*}
\underline{\mathbb{E}}_{\rateset}^\mathrm{WHM}[f(X_{10}),\vert\,X_0=2] \approx 0.16038\,.
\end{equation*}
...
By comparison, for the non-homogeneous case, the optimization using Algorithm~\ref{alg:compute_singlevar}, with $\epsilon=10^{-2}$, gives
\begin{equation*}
\underline{\mathbb{E}}_{\rateset}^\mathrm{WM}[f(X_{10}),\vert\,X_0=2] \approx 0.128954\pm 10^{-2}\,,
\end{equation*}
which \emph{should} establish the difference within error tolerances, although it's close given the (unknown-error) approximation I used for the homogeneous case. My Mathematica implementation of Algorithm~\ref{alg:compute_singlevar} is \emph{super slow} though, so I didn't yet check for higher precision. Will start that computation after committing this, will hopefully finish some time tonight.
\exampleend
\end{exmp}

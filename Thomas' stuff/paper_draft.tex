...
A real-valued $m\times m$ matrix $A$ is said to be a \emph{transition matrix} if
\vspace{5pt}
...
Consider any $m\times m$ transition matrix $A$, and any $\Delta\in\realspos$. Let $I$ denote the $m\times m$ identity matrix. Then, $[\nicefrac{1}{\Delta}\cdot(A-I)]$ is a transition rate matrix.
\end{proposition}
\begin{proof}
...
\subsubsection{*** onder van homogene ***}
...
\subsubsection{*** onder van inhomogene Markov ***}
...
\subsubsection{*** onder van niet-Markov}

...
Note first of all that this definition takes conditional probabilities as the basic entity; $P(A\vert C)$ is well-defined even if $P(C)=0$. Furthermore, by property~\ref{def:coh_prob_3}, it only requires that the probability is finitely-additive; specifically, $\sigma$-additivity is not imposed by the axiomatization, regardless of the size of $\power$. We will later see that this is not problematic for our current work. We refer to {\bf CITE CITE} for philosophical motivations behind this.
...
Therefore, we next introduce the notion of \emph{coherent conditional probabilities}~{\bf CITE}. This notion was developed from DeFinetti's work on coherent (unconditional) probabilities~{\bf CITE}. We refer to {\bf CITE CITE CITE} for philosophical discussions on its merits. From a practical point of view, suffice it to say that the following definition provides us with a convenient alternative characterization of properties~\ref{def:coh_prob_2}-\ref{def:coh_prob_6} above.
...
The reason that this result is of interest to us, is that it allows us to prove the existence of certain processes $P\in\wmprocesses_\rateset$. In particular, it allows us to establish the existence of limits of sequences $\left\{P_i\right\}_{i\in\nats}$ in $\wmprocesses_\rateset$ that satisfy certain behavior on a specified interval $[t,s]$. This works because, since the space $\mathbb{T}^{[t,s]}$ is compact, not only do convergent sequences converge within this set, any sequence in this set will always have a convergent subsequence.
...
It should be clear from the above that lower transition operators $\lt$ are a good starting point for alternative representations of lower expectations for imprecise continuous-time Markov chains. We next focus on the generalization of transition rate matrices $Q$ to lower transition rate operators $\lrate$, as follows.

...
\begin{lemma}
Consider any non-empty, bounded and closed set of rate matrices $\rateset$, and any $t,s\in\realsnonneg$ such that $t\leq s$. Then, the metric space $(\mathcal{T}_\rateset^{[t,s]},d)$ is totally bounded under the metric $d$, defined as in Equation~\eqref{eq:trans_mat_system_metric}.
\end{lemma}
\begin{proof}
...
\begin{lemma}
Consider any $t,s\in\realsnonneg$ such that $t\leq s$, any non-empty, bounded and convex set of rate matrices $\rateset$, and any $u\in\mathcal{U}_{[t,s]}$ with $u=t_0,\ldots,t_n$. For all $i\in\{1,\ldots,n\}$, choose some $Q_i\in\rateset$. Then, there exists a $Q\in\rateset$ such that
\begin{equation*}
\prod_{i=1}^n(I + \Delta_iQ_i) = \prod_{i=1}^n(I + \Delta_iQ)\,.
\end{equation*}
\end{lemma}
\begin{proof}
*** I have this written down somewhere (take $Q$ as the weighted average of the $Q_i$, using weights $\nicefrac{\Delta_i}{(s-t)}$).
\end{proof}

\begin{lemma}
Let $\rateset$ be any non-empty, bounded and convex set of rate matrices. Then, for all $P\in\wmprocesses_\rateset$, all $t,s\in\realsnonneg$ with $t\leq s$, all $\epsilon>0$, and all $\delta>0$, there is some $u\in\mathcal{U}_{[t,s]}$ with $\sigma(u)<\delta$, and some $Q\in\rateset$, such that
\begin{equation*}
\norm{T_{t}^s - \prod_{i=1}^n(I+\Delta_iQ)} < (s-t)\epsilon\,.
\end{equation*}
\end{lemma}
\begin{proof}

...
If we now let $C^*\coloneqq\cap_{1\leq \ell<m}(X_{w_\ell}=\omega(w_\ell))$, then by the assumptions of this lemma, there is some probability mass function $p$ on $\states$ such that, for all $x\in\states$, $P_w(X_{w_m}=x\vert C^*)=p(x)$. Define $S^*\coloneqq\{i\in\{1,\dots,n\}\colon C_i=C^*\}$ and $\states^*\coloneqq\{x\in\states\colon (\exists i\in S^*) z_{m,i}=x\}$. Then clearly, for any $x\in\states^*$, there is some $i\in S^*$ such that $z_{m,i}=x$. Furthermore, since $(A_{i_1},C_{i_1})\neq(A_{i_2},C_{i_2})$ for all $i_1,i_2\in\{1,\dots,n\}$ such that $i_1\neq i_2$, this $i$ is unique. We will denote it by $i_x$ and let $\lambda_x\coloneqq\lambda_{i_x}$. For all $x\in\states\setminus\states^*$, we let $\lambda_x\coloneqq 0$. Since $p$ is a probability mass function, it then follows that
\begin{equation*}

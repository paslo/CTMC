...
\end{equation*}
Indeed, it is easily verified that any rate matrix $Q\in\mathcal{R}$ will satisfy properties~\ref{LR:constantzero}-\ref{LR:nondiagpos} from Definition~\ref{def:coh_low_trans_rate}, and hence any such $Q$ is in fact a lower transition rate operator. It follows from the above that the lower transition operator $L_t^s$ corresponding to this $\lrate=Q$ will satisfy $L_t^s=e^{Q(s-t)}$. We can conclude from this that Equation~\eqref{eq:lowerbound} expresses a proper generalization of the matrix exponential, that can be used when working with lower transition rate operators.

Even stronger, if $\lrate=Q$ for some $Q\in\mathcal{R}$, we find that the family $\underline{\mathcal{T}}_{\lrate}$ is equal to $\mathcal{T}_Q$, the exponential transition matrix system from Definition~\ref{def:systemfromQ} that, by Corollary~\ref{cor:rate_has_unique_homogen_markov_process}, corresponds to some well-behaved homogeneous Markov chain $P\in\whmprocesses$.

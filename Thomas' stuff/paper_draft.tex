...
Now, because $n$ is finite, there exists a finite sequence of time points $u\in\mathcal{U}$ such that $0\in u$ and, for all $i\in\{1,\ldots,n\}$, $t_i\in u$. Let $y\in\states$ be any state such that $y\neq x$. Consider a state assignment $x_u\in\states_u$ such that $x_{0}\coloneqq x$ and, for all $i\in\{1,\ldots,n\}$,
\begin{equation*}
...
\end{array} \right.
\end{equation*}
Again because $n$ is finite, Equation~\eqref{eq:path_exists_for_finite_points} guarantees that there is some $\omega\in\Omega$ such that $\omega\vert_{u}=x_u$. Furthermore, because $\omega(0)=x$, we clearly have that $\omega\in (X_0=x)\subset\Omega$. Evaluating the summand in Equation~\eqref{exmp:coherence:eq:coherence} using this $\omega$, we find that
\begin{align*}

...
=2^{-k}\leq2^{-n_\epsilon},
\end{equation*}
it follows that $\smash{d(\mathcal{T}_{k},\mathcal{T}_\ell)\leq c 2^{-n_\epsilon}}<\epsilon$, as required.\exampleend
...
Let $Q_1,Q_2\in\mathcal{R}$ be two commuting rate matrices. For example, let $Q_1\in\mathcal{R}$ be an arbitrary rate matrix and let $Q_2\coloneqq\lambda Q_1$, with $\lambda\in\realsnonneg$. 

Now let $\{Q_i\}_{i\in\nats_0}$ be defined by $Q_i\coloneqq Q_1$ if $i$ is odd and $Q_i\coloneqq Q_2$ if $i$ is even, and consider the corresponding sequence of transition matrix systems $\{\mathcal{T}_i\}_{i\in\nats}$ that was defined in Example~\ref{exmp:limit_trans_mat_system}.  Since $\norm{Q_i-Q_{i-1}}=\norm{Q_1-Q_2}$ for all $i\in\nats$, the sequence $\{Q_i\}_{i\in\nats_0}$ clearly satisfies the conditions in Example~\ref{exmp:limit_trans_mat_system}---just choose $c=\norm{Q_1-Q_2}$---and therefore, as we have seen, $\{\mathcal{T}_i\}_{i\in\nats}$ converges to a limit in $\mathbb{T}$. Let us denote this limit by $\mathcal{T}_*\coloneqq \lim_{i\to\infty}\mathcal{T}_i$. 

As we will show in this example, for any $\Delta\in(0,1]$, the transition matrix that corresponds to the transition matrix system $\mathcal{T}_*$ is then equal to
\vspace{4pt}
...
**** blabla, voorbeeldje voor waarom we zoveel moeite doen met de definitie

\begin{exmp}
**** I'll construct a stochastic process $P$ that satisfies $P(X_t=x\,\vert\,X_0=x)=p(t)$ for all $t\in\realsnonneg$, for given $x\in\states$ and any given $p:\realsnonneg\to[0,1]$ that satisfies $p(0)=1$.

We assume for this example that $\states$ is not singleton, i.e., there are at least two states $x,y\in\states$. Fix any $x\in\states$. Construct a set of events $\mathcal{C}\subset\mathcal{C}^\mathrm{SP}$ as
\begin{equation*}
\mathcal{C} \coloneqq \left\{ (X_t=x,X_0=x)\in\mathcal{C}^\mathrm{SP} : t\in\realsnonneg \right\}.
\end{equation*}
Let $p:\realsnonneg\to[0,1]$ be any function that satisfies $p(0)=1$. Consider a map $\tilde{P}:\mathcal{C}\to\reals$, defined for all $(X_t=x,X_0=x)\in\mathcal{C}$ as
\begin{equation*}
\tilde{P}(X_t=x\vert X_0=x) \coloneqq p(t)\,.
\end{equation*}
We will now show that $\tilde{P}$ is a coherent conditional probability on $\mathcal{C}$. In order to show this, consider any $n\in\nats$ and, for all $i\in\{1,\ldots,n\}$, any $(X_{t_i}=x,X_0=x)\in\mathcal{C}$ and any $\lambda_i\in\reals$. According to Definition~\ref{def:coherence}, we now have to show that
\begin{equation*}
...
Note that each of the conditional events $(X_{t_i}=x,X_0=x)$ depends only on the time points 0 and $t_i$. Observe that we can assume without loss of generality that $t_i\neq t_j$ for all $i,j\in\{1,\ldots,n\}$ such that $i\neq j$. **** maybe explain why, by taking $\lambda_{i'}\coloneqq \lambda_i+\lambda_j$ instead.

**** also note that if $t_i=0$ for some $i$, the term cancels out because $p(0)=1$. explain this

Because $n$ is finite, this implies the existence of a finite sequence of time points $u\in\mathcal{U}$ such that $0\in u$ and, for all $i\in\{1,\ldots,n\}$, $t_i\in u$. Let $y\in\states$ be any state such that $y\neq x$. Consider a state assignment $x_u\in\states_u$ such that $x_{0}\coloneqq x$ and, for all $i\in\{1,\ldots,n\}$,
\begin{equation*}
...
\end{array} \right.
\end{equation*}
Now, Equation~\eqref{eq:path_exists_for_finite_points} guarantees that there is some $\omega\in\Omega$ such that $\omega\vert_{u}=x_u$. Furthermore, because $\omega(0)=x$, we clearly have that $\omega\in (X_0=x)\subset\Omega$. Therefore, we find that
\begin{align*}
...
\end{align*}
where the last inequality follows from the fact that $\tilde{P}(X_{t_i}=x\vert X_0=x)=p(t_i)\in [0,1]$\,. Therefore, and because $\omega\in C_0$, we find that Equation~REF indeed holds. Hence, we conclude that $\tilde{P}$ is a coherent conditional probability on $\mathcal{C}$.

Corollary REF now implies that $\tilde{P}$ can be extended to a full conditional probability $\tilde{P}^*$. If we now let $P$ be the restriction of $\tilde{P}^*$ to $\mathcal{C}^\mathrm{SP}$, then due to Corollary REF, $P$ is a stochastic process that, because $\mathcal{C}\subset\mathcal{C}^\mathrm{SP}$, extends $\tilde{P}$. Therefore, Equation REF implies that the stochastic process $P$ satisfies $P(X_t=x\vert X_0=x)=p(t)$ for all $t\in\realspos$.
\exampleend
\end{exmp}
...
\begin{exmp}
*** just a sketch for now, don't even know yet if it will work ***

Consider any rate matrix $Q\in\mathcal{R}$ such that $\norm{Q}=1$.
Let $Q_i\coloneqq 2^{i-1}Q$ and consider the corresponding sequence $\mathcal{T}_i$. Then $\{Q_i\}_{i\in\nats_0}$ satisfies the conditions of Example~\ref{exmp:limit_trans_mat_system}, and therefore $\{\mathcal{T}_i\}_{i\in\nats_0}$ has a limit $\mathcal{T}_*$ in $\mathbb{T}$.

Since $\mathcal{T}_*=\lim_{i\to+\infty}\mathcal{T}_i$, there is some $n\in\nats$ such that, for all $i\geq n$, $d(\mathcal{T}_*,\mathcal{T}_i)\leq\nicefrac{1}{8}$, and therefore also, for all $\Delta\in\realsnonneg$, $\norm{\presuper{i}T_0^\Delta-\presuper{*}T_0^\Delta}\leq\nicefrac{1}{8}$.

Consider now any $\Delta\in(0,\Delta_n]$ and let $i$ be the unique element of $\nats_0$ such that $\Delta_{i+1}\leq\Delta\leq\Delta_i$.
It then follows from Equation~\eqref{eq:ex4:def} that $\presuper{i}T_0^{\Delta}=e^{Q_i\Delta}$, and therefore, we find that
\begin{align*}
\norm{\frac{1}{\Delta}(\presuper{*}T_0^\Delta-I)}
&=\norm{\frac{1}{\Delta}\left(\presuper{*}T_0^\Delta-\presuper{i}T_0^\Delta+e^{Q_i\Delta}-(I+\Delta Q_i)\right)+Q_i}\\
&\geq
\norm{Q_i}
-\norm{\frac{1}{\Delta}\left(e^{Q_i\Delta}-(I+\Delta Q_i)\right)}-\norm{\frac{1}{\Delta}(\presuper{*}T_0^\Delta-\presuper{i}T_0^\Delta)}
\\
\end{align*}
\exampleend
\end{exmp}

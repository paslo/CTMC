...
Continuous-time Markov chains are mathematical models that can be used to describe the behavior of dynamical systems under stochastic uncertainty. In particular, they can describe the stochastic evolution of such a system through a discrete state-space and over a continuous time-dimension. This class of models has found widespread applications in various fields, including queuing theory~\cite{asmussen2008applied,bolch2006queueing}, mathematical finance~\cite{elliott2013default, rolski2009stochastic,sass2004optimizing}, epidemiology~\cite{ duffy1995estimation,jackson2003multistate, lemey2009reconstructing}, system reliability analysis~\cite{besnard2010approach,gokhale2004analysis, wang2007reliability}, and many others~\cite{yin2012continuous}.

As in any modeling task, simplifying assumptions are made when we choose to model a problem using a continuous-time Markov chain. We would argue, however, that in many cases, these assumptions are grounded more in pragmatism than in informed consideration of the underlying system.

For example, it is typically assumed that the parameters of the model are known exactly. They may be estimated and therefore inherently approximate

 In the particular case of a continous-time Markov chain, two simplifying assumptions are often made. The most restrictive one is time-homogeneity, which assumes that the behavior of the system is independent of the absolute point in time at which it is considered; the Markov assumption, that the future behavior of the system only depends on its current state, and not on its history; and differentiability of the state-transition probabilities, which leads to convenient analytic expressions but which need not hold if the system is exposed to unknown external perturbations. 

Therefore, we consider in this paper \emph{imprecise continuous-time Markov chains}, which can be used to robustly model systems for which the above assumptions need not hold. The following provides a motivating toy example that will return throughout this work. Although the model in this example is clearly too simple to be of practical use, it does already illustrate that the simplifying assumptions above are clearly unrealistic.
...
Let $Q_1,Q_2\in\mathcal{R}$ be two commuting rate matrices---for example, let $Q_1\in\mathcal{R}$ be an arbitrary rate matrix and let $Q_2\coloneqq\lambda Q_1$, with $\lambda\in\realsnonneg$---and consider again the sequence $\{\mathcal{T}_i\}_{i\in\nats}$ from Example~\ref{exmp:limit_trans_mat_system}, which, as explained in that example, converges to a limit in $\mathbb{T}$. Now denote this limit by $\mathcal{T}_*\coloneqq \lim_{i\to\infty}\mathcal{T}_i$. As we will show in this example, for any $\Delta\in(0,1]$, the transition matrix that corresponds to the transition matrix system $\mathcal{T}_*$ is then equal to
...
&\text{ if $\Delta_{i+1}\leq\Delta\leq\Delta_{i}$ with $i$ odd}\\
\nicefrac{2}{3}\Delta_{i+1}
&\text{ if $\Delta_{i+1}\leq\Delta\leq\Delta_{i}$ with $i$ even} 
\end{cases}
\end{equation}
and
...
=\presuper{*}T_0^{\Delta_{j+1}}e^{Q_j(t-\Delta_{j+1})}.
\end{align}
...
\presuper{*}T_0^{\Delta_j}
=\presuper{*}T_0^{\Delta_{j+1}}e^{Q_j\Delta_{j+1}}
~\text{ and }~
...
Using these properties, proving Equation~\eqref{eq:ex4:def} is now relatively easy. Consider any $\Delta\in(0,1]$, let $i$ be the unique element of $\nats_0$ such that $\Delta_{i+1}\leq\Delta\leq\Delta_i$, and fix any $k\geq i$. Then on the one hand, we find that
\begin{align*}
\norm{\presuper{*}T_0^\Delta-\presuper{k}T_0^\Delta}
&=
\norm{\presuper{*}T_0^{\Delta_{i+1}}e^{Q_i(\Delta-\Delta_{i+1})}-\presuper{k}T_0^{\Delta_{i+1}}e^{Q_i(\Delta-\Delta_{i+1})}}\\
&\leq
\norm{\presuper{*}T_0^{\Delta_{i+1}}-\presuper{k}T_0^{\Delta_{i+1}}}
+
\norm{e^{Q_i(\Delta-\Delta_{i+1})}-e^{Q_i(\Delta-\Delta_{i+1})}}\\
&=
\norm{\presuper{*}T_0^{\Delta_{i+1}}-\presuper{k}T_0^{\Delta_{i+1}}}
=
\norm{\presuper{*}T_0^{\Delta_{i+2}}e^{Q_{i+1}\Delta_{i+2}}-\presuper{k}T_0^{\Delta_{i+2}}e^{Q_{i+1}\Delta_{i+2}}}\\
&\quad\quad\quad\quad\quad\quad\quad\quad~\,\,\leq
\norm{\presuper{*}T_0^{\Delta_{i+2}}-\presuper{k}T_0^{\Delta_{i+2}}}
+
\norm{e^{Q_{i+1}\Delta_{i+2}}-e^{Q_{i+1}\Delta_{i+2}}}\\
&\quad\quad\quad\quad\quad\quad\quad\quad~\,\,=
\norm{\presuper{*}T_0^{\Delta_{i+2}}-\presuper{k}T_0^{\Delta_{i+2}}}
\leq\dots\leq
\norm{\presuper{*}T_0^{\Delta_{k}}-\presuper{k}T_0^{\Delta_{k}}},
\end{align*}
...
=\Delta_k\norm{Q_1-Q_2},
\end{align*}
where the first equality follows from Equation~\eqref{eq:ex4:recursekandstar}, the second equality holds because $Q_1$ and $Q_2$ commute, and the two inequalities follow from Lemmas~\ref{lemma:recursive} and~\ref{lemma:differencebetweenexponentials} in the appendix. Hence, we find that $\norm{\presuper{*}T_0^\Delta-\presuper{k}T_0^\Delta}\leq\Delta_k\norm{Q_1-Q_2}$. Since this is true for any $k\geq i$, it follows that $\lim_{k\to+\infty}\presuper{k}T_0^\Delta=\presuper{*}T_0^\Delta$. Therefore, because $\mathcal{T}_*\coloneqq \lim_{i\to\infty}\mathcal{T}_i$, we can conclude that $\presuper{*}T_0^\Delta$ indeed corresponds to $\mathcal{T}_*$.
...
\begin{proposition}
Consider an arbitrary non-empty and bounded set of rate matrices $\rateset$, and any two non-empty sets $\mathcal{M}_1,\mathcal{M}_2$ of probability mass functions on $\states$. Consider any $P\in\wprocesses_{\rateset,\,\mathcal{M}_1}$. Then, for all $p\in\mathcal{M}_2$, there exists a $\tilde{P}\in\wprocesses_{\rateset,\,\mathcal{M}_2}$ such that $\tilde{P}(X_0)=p$ and, for all $(A,X_u=x_u)\in\mathcal{C}^\mathrm{SP}$ with $u\neq\emptyset$, it holds that $\tilde{P}(A\,\vert\,X_u=x_u)=P(A\,\vert\,X_u=x_u)$.

Furthermore, if $P\in\wmprocesses_{\rateset,\,\mathcal{M}_1}\subseteq \wprocesses_{\rateset,\,\mathcal{M}_1}$ then $\tilde{P}\in\wmprocesses_{\rateset,\,\mathcal{M}_2}\subseteq \wprocesses_{\rateset,\,\mathcal{M}_2}$, and similarly, if $P\in\whmprocesses_{\rateset,\,\mathcal{M}_1}\subseteq \wprocesses_{\rateset,\,\mathcal{M}_1}$ then $\tilde{P}\in\whmprocesses_{\rateset,\,\mathcal{M}_2}\subseteq \wprocesses_{\rateset,\,\mathcal{M}_2}$.
\end{proposition}
\begin{proof}
Choose any $P\in\wprocesses_{\rateset,\,\mathcal{M}_1}$ and any $p\in\mathcal{M}_2$. Consider a set of conditional events $\mathcal{C}\subset\mathcal{C}^\mathrm{SP}$, defined by
\begin{equation*}
\mathcal{C} \coloneqq \left\{(A,X_u=x_u)\in\mathcal{C}^\mathrm{SP}\,:\,u\neq\emptyset\right\} \cup \left\{ (X_0=x,X_{\emptyset}=x_{\emptyset})\in\mathcal{C}^\mathrm{SP}\,:\,x\in\states \right\}.
\end{equation*}
Let $P^*$ be a real-valued map from $\mathcal{C}$ to $\reals$ that is defined, for all $(A,X_u=x_u)\in\mathcal{C}$, as
\begin{equation}\label{eq:prop_initial_dist_swap_def}
P^*(A\,\vert\,X_u=x_u) \coloneqq \left\{ \begin{array}{ll}
p(x) & \text{if $u=\emptyset$ and $A=(X_0=x)$, and} \\
P(A\,\vert\,X_u=x_u) & \text{otherwise.}
\end{array}\right.
\end{equation}
We will start by proving that $P^*$ is a coherent conditional probability on $\mathcal{C}$.

So, consider any $n\in\nats$ and, for all $i\in\{1,\ldots,n\}$, choose $(A_i,C_i)\in\mathcal{C}$ and $\lambda_i\in\reals$. We need to show that
\begin{equation}\label{eq:prop_initial_dist_swap_coherent}
\max\left\{ \sum_{i=1}^n\lambda_i\ind{C_i}(\omega)\left(P^*(A_i\,\vert\,C_i) - \ind{A_i}(\omega)\right) \,\vert\, \omega\in C_0\right\} \geq 0\,,
\end{equation}
with $C_0\coloneqq \cup_{i=1}^nC_i$. 

Let $S\coloneqq\left\{i\in\{1,\ldots,n\}\,:\,C_i = (X_{\emptyset}=x_{\emptyset}) = \Omega\right\}$, that is, $S$ is the index-set of the \emph{unconditional} events $(A_i,C_i)$. Note that if $S=\emptyset$, it follows from Equation~\eqref{eq:prop_initial_dist_swap_def} that $P^*(A_i\,\vert\,C_i)=P(A_i\,\vert\,C_i)$ for all $i\in\{1,\ldots,n\}$, and hence Equation~\eqref{eq:prop_initial_dist_swap_coherent} will be trivially satisfied because $P$ is a coherent conditional probability on $\mathcal{C}^\mathrm{SP}\supset\mathcal{C}$. Hence, we can assume without loss of generality that $S\neq\emptyset$. 

Note that for all $i\in S$, $A_i=(X_0=x)$ for some $x\in\states$. Let $x_i\in\states$ denote these states such that $A_i=(X_0=x_i)$. Then, because $p$ is trivially coherent\footnote{This might need some explanation?}, it follows that there is some $\omega\in\Omega$ such that
\begin{align*}
\sum_{i\in S} \lambda_i \ind{C_i}(\omega)\left(P^*(A_i\,\vert\,C_i) - \ind{A_i}(\omega)\right) &= \sum_{i\in S} \lambda_i \ind{\Omega}(\omega)\left(P^*(X_0=x_i\,\vert\,X_{\emptyset}=x_{\emptyset}) - \ind{(X_0=x_i)}(\omega)\right) \\
&= \sum_{i\in S} \lambda_i\left(p(x_i) - \ind{(X_0=x_i)}(\omega)\right) \geq 0\,.
\end{align*}
Note also that this sum only depends on the exact path $\omega$ through the state $\omega(0)$.

We next consider the indices of the conditional events $(A_i,C_i)$, $i\in\{1,\ldots,n\}\setminus S$, whose conditioning event $C_i$ does not disagree with $\omega(0)$, that is, we consider the index set
\begin{equation*}
S^* \coloneqq \{ i\in\{1,\ldots,n\}\setminus S : C_i\cap(X_0=\omega(0))\neq\emptyset \}\,.
\end{equation*}
Because, for all $i\in S^*$, the events $(A_i,C_i)$ are conditional events, it follows from Equation~\eqref{eq:prop_initial_dist_swap_coherent} that $P^*(A_i\,\vert\,C_i)=P(A_i\,\vert\,C_i)$ for all $i\in S^*$. Therefore, and because $P$ is a coherent conditional probability on $\mathcal{C}^\mathrm{SP}\supset\mathcal{C}$, there is some $\omega^*\in \cup_{i\in S^*}C_i$ such that
\begin{equation*}
\sum_{i\in S^*} \lambda_i\ind{C_i}(\omega^*)\left(P^*(A_i\,\vert\,C_i) - \ind{A_i}(\omega^*)\right) \geq 0\,.
\end{equation*}

For any $i\in S^*$, because $(A_i,C_i)\in\mathcal{C}\subset\mathcal{C}^\mathrm{SP}$, there exists some finite sequence of time points $w_{C_i}\in\mathcal{U}$ such that $C_i$ only depends on the time points in $w_{C_i}$. Furthermore, and using this notation, it follows from Definition~\ref{def:stoch_process} that $A_i$ is an element of an algebra $\mathcal{A}_{w_{C_i}}$ that is generated by a set of events that only depend on a finite number of time points. Therefore, there is also some finite sequence of time points $w_{A_i}\in\mathcal{U}$, such that $A_i$ only depends on the time points in $w_{A_i}$. If we now let $w_i\coloneqq w_{A_i}\cup w_{C_i}$, then $(A_i,C_i)$ only depends on the (finite) sequence of time points $w_i$.

Because this holds for any $i\in S^*$, this implies the existence of some finite sequence $w\in\mathcal{U}$ such that $w_i\subseteq w$ for all $i\in S^*$.

**** Still need the fact that either $\omega^*(0)=\omega(0)$, or $0\notin w$. This is clear, but not easy to write down succinctly.

Now let $\omega^{**}\in\Omega$ be any path such that, for all $s\in (\{0\}\cup w)$,
\begin{equation*}
\omega^{**}(s) \coloneqq \left\{ \begin{array}{ll}
\omega(0) & \text{if $s=0$, and} \\
\omega^*(s) & \text{otherwise.}
\end{array}\right.
\end{equation*}
The existence of this $\omega^{**}\in\Omega$ is guaranteed by Equation~\ref{eq:path_exists_for_finite_points}. Furthermore, since by assumption $S\neq \emptyset$, there is some $C_i=\Omega$, so that $C_0 = \cup_{i=0}^n C_i = \Omega$, and hence $\omega^{**}\in C_0$.


-- Still need to prove that $\omega^{**}\in C_0$ implies that Equation~\eqref{eq:prop_initial_dist_swap_coherent} holds. Should be trivial from here though.

%A- $p$ is trivially coherent, so there is some $\omega$ such that the events indexed by $S$ sum to non-negative. Furthermore, that sum only depends on $\omega$ through $\omega(0)$.

%2B- All the conditional events $i\in S^*$ for which $(X_0=\omega(0))\cap C_i=\emptyset$ can be made to drop out, due to the term $\ind{C_i}(\omega)$.

%C- For the remaining events, since $P$ is coherent, there is an $\omega^*$ so that those terms sum to non-negative. Furthermore, because $(X_0=\omega(0))\cap C_i\neq\emptyset$, we must have that $\omega^*(0)=\omega(0)$. Because the terms dropped out in step $B$ were not compatible with $\omega(0)$, they are also not compatible with $\omega^*(0)$. Hence, the remaining sum is a sum of two non-negative sums, so must be non-negative itself. Since $\omega^*\in \cup C_{i^*}\subset C_0$, we must have that $P^*$ is coherent on $\mathcal{C}$.

-- Sketch of the remainder:

-- Because $P^*$ is coherent on $\mathcal{C}\subset\mathcal{C}^\mathrm{SP}$, it can be extended to a coherent conditional probability $\tilde{P}$ on $\mathcal{C}^\mathrm{SP}$. By corollary REF, that makes $\tilde{P}$ a stochastic process.

-- Clearly, due to equation~\eqref{eq:prop_initial_dist_swap_def} and because $\tilde{P}$ extends $P^*$, all conditional probabilities $\tilde{P}(A\,\vert\,X_u=x_u)$, with $u\neq\emptyset$, will coincide with $P(A\,\vert\,X_u=x_u)$. Therefore, the transition matrices of $\tilde{P}$ and $P$ are the same, and hence so are their outer partial derivatives. Similarly, because $P$ is well-behaved, so is $\tilde{P}$. Therefore, and because $P\sim\rateset$ (since $P\in\wprocesses_{\rateset,\,\mathcal{M}_1}$), we must have that $\tilde{P}\sim\rateset$. Because by Equation~\ref{eq:prop_initial_dist_swap_def} we also have that $\tilde{P}(X_0)=p\in\mathcal{M}_2$, it follows that $\tilde{P}\in\wprocesses_{\rateset,\,\mathcal{M}_2}$.

-- Similarly, Markov and homogeneity properties of $P$ trivially transfer to $\tilde{P}$.
\end{proof}

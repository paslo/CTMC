...
~
\norm{
T^{s_{i+1}}_{s_i,\,x_u}-(I+\Delta_{i+1}Q)
}<\Delta_{i+1}\epsilon,
\end{equation*}
with $\Delta'_{i+1}\coloneqq s_{i+1}-s_i<\delta$. If $s_1\neq t$, we also define $s_0\coloneqq t$. 

We now consider two cases: $s_1=t$ and $s_1\neq t$.

If $s_1=t$, we let $t_i\coloneqq s_{i+1}$ for all $i\in\{0,\dots,2m-2\}$. If $s_{2m-1}=t$, this establishes the result [choose $n\coloneqq 2m-2$]. If $s_{2m-1}\neq t$

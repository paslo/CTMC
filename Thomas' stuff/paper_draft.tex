...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require{Non-empty, bounded set of rate matrices $\rateset$ with separately specified rows, state assignment $x_u\in\states^u$, and $m$ functions $g_i\in\gambles(\states^{u\cup\{s_i\}})$.}
    \Ensure{$\hat{\underline{\mathbb{E}}}_\rateset^\mathrm{W}[f(x_{t_0},\ldots,x_{t_n},X_{s_0},\ldots,X_{s_m})\,\vert\,X_{t_0}=x_{t_0},\ldots,X_{t_n}=x_{t_n}]$, where $f=\sum_{i=0}^m g_i$.}
    \Statex
    \Function{ComputeSFSSum}{$\rateset, x_u, g_0,\ldots,g_m$}
      \State $\hat{f}\gets 0$
		\For{$i\in\{0,\ldots,m\}$}
		\State $\hat{g}_i\gets [L_{s_{i-1}}^{s_i}g_i](x_u)$
		\State $\hat{f}\gets\hat{f} + \hat{g}_i$
		\EndFor
      \State \Return{$\hat{f}$}
    \EndFunction
  \end{algorithmic}
...
\begin{align*}
 ...
 &\approx \sum_{i=0}^m \hat{g}_i(x_{t_0},\ldots,x_{t_n})\,,
\end{align*}
where $\hat{g}_i:\!\approx L_{s_{i-1}}^{s_i}g_i$ is the numerical result of computing $L_{s_{i-1}}^{s_i}g_i$.
\end{proposition}
\begin{proof}

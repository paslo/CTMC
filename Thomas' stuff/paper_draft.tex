...
Consider any two commuting rate matrices $Q_1,Q_2\in\mathcal{R}$. For example, let $Q_1\in\mathcal{R}$ be an arbitrary rate matrix and let $Q_2\coloneqq\lambda Q_1$, with $\lambda\in\realsnonneg$.
...
Consider now any $\Delta\in(0,1]$, let $i$ be the unique element of $\nats_0$ such that $\Delta_{i+1}\leq\Delta\leq\Delta_i$, and consider any $k\geq i$. Then
\begin{align*}
...
\norm{\presuper{*}T_0^{\Delta_{i+2}}-\presuper{k}T_0^{\Delta_{i+2}}}
\leq\dots\leq
\norm{\presuper{*}T_0^{\Delta_{k+1}}-\presuper{k}T_0^{\Delta_{k+1}}}
\end{align*}

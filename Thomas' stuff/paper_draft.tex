...
For all $i\in\{2,\dots,n\}$, since $\Delta_i\leq\Delta\leq\nicefrac{1}{\norm{\rateset}}\leq\nicefrac{1}{\norm{Q_i}}$, it follows from Proposition~\ref{prop:stochastic_from_rate_matrix} that $(I+\Delta_iQ_i)$ and $I$ are (obviously non-negatively homogeneous) transition matrices, which, because of Proposition~\ref{prop:normoftransisone}, implies that $\norm{I+\Delta_iQ_i}\leq1$ and $\norm{I}\leq1$. Therefore,
\begin{multline*}
...
For all $i\in\{2,\dots,n\}$, since $\Delta_i\leq\nicefrac{1}{\norm{\lrate}}$, it follows from Proposition~\ref{lemma:normQsmallenough} that $(I+\Delta_i\lrate)$ and $I$ are lower transition operators and, because of Proposition~\ref{lemma:normofcoherenttrans}, that $\norm{I+\Delta_i\lrate}\leq1$ and $\norm{I}\leq1$. Therefore,
\begin{multline*}
...
For any $k\in\{1,\dots,n\}$, consider a sequence $\Delta_{k,i}>0$, $i\in\{1,\dots,n_k\}$, and let $\Delta_k\coloneqq\sum_{i=1}^{n_k}\Delta_{k,i}$. Let $C\coloneqq\sum_{k=1}^n\Delta_k$ and let $\delta\coloneqq\max_{k=1}^n\Delta_k$. Then for any lower transition rate operator $\lrate$ such that $\delta\leq\nicefrac{1}{\norm{\lrate}}$:
\begin{equation*}
...
\end{lemma}
\begin{proof}
Fix any $k\in\{1,\dots,n\}$. Then for all $i\in\{1,\dots,n_k\}$, since $\Delta_{k,i}\leq\Delta_k\leq\delta\leq\nicefrac{1}{\norm{\lrate}}$, it follows from Propositions~\ref{lemma:normQsmallenough} and~\ref{lemma:compositioncoherence} that $\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)$ and $(I+\Delta_k\lrate)$ are lower transition operators and, because of Proposition~\ref{lemma:normofcoherenttrans}, that $\norm{\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)}\leq1$ and $\norm{(I+\Delta_k\lrate)}\leq1$. Therefore,
\begin{align*}
\norm{\prod_{k=1}^n\left(\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)\right)
-
\prod_{k=1}^n(I+\Delta_k\lrate)

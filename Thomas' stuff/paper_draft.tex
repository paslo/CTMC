...
{T^0_{0}}$ is a non-empty set of rate matrices. Let $Q$ be any of its elements.

Because of Equation~\eqref{eq:rightouterderivative}, we know that there is some strictly positive sequence $(\Delta_i)_{i\in\nats}$ such that $\lim_{i\to+\infty}\Delta_i=0$ and
\begin{equation*}
\lim_{i\to+\infty}
\frac{1}{\Delta_i}
(T^{\Delta_i}_{0}-I)
...
Fix any $c\geq0$ and any $i\in\nats$. Let $n_{c,i}\coloneqq\lfloor\nicefrac{c}{\Delta_i}\rfloor$ and let $d_{c,i}\coloneqq c-n_{c,i}\Delta_i$. Then clearly, $0\leq d_{c,i}<\Delta_i$.

\begin{equation*}
T_0^c=\left(
\prod_{j=1}^{n_{i,c}}
T_{(j-1)\Delta_i}^{j\Delta_i}
\right)
T_{n_{c,i}\Delta_i}^c
=\left(T_0^{\Delta_i}\right)^{n_{i,c}}
T_0^{d_{c,i}}
\end{equation*}
...
\leq
n_{i,c}\norm{T_0^{\Delta_i}-e^{Q\Delta_i}}
+\norm{T_0^{d_{c,i}}-e^{Qd_{i,c}}}
\end{equation*}

\begin{equation*}
\norm{T_0^{\Delta_i}-e^{Q\Delta_i}}
\leq
\norm{T_0^{\Delta_i}-(I+\Delta_iQ)}
+
\norm{(I+\Delta_iQ)-e^{Q\Delta_i}}
\leq
\end{equation*}


We will now prove that $T_0^c=e^{Qc}$.
...
Consider any $Q\in\mathcal{R}$ and any $\Delta>0$. Then
\begin{equation*}

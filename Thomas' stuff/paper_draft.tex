...
Since $n$ and $\abs{\rateset_{\epsilon_2}}$ are finite, $\mathbb{C}_\epsilon$ is clearly finite and, because of Claim~\ref{claim:composition_restricted_trans_mat_sys_in_set}, $\mathbb{C}_\epsilon$ is a subset of $\mathbb{T}_\rateset^{[t,s]}$. The only thing that we still need to prove is that $\mathbb{C}_\epsilon$ satisfies Equation~\eqref{lemma:restricted_trans_mat_system_totally_bounded_if_Q_bounded_convex:thereisacover}.

So fix any $\mathcal{T}^{[t,s]}\in\mathbb{T}_\rateset^{[t,s]}$. %We will show that there is some $\mathcal{S}^{[t,s]}\in \mathbb{C}_{\epsilon}$ such that $d(\mathcal{T}^{[t,s]},\mathcal{S}^{[t,s]})<\epsilon$. We start by constructing this $\mathcal{S}^{[t,s]}$.
Then for all $i\in\{1,\dots,n\}$, since $\Delta_i\leq\sigma(u)<\delta\leq\delta_1$, 

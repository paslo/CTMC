...
Fix any $\epsilon\in\realspos$. We will start by constructing $C_\epsilon$.

**** Find $\delta$, find $\epsilon'$.
...
Consider any $u\in\mathcal{U}_{[t,s]}$ with $\sigma(u)<\delta$, such that $u=t_0,\ldots,t_n$. Consider any $i\in\{1,\ldots,n\}$. We then let $\mathcal{T}_{Q_j}^{[t_{i-1},t_i]}\subset\mathcal{T}_{Q_j}$ be the set of all transition matrices in $\mathcal{T}_{Q_j}$ that are defined on the interval $[t_{i-1},t_i]$, for all $j\in\{1,\ldots,N'\}$.
...
It remains to show that $C_\epsilon$ generates an open $\epsilon$-cover of $\mathcal{T}_\rateset^{[t,s]}$. To this end, consider any $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$. We will show that there is some $\mathcal{S}\in C_{\epsilon}$ such that $d(\mathcal{T},\mathcal{S})<\epsilon$.

%Because $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$, there must be some $P\in\wmprocesses_\rateset$ such that $\mathcal{T}_P^{[t,s]}=\mathcal{T}$. Furthermore, because $P\in\wmprocesses_\rateset$, by Lemma REF there must be some $v\in\mathcal{U}_{[t,s]}$ with $\sigma(v)<\delta^*$ **** define $\delta^*$ and $\epsilon^*$ **** such that, writing $v=\tau_0,\ldots,\tau_m$, it holds for all $i\in\{0,\ldots,m-1\}$ that
%\begin{equation*}
%(\exists Q\in\rateset)\,\norm{T_{\tau_i}^{\tau_{i+1}} - (I+\Delta_{i+1}^vQ)} < \Delta_{i+1}^v\epsilon^*\,,
%\end{equation*}
%where the $\Delta_{i+1}^v$ are understood to be the differences taken with respect to $v$. Let now $u^*$ be the ordered union of $u$ and $v$. We will write $u^*=\tau_0^*,\ldots,\tau_k^*$, with differences $\Delta_i^*$. Then, by Lemma REF, it holds for all $i\in\{0,\ldots,k-1\}$ that
%\begin{equation*}
%(\exists Q\in\rateset)\,\norm{T_{\tau_i^*}^{\tau_{i+1}^*} - (I+\Delta_{i+1}^*Q)} < \Delta_{i+1}^*\epsilon^*\,.
%\end{equation*}
%
%**** {\bf TODO} finish this.
\end{proof}
...
We next give an upper bound on the remaining product term. Consider the exponential function $e^x$. It is well known that this function has the series representation
\begin{equation*}
e^x = \sum_{i=0}^\infty\frac{x^n}{n!} = 1 + x + \frac{x^2}{2!} + \frac{x^3}{3!} + \cdots
\end{equation*}
Hence, it follows that $(1+x)<e^x$ for all $x\in\realspos$. Therefore in particular, we find that $(1+\Delta_i\norm{\rateset})<e^{\Delta_i\norm{\rateset}}$ for all $i\in\{1,\ldots,n\}$. Hence,
\begin{align*}
...
For all $x\in\realsnonneg$ with $x<1$, it holds that
\begin{equation*}
e^x - (1 + x) < x^2\,.
\end{equation*}
\end{claim}
...
e^x - (1 + x) < x^2
\end{equation*}
holds for all $x\in(0,1.79328)$, up to some numerical error on the upper bound.
...
\begin{lemma}
Let $\rateset$ be any non-empty, bounded and convex set of rate matrices. Then, for all $P\in\wmprocesses_\rateset$, all $t\in\realsnonneg$, all $\Delta\in\realspos$, all $\epsilon\in\realspos$, and all $\delta\in\realspos$, there is some $u\in\mathcal{U}_{[t,t+\Delta]}$ with $\sigma(u)<\delta$, and some $Q\in\rateset$, such that
\begin{equation*}
\norm{T_{t}^{t+\Delta} - \prod_{i=1}^n(I+\Delta_iQ)} < \Delta\epsilon\,.
\end{equation*}
\end{lemma}
\begin{proof}

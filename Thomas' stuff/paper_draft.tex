...
\end{equation*}
resulting in a step size of $\Delta=\nicefrac{(s-t)}{n}=16\cdot 10^{-3}$. Note that this choice of $n$ here also satisfies the requirement $n\geq (s-t)\norm{\lrate}$ from Proposition~\ref{prop:approximation_error_bound}. 
...
The general method for computing conditional lower expectations of functions $f\in\gambles(\states_{u\cup v})$ is outlined in Algorithm~\ref{alg:compute_multivar}. **** this algorithm blabla
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Require{A non-empty, bounded, and convex set of rate matrices $\rateset$ with separately specified rows, two sets of time points $u,v\in\mathcal{U}_\emptyset$ such that $u\leq v$, a function $f\in\gambles(\states_{u\cup v})$, and a maximum numerical error per step of $\epsilon\in\realspos$.}
\Ensure{An approximation of the function $\underline{\mathbb{E}}_\rateset^\mathrm{W}[f(X_u,X_v)\,\vert\,X_u]$ in $\gambles(\states_u)$.}
	 \Statex
...
%		\Statex
		\For{$i\in\{m,m-1,\ldots,0\}$}	\Comment{Iterate backward over $v=s_0,\ldots,s_m$.}
			...
			\State $g_{(i-1)}\gets ${\tt ComputeLuf}($\rateset,u\cup w,s_i,g_i,\epsilon$)  \Comment{Compute $g_{(i-1)}\in\gambles(\states_{u\cup w})$.}
%			\Statex
		\EndFor
%		\Statex

...
*** lalala nu gaan we het over $L_t^s$ hebben ***

*** $L_t^s$ is een lower bound van verwachtingswaarden ***
...
Equation \eqref{eq:nonmarkov_bound_proof_deriv_bounded} now implies that for all $\tau\in\realspos$, all $v\in\mathcal{U}_{[0,\tau]}$, and all $(x_{\tau_0},\ldots,x_{\tau_{m-1}})\in\states^{\{\tau_0,\ldots,\tau_{m-1}\}}$, there is some $Q^v_{x_{\tau_0},\ldots,x_{\tau_{m-1}}}\in\rateset$ such that, for all $f\in\gambles(\states^{v\cup\{\tau+\Delta\}})$ and all $x_{\tau_m}\in\states^{\{\tau_m\}}$,
...
It follows from Equation \eqref{eq:nonmarkov_bound_proof_lbound_approx} and the fact that $\Delta<\delta'$, that
\begin{equation*}
...
*** lalala $L_t^s$ kan arbitrair dicht benaderd worden ***
...
*** lalala $L_t^s$ doet wat we willen ***
...
*** lalala met andere woorden, $L_t^s$ doet niet alleen wat we willen voor Markov ketens, zoals al vastgesteld in eerder werk van Damjan, maar het werkt ook voor niet-Markov ketens! ***

\section{Functions Defined on Multiple Time Points}

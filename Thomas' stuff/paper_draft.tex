...
It now follows from Lemma~\ref{lemma:bound_on_linear_approx_partition} that there is some $v\in\mathcal{U}_{[t,t+\Delta]}$ with $v=t_0,\ldots,t_n$, such that, for all $i\in\{1,\ldots,n\}$, there is some $Q_i\in\rateset$ such that
\begin{equation*}
...
\end{align*}
Furthermore, since we know that, for all $i\in\{1,\dots,n\}$, $\Delta_i\norm{Q_i}\leq\Delta\norm{\rateset}\leq 1$, we can apply Lemma~\ref{lemma:convex_rate_set_contains_approximation} to find that
\begin{equation*}
...
\end{equation*}
Then $Q\in\rateset$ because $\rateset$ is convex and

...
If $\max u=w_j$, then with $v\coloneqq\{w_0,\dots,w_j\}\setminus u$, we find that
\begin{align*}
...
=T_{\max u}^{w_{j+1}}(x_{\max u},y),
\end{align*}
where the first equality follows from~\ref{def:coh_prob_3}, the second equality follows from \ref{def:coh_prob_4}, the third equality follows from the assumptions in this lemma and the fact that $\max u=w_j$, and the fifth equality follows from \ref{def:coh_prob_1} and~\ref{def:coh_prob_3}. *** need to check this and maybe adapt the F's such that this is true ***

If $\max u<w_j$, then with $v\coloneqq\{w_0,\dots,w_{j-1}\}\setminus u$, we find that
\begin{align*}
...
=T_{\max u}^{w_{j+1}}(x_{\max u},y),
\end{align*}
*** need to check this and maybe adapt the F's such that this is true ***
\end{proof}

...
We will in this section formalize the notion of continuous-time stochastic processes. This is done in the framework of full conditional probabilities, which we describe in Section~\ref{sec:cond_prob}. This framework is then used in Section~\ref{sec:finite_add_stoch_proc} to define stochastic processes, and Section~\ref{sec:well_behaved} describes a specific class of stochastic processes, which we call well-behaved, on which we will largely focus throughout this work. Finally, Section~\ref{sec:dynamics} provides some tools with which we can describe the dynamics of stochastic processes.

\subsection{Full conditional probabilities}\label{sec:cond_prob}

A cadlag function $\omega$ from $\realsnonneg$ to $\states$ is called a \emph{path}, where cadlag means that it is right continuous for all $t\in\realsnonneg$ and that the left limit exists for all $t\in\realspos$. Let $\paths$ be the set of all paths. For any path $\path\in\paths$ and any time point $t\in\realsnonneg$, the value of $\path$ in $t$ is denoted by $\path(t)$.
...
\subsection{Finitely additive stochastic processes}\label{sec:finite_add_stoch_proc}

We will next use the notion of full conditional probabilities to define continuous-time stochastic processes. These stochastic processes will be defined on a specific subset of the set $\mathcal{P}(\Omega)$ of all events, as follows.

...
For the second statement, take any $P\in\processes$ such that $P\notin\wprocesses_{\rateset_{\lrate}}$. Then, $\smash{\overline{\partial}}T_{t,x_u}^{t}\nsubseteq\rateset_{\lrate}$, and hence either $\smash{\overline{\partial}}T_{t,x_u}^{t}=\emptyset$, or, there is some $Q\in\smash{\overline{\partial}}T_{t,x_u}^{t}$ such that $Q\notin\rateset_{\lrate}$. Assume the latter case for now, and assume that $Q\in\smash{\overline{\partial}}_+T_{t,x_u}^{t}$. Because $Q\notin\rateset_{\lrate}$, by Equation~\eqref{eq:dominatingratematrices} there must be some $f\in\gamblesX$ and some $x\in\states$ such that $\left[Qf\right](x)<\left[\lrate f\right](x)$. Furthermore, because $Q\in\smash{\overline{\partial}}_+T_{t,x_u}^{t}$, there is some $\{\Delta_i\}_{i\in\nats}\to0$ such that $\lim_{i\to+\infty}\nicefrac{1}{\Delta_i}(T_{t,x_u}^t - I)=Q$.

Now, assume \emph{ex absurdo} that the statement is false. Then, we must have that for all $\Delta>0$,
\begin{equation*}
\left[L_t^{t+\Delta}f\right](x) \leq \left[T_{t,x_u}^{t+\Delta}f\right](x)\,,
\end{equation*}
or equivalently, that
\begin{equation*}
0 \leq \left[\frac{1}{\Delta}\left(T_{t,x_u}^{t+\Delta} - I\right)f\right](x) - \left[\frac{1}{\Delta}\left(L_t^{t+\Delta} - I\right)f\right](x)\,.
\end{equation*}
Consider the earlier sequence $\{\Delta_i\}_{i\in\nats}$. Because this inequality holds for each $\Delta_i$ in the sequence, we must have that
\begin{align*}
0 &\leq \lim_{i\to+\infty}\left[\frac{1}{\Delta_i}\left(T_{t,x_u}^{t+\Delta_i} - I\right)f\right](x) - \lim_{i\to+\infty}\left[\frac{1}{\Delta}\left(L_t^{t+\Delta} - I\right)f\right](x) \\
 &= \left[Qf\right](x) - \left[\lrate f\right](x)\,,
\end{align*}

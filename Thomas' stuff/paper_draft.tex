...
\end{equation*}
Let $y^*$ be any element of $\states$ such that $\min_{y\in\states}\lambda_y=\lambda_{y^*}$ (since $\states$ is finite, this is always possible) and let $\omega^{**}$ be any path in $\Omega$ such that $\omega^{**}\in C^*$ and $\omega^{**}(w_m)=y^*$.

Let $S^{**}\coloneqq\{i\in\{1,\dots,n\}\setminus(S\cup S^*)$. Since $\omega^{**}\in C^*$, we find that $\ind{C_i}(w^{**})=\ind{C_i}(w^{*})$ and $\ind{A_i}(w^{**})=\ind{A_i}(w^{*})$ for all $i\in S$, that $\ind{C_i}(w^{**})=1$ for all $i\in S^{*}$, and that $\ind{C_i}(w^{**})=0$ for all $i\in S^{**}$. Hence, it follows from Equation~\eqref{eq:lemma:simplechaincoherence:supremumreached} that
\begin{equation*}

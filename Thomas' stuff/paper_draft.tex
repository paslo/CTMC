...
\begin{equation*}
\hat{f}(x_0)\coloneqq
\underline{\mathbb{E}}_{\rateset,\,\mathcal{M}}^\mathrm{W}[f(X_u)\,\vert\,X_0=x_0]=\left[L_{t_0}^{t_1}L_{t_1}^{t_2}\cdots L_{t_{n-1}}^{t_n}f\right](x_0),
%~\text{ for all $x_0\in\states$},
\end{equation*}
with $u=t_0,\ldots,t_n$ and, without loss of generality, $t_0=0$. Hence, the only part of the problem that is new here is the computation of $\underline{\mathbb{E}}_{\rateset,\,\mathcal{M}}^\mathrm{W}[\hat{f}(X_0)]$.

The first part consists in computing $\hat{f}$, which we already know how to do, , we need to c

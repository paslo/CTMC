...
In order to formalize our approach, we turn in this work to the field of \emph{imprecise probability}~\cite{Walley:1991vk,troffaes2013:lp,augustin2013:itip}. The basic premise of this field is that, whenever it is impossible or unrealistic to specify a single probabilistic model, say $P$, it is better to instead consider a \emph{set} of probabilistic models $\mathcal{P}$, and to then draw conclusions that are robust with respect to variations in this set. In our particular case, $\mathcal{P}$ will be a set of stochastic processes. Some of these processes are homogeneous continuous-time Markov chains. However, the majority of them are not. As explained in Example~\ref{ex:health_sick_exmp}, we also consider other, more general stochastic processes, which are not required to be time-homogeneous, do not need to satisfy the Markov condition, and do not even need to be differentiable.
...
So in order to recapitulate, this paper is about a specific type of robust generalisation of a continuous-time Markov chain, called imprecise continuous-time Markov chains.
...
The remainder of this paper is organized as follows. We start in Section~\ref{sec:prelim}, where we  introduce our notation and discuss some basic mathematical concepts that will be used throughout. Next, in Section~\ref{sec:systems}, we define some algebraic tools for representing the behaviour of stochastic processes. Section~\ref{sec:stochastic_processes} then goes on to formally introduce stochastic processes and provides some powerful tools for describing their dynamics.

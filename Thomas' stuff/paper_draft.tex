...
The remainder of this paper is organized as follows. In Section~\ref{sec:prelim}, we introduce notation and some basic concepts that will be used throughout. Section~\ref{sec:stochastic_processes} formally introduces stochastic processes, and provides tools for describing their dynamics. Section~\ref{sec:cont_time_markov_chains} focuses on the special case of continuous-time Markov chains. 
...
Linear maps from $\gamblesX$ to $\gamblesX$ will play an important role in this work, and will be represented by matrices. Because the state space $\states$ is fixed throughout, we will always consider square, $m\times m$, real-valued matrices. As such, we will for the sake of brevity simply refer to them as `matrices'. If $A$ is a matrix, we will index its elements as $A(x,y)$ for all $x,y\in\states$, where the indexing is understood to be row-major. Furthermore, $A(x,\cdot)$ will denote the $x$-th row of $A$, and $A(\cdot,y)$ will denote its $y$-th column. The symbol $I$ will be reserved throughout to refer to the $m\times m$ identity matrix.
...
As a special case, we consider finite sequences of time points that partition a given time interval $[t,s]$, with $t,s\in\realsnonneg$ such that $t\leq s$. Such a sequence is taken to be ordered, and includes the end-points of this interval. In this specific case, we do \emph{not} require the sequence to be non-degenerate. Thus, the sequence is of the form $t=t_0\leq t_1\leq\cdots\leq t_n=s$. We denote the set of all such sequences by $\mathcal{U}_{[t,s]}$. For any $u\in\mathcal{U}_{[t,s]}$ with $u=t_0,\ldots,t_n$, we define the sequential differences $\Delta_i^u\coloneqq t_i-t_{i-1}$, for all $i\in\{1,\ldots,n\}$. We then use $\sigma(u)\coloneqq \max\{\Delta_i^u:i\in\{1,\ldots,n\}\}$ to denote the maximum such difference.

Finally, as some notes on notational conventions, we typically use $t,r,s\in\realsnonneg$ to denote time points, $u,v,w\in\mathcal{U}$ to denote sequences of time points, and $x,y,z\in\states$ to denote state assignments. We use the notation $\ind{C}$ for the indicator function on a subset $C$ of any set $A$, meaning that, for all $a\in A$, $\ind{C}(a)=1$ if $a\in C\subseteq A$, and $\ind{C}(a)=0$, otherwise. If $C$ is a singleton $C=\{c\}$, we may instead write $\ind{c}\coloneqq\ind{\{c\}}$.
...
%******
%
%The sets $\mathcal{U}$ and $\mathcal{U}_{<t}$ (and so on) can no longer contain non-empty degenerate sequences (all time points must be unique). The exception is $\mathcal{U}_{[t,s]}$, which can contain degenerate sequences.
%
%If you're aware of any issues this might cause, please let me know. (I checked, but couldnt find any).
%
%******
...
\end{equation*}
be the maximum norm. For any operator $A$ from $\gamblesX$ to $\gamblesX$ that is non-negatively homogeneous, meaning that $A(\lambda f)=\lambda A(f)$ for all $f\in\gamblesX$ and all $\lambda\in\realsnonneg$, we consider the induced operator norm
\begin{equation*}
...
\subsection{Notation for the Time-Indexed State Space}\label{sec:multivar_notation}

We will also find it convenient to have notation for functions defined on the state space at (multiple) explicit time points. To this end, for any finite sequence of time points $u\in\mathcal{U}$ such that $u=t_0,\ldots,t_n$, we let
\begin{equation*}
\states^u \coloneqq \prod_{i=0}^n \states
\end{equation*}
denote the joint state space at the time points in $u$. Note that if $u$ only contains a single time point $t$, then we simply have that $\states^u=\states^{\{t\}}=\states$. In that case, all our previous conventions apply, but the explicit use of the time point is at times convenient. The corresponding joint state assignment will be written as $x_u\in\states^u$, with which we then mean the state assignment $(x_{t_0},\ldots,x_{t_n})\in\states^{\{t_0\}}\times\cdots\times\states^{\{t_n\}}$. If $u=\emptyset$, then the notation $x_\emptyset\in\states^\emptyset$ may be used as a ``dummy'' placeholder, typically leading to statements that are vacuously true.

For any $u\in\mathcal{U}_\emptyset$, we use $\gambles(\states^u)$ to denote the set of real-valued functions on $\states^u$. For any $f\in\gambles(\states^u)$, let the norm $\norm{f}$ be defined as
\begin{equation*}
\norm{f} \coloneqq \norm{f}_{\infty} \coloneqq \max\{\abs{f(x_u)}\,:\,x_u\in\states^u\}\,.
\end{equation*}

For a function $f\in\gambles(\states^{u\cup\{s\}})$, we will write $f(x_u,X_s)$ for the restriction of $f$ to $\states^{\{s\}}$ specific to the joint state assignment $x_u\in\states^u$. Thus, $f(x_u,X_s)$ corresponds to a $g\in\gambles(\states^{\{s\}})$ such that $g(x_s) = f(x_u,x_s)$ for all $x_s\in\states^{\{s\}}$.
...
\emph{Transition matrices} are a convenient tool in describing both discrete-time and continuous-time Markov chains. A transition matrix $T$ is simply a matrix that is row-stochastic, meaning that, for each $x\in\states$, the row $T(x,\cdot)$ is a probability mass function on $\states$.
...
% \begin{proposition}\label{prop:normoftransisone}
...
We will in Section~\ref{sec:stochastic_processes} define continuous-time stochastic processes using the framework of full conditional probabilities {\bf CITE}, which we describe here. 
...
Therefore, we next introduce the notion of \emph{coherent conditional probabilities}~{\bf CITE}. This notion was developed from DeFinetti's work on coherent (unconditional) probabilities~{\bf CITE}. We refer to {\bf CITE CITE CITE} for philosophical discussions on its merits, but will give some interpretation below. From a practical point of view, suffice it to say that the following definition provides us with a convenient alternative characterization of properties~\ref{def:coh_prob_2}-\ref{def:coh_prob_6} above.
...
In particular, for each event $(A,C)\in\mathcal{C}$ a bettor will specify a price $P(A\vert C)$ for which she is willing to both buy and sell the gamble---with a reward of one currency unit---that event $A$ will come true, given that event $C$ comes true. These gambles are then bought and sold under the proviso that, should event $C$ not happen, then all parties involved are refunded. 

Suppose now that the outcome is $\omega$. For each ticket the bettor bought, she will receive one unit of currency if event $A$ came true. Her profit is then $(\ind{A}(\omega) - P(A\vert C))$, per ticket. However, she only receives this profit if event $C$ also came to pass. Hence, her actual profit is $\ind{C}(\omega)(\ind{A}(\omega) - P(A\vert C))$. Allowing for fractional tickets, if she has bought a total of $\lambda\in\realsnonneg$ such tickets, her total profit is $\lambda \ind{C}(\omega)(\ind{A}(\omega) - P(A\vert C))$.

Similarly, for each such ticket that she sold, she has received $P(A\vert C)$ amount of currency in advance, and must pay $\ind{A}(\omega)$ to its buyer. Because this is conditional on event $C$ coming true, her profit after paying is $\ind{C}(\omega)(P(A\vert C) - \ind{A}(\omega))$, per ticket sold. Comparing to the above, it follows that her profit from buying is equal to her loss from selling, and vice versa.

Now, because the bettor quoted prices $P(A\vert C)$ for each $(A,C)\in\mathcal{C}$, we also have to consider combinations of gambles. Specifically, after quoting her prices, the bettor can be forced to buy and/or sell any number of gambles in any combination. In particular, for any $n\in\nats$ and any $i\in\{1,\ldots,n\}$, she can be forced to buy $\lambda_i\in\reals$ gambles (which is equivalent to selling, if $\lambda_i$ is negative) on any event $(A_i,C_i)\in\mathcal{C}$. After being forced into this combined gamble, her total loss in case of the outcome $\omega$ is then
\begin{equation*}
...
Her prices $P$ are then said to be \emph{coherent}, as in the definition above, if regardless of this choice of gambles, there exists an outcome $\omega$ in which at least one of the conditioning events $C_i$ came true, and for which she does not make a loss. Note that if none of these events $C_i$ came true, i.e. if $\omega\notin \cup_{i=1}^n C_i$, then she would trivially not make a loss because she would get refunded on all the gambles.

The well-known result from coherent probability theory is then that her prices $P$ are coherent if and only if they satisfy the axioms of probability, i.e., if $P$ satisfies properties~\ref{def:coh_prob_2}-\ref{def:coh_prob_6} above.
...
\colon t> u)~~
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t+\Delta}=y\vert X_t=x, X_u=x_u)-\delta_{xy}}<+\infty
\end{equation}
and
\begin{equation}\label{eq:def:well-behaved:left}
(\forall t\in\reals_{>0}
\colon t> u)~~
\limsup_{\Delta\to 0^{+}}\frac{1}{\Delta}\abs{P(X_{t}=y\vert X_{t-\Delta}=x, X_u=x_u)-\delta_{xy}}<+\infty.
\end{equation}
...
\begin{equation*}
\frac{1}{\Delta}\abs{P(X_{t+\Delta}=x\,\vert\,X_{t}=x,X_u=x_u) - \delta_{xx}} \to +\infty\,,\quad\text{as}\quad \Delta\to0^+\,,
\end{equation*}
...
**** reword the below, since this is intentional
...
A stochastic process $P\in\processes$ satisfies the \emph{Markov property} if, for any time sequence $u\in\mathcal{U}$, any $x_u\in\states^u$, any $t,s\in\realsnonneg$ such that $u< t\leq s$, and any states $x,y\in\states$:
\begin{equation*}
...
For any interval $\mathbf{I}\subseteq\realsnonneg$, a family $\mathcal{T}^{\mathbf{I}}$ is a restricted transition matrix system defined on $\mathbf{I}$ if and only if it is a family of transition matrices $T_t^s$ defined for all $t,s\in\mathbf{I}$ with $t\leq s$, and, for all $t,r,s\in\mathbf{I}$ such that $t\leq r\leq s$, it holds that $T_t^s = T_t^rT_r^s$, and $T_t^t=I$.
\end{proposition}
\begin{proof}
Trivial consequence of Definition~\ref{def:trans_mat_system}.
\end{proof}

We will henceforth adopt the same terminology for restricted transition matrix systems that we use for (normal) transition matrix systems. In particular, we will call $\mathcal{T}^{\mathbf{I}}$ well-behaved if it satisfies Equation~\eqref{eq:wellbehavedtransitionmatrixsystem} on the interval $\mathbf{I}$.
...
\end{proposition}
Note that this result includes as a special case completeness of the set $\mathbb{T}^{\realsnonneg}$ of all (unrestricted) transition matrix systems. We will later return to this result in Section~\ref{sec:iCTMC}.
...
Consider two adjacent intervals $\mathbf{I},\mathbf{J}\subseteq\realsnonneg$, and any two restricted transition matrix systems $\mathcal{T}^{\mathbf{I}}$ and $\mathcal{T}^{\mathbf{J}}$. Then,
\begin{equation*}
\mathcal{T}^{\mathbf{I}\cup \mathbf{J}} \coloneqq \mathcal{T}^{\mathbf{I}}\otimes \mathcal{T}^{\mathbf{J}}
\end{equation*}
is a restricted transition matrix system defined on $\mathbf{I}\cup\mathbf{J}$. Furthermore, if both $\mathcal{T}^{\mathbf{I}}$ and $\mathcal{T}^{\mathbf{J}}$ are well behaved, then $\mathcal{T}^{\mathbf{I}\cup\mathbf{J}}$ is also well-behaved.
\end{proposition}

We now have the required tools for the construction of the non-homogeneous Markov chains in which we will be mostly interested. In particular, if for any rate matrix $Q\in\mathcal{R}$ we now let $\mathcal{T}_Q^{\mathbf{I}}$ be the restriction of $\mathcal{T}_Q$, defined as in Definition~\ref{def:systemfromQ}, we have the following result.
...
\noindent It is well known {\bf CITE} that the corresponding \emph{upper expectation},
\begin{equation*}
\overline{\mathbb{E}}[\cdot\,\vert\,\cdot]\coloneqq \sup\{\mathbb{E}[\cdot\,\vert\,\cdot]\,:\,P\in\mathcal{P}\}\,,
\end{equation*}
can always be defined through the conjugacy relation $\overline{\mathbb{E}}[\cdot\,\vert\,\cdot]=-\underline{\mathbb{E}}[-\cdot\,\vert\,\cdot]$. Therefore, we will in the remainder of this work focus only on the lower expectation with respect to some set $\mathcal{P}$.
...
\end{equation*}
This first equality follows from the Markov property of the elements in $\wmprocesses$, ensuring that $\smash{\overline{\partial}}T_{t,x_u}^t=\smash{\overline{\partial}}T_t^t$ for all $u\in\mathcal{U}_{<t}$ and all $x_u\in\states^u$. The second equality follows from the Markov property and the homogeneity of the elements of $P\in\whmprocesses$, ensuring furthermore that $\smash{\overline{\partial}}T_t^t=\{Q_P\}$ for all $t\in\realsnonneg$.
...
We have already seen in Section~\ref{sec:cont_time_markov_chains} that the space $\mathbb{T}^{[t,s]}$ of \emph{all} restricted transition matrix systems on $[t,s]$ is complete. We will now look at the subspace of $\mathbb{T}^{[t,s]}$ corresponding to all $P\in\wmprocesses_\rateset$. To this end, for any non-empty bounded set of rate matrices $\rateset$ and any $t,s\in\realsnonneg$ such that $t\leq s$, let
\begin{equation*}
...
Consider any $t,s\in\realsnonneg$ with $t\leq s$, any $\delta\in\realspos$ such that $\delta\leq\nicefrac{1}{\norm{\lrate}}$, and any $u,u^*\in\mathcal{U}_{[t,s]}$ such that $\sigma(u)<\delta$ and $\sigma(u^*)<\delta$. Let $C\coloneqq s-t$. Then,
\begin{equation*}
...
For any $t,s\in\realsnonneg$ such that $t\leq s$ and any lower transition rate operator $\lrate$, there is a lower transition operator $\lt\in\underline{\mathcal{T}}$ such that 
\begin{equation*}
(\forall\epsilon>0)\,
(\exists\delta>0)\,
...
\begin{equation*}
\frac{\partial}{\partial t}\lbound_t^s=-\lrate\lbound_t^s\,,\quad\text{and,}\quad\frac{\partial}{\partial s}\lbound_t^s=\lrate\lbound_t^s\,.
\end{equation*}
...
\end{equation*}
Indeed, it is easily verified that any rate matrix $Q\in\mathcal{R}$ will satisfy properties~\ref{LR:constantzero}-\ref{LR:homo} from Definition~\ref{def:coh_low_trans_rate}, and hence any such $Q$ is in fact a lower transition rate operator. It follows from the above that the lower transition operator $L_t^s$ corresponding to this $\lrate=Q$ will satisfy $L_t^s=e^{Q(s-t)}$. 
...
Let $\rateset$ be an arbitrary non-empty bounded set of rate matrices with corresponding lower transition rate operator $\lrate$, and let $\underline{\mathcal{T}}_{\lrate}$ be the family of lower transition operators corresponding to $\lrate$. Then, for any $P\in\wprocesses_\rateset$, any $t,s\in\realsnonneg$ such that $t\leq s$, any $u\in\mathcal{U}_{<t}$, any $x\in\states$ and $x_u\in\states^u$, and any $f\in\gamblesX$,
\begin{equation*}
...
Let $\rateset$ be an arbitrary non-empty bounded set of rate matrices that has separately specified rows, with corresponding lower transition rate operator $\lrate$, and let $\underline{\mathcal{T}}_{\lrate}$ be the family of lower transition operators corresponding to $\lrate$. Then for all $t,s\in\realsnonneg$ such that $t\leq s$, all $f\in\gamblesX$, and all $\epsilon\in\realspos$, there is a $P\in\wmprocesses_{\rateset}$ such that
\begin{equation*}
...
Let $\rateset$ be an arbitrary non-empty bounded set of rate matrices that has separately specified rows, with corresponding lower transition rate operator $\lrate$, and let $\underline{\mathcal{T}}_{\lrate}$ be the family of lower transition operators corresponding to $\lrate$. Then, for all $t,s\in\realsnonneg$ such that $t\leq s$, all $u\in\mathcal{U}_{<t}$, all $x\in\states$ and $x_u\in\states^u$, and all $f\in\gamblesX$,
\begin{align*}
...
Let $\lrate$ be an arbitrary lower transition rate operator, with $\rateset_{\lrate}$ its set of dominating rate matrices, and let  $\underline{\mathcal{T}}_{\lrate}$ be the family of lower transition operators corresponding to $\lrate$. Then, the set $\wprocesses_{\rateset_{\lrate}}$ is the largest set of stochastic processes for which, for all $t,s\in\realsnonneg$ such that $t\leq s$, all $u\in\mathcal{U}_{<t}$, all $x\in\states$ and $x_u\in\states^u$, and all $f\in\gamblesX$,
\begin{equation*}
...
Let $\rateset$ be an arbitrary non-empty bounded set of rate matrices that has separately specified rows, with corresponding lower transition rate operator $\lrate$, and let $\underline{\mathcal{T}}_{\lrate}$ be the family of lower transition operators corresponding to $\lrate$. Then, for all $s\in\realsnonneg$, all $u\in\mathcal{U}_{<s}$ such that $u\neq\emptyset$, all $x_u\in\states^u$, and all $f\in\gambles(\states^{u\cup\{s\}})$,
\begin{equation*}
...
\end{equation*}
\ref{def:T:nonneg} follows from~\ref{def:Q:nonnegoffdiagonal} and because $0\leq \Delta \leq \nicefrac{1}{\norm{Q}}$: for all $x,y\in\states$ such that $x\neq y$, $0\leq \Delta \leq \nicefrac{1}{\norm{Q}}$ implies that $[I+\Delta Q](x,x)=1+\Delta Q(x,x)\geq 1-\Delta\norm{Q}\geq0$, and \ref{def:Q:nonnegoffdiagonal} and $\Delta\geq0$ implies that $[I+\Delta Q](x,y)=\Delta Q(x,y)\geq0$.
\end{proof}
...
Clearly, $\mathcal{T}^{\mathbf{I}\cup\mathbf{J}}$ is a family of transition matrices, and in particular, for all $t,s\in\mathbf{I}\cup\mathbf{J}$ such that $t\leq s$, there is a transition matrix $T_t^s$ corresponding to $\mathcal{T}^{\mathbf{I}\cup\mathbf{J}}$. Furthermore, for all $t\in\mathbf{I}\cup\mathbf{J}$, we have that either $t\in\mathbf{I}$ or $t\in\mathbf{J}$. In either case, we have that $T_t^t=I$, because either $T_t^t=\presuper{i}T_t^t=I$ or $T_t^t=\presuper{j}T_t^t=I$, with $\presuper{i}T_t^t$ and $\presuper{j}T_t^t$ corresponding to $\mathcal{T}^{\mathbf{I}}$ and $\mathcal{T}^{\mathbf{J}}$, respectively. It remains to show that for all $t,q,s\in\mathbf{I}\cup\mathbf{J}$ with $t\leq q\leq s$, it holds that
\begin{equation*}
T_t^s = T_t^qT_q^s\,.
\end{equation*}
If both $t,s\in\mathbf{I}$ or if both $t,s\in\mathbf{J}$, this clearly holds. Therefore, suppose that $t\in\mathbf{I}$ and $s\in\mathbf{J}$. Suppose furthermore that $q\in\mathbf{I}$. Then, from the definition of the concatenation operator, we have that $T_q^s=T_q^rT_r^s$, with $r=\max\mathbf{I}=\min\mathbf{J}$. Because $t,q,r\in\mathbf{I}$, we know that $T_t^qT_q^r=T_t^r$, and hence, by the definition of the concatenation operator,
\begin{equation*}
T_t^qT_q^s = T_t^qT_q^rT_r^s = T_t^rT_r^s = T_t^s\,.
\end{equation*}
An exactly analogous argument proves the case for $q\in\mathbf{J}$. Therefore, and using Proposition~\ref{prop:restr_trans_mat_system_if_semigroup}, we conclude that $\mathcal{T}^{\mathbf{I}\cup\mathbf{J}}$ is a restricted transition matrix system.
...
Consider any $P\in\wprocesses_\rateset$, any $0\leq t<s$ and any $x_u$. Then for all $\epsilon>0$ and $\delta>0$, there is some $v\in\mathcal{U}_{[t,s]}$ such that $\sigma(v)<\delta$ and, for all $i\in\{0,\dots,n-1\}$:
\begin{equation*}
(\exists Q\in\rateset)
~
\norm{
...
\begin{equation*}
\norm{\prod_{i=1}^n(I+\Delta_iQ_i) - (I+\sum_{i=1}^n\Delta_iQ_i)} < \Delta^2\norm{\rateset}^2.
\end{equation*}
\end{lemma}
\begin{proof}
...
\norm{\prod_{i=1}^n(I+\Delta_iQ_i)-(I+\Delta Q)}=
\norm{\prod_{i=1}^n(I+\Delta_iQ_i) - (I+\sum_{i=1}^n\Delta_i Q_i)} < \Delta^2\norm{\rateset}^2.
\end{equation*}
...
Throughout this proof, for any $q,r\in[t,s]$ such that $q\leq r$, we will use $\presuper{*}T_q^r$ to denote the transition matrix that corresponds to $\mathcal{T}_*^{[t,s]}$ and, for all $i\in\nats$, we will use $\presuper{i}T_q^r$ to denote the transition matrix that corresponds to $\mathcal{T}_i^{[t,s]}$. 

Consider any $r\in[t,s]$ and fix $\Delta>0$. Then due to Lemma~\ref{lemma:uniformelywellbehaved}, for all $i\in\nats$:
\begin{equation*}
...
So fix any $Q_*\in\smash{\overline{\partial}_{+}}\hspace{-3pt}\presuper{*}T_r^r$. We will show that $Q_*\in\rateset$. If $r<t$ or $s\leq r$, this holds trivially because in that case, we know that $\smash{\overline{\partial}}_+\presuper{*}T_r^r=\{Q\}\subseteq\rateset$ *** REF TO PROP ***. Therefore, without loss of generality, we may assume $r\in[t,s)$. 
...
\end{corollary}
\begin{proof}
Let $\rateset'$ denote the convex hull of $\rateset$. Then, clearly, $\rateset'$ is a non-empty, bounded, and convex set of rate matrices. Furthermore, since $\rateset\subseteq\rateset'$, we know that $\mathbb{T}_\rateset^{[t,s]}\subseteq \mathbb{T}_{\rateset'}^{[t,s]}$. Because any subspace of a totally bounded metric space is itself totally bounded *** {\bf CITE} ***, the result now follows trivially from Lemma~\ref{lemma:restricted_trans_mat_system_totally_bounded_if_Q_bounded_convex}.
...
Because this holds for any $i\in\{1,\ldots,n\}$, this implies the existence of some finite sequence $w\in\mathcal{U}$, with $w=s_0< s_1<\ldots<s_m$, such that $w_i\subseteq w$ for all $i\in\{1,\ldots,n\}$.
...
\end{align*}
Here, in the second inequality, we applied Proposition~\ref{lemma:compositioncoherence} and property~\ref{LT:differencenorm}. In the third inequality, we used property~\ref{LT:norm_at_most_one}. In the final inequality, we used the induction hypothesis.
\end{proof}

\begin{lemma}\label{lemma:justthelinearpart}
Consider a lower transition rate operator $\lrate$ and, for all $i\in\{1,\ldots,n\}$, some $0\leq\Delta_i\leq\nicefrac{1}{\norm{\lrate}}$ if $\norm{\lrate}\neq 0$, or any $\Delta_i\in\realsnonneg$ otherwise. Let $\Delta\coloneqq\sum_{i=1}^n\Delta_i$. Then
\begin{equation*}
...
For all $i\in\{2,\dots,n\}$, since $\Delta_i\leq\nicefrac{1}{\norm{\lrate}}$, it follows from Proposition~\ref{lemma:normQsmallenough} that $(I+\Delta_i\lrate)$ and $I$ are lower transition operators. Therefore,
\begin{multline*}
...
For any $k\in\{1,\dots,n\}$, consider a sequence $\Delta_{k,i}\geq 0$, $i\in\{1,\dots,n_k\}$, and let $\Delta_k\coloneqq\sum_{i=1}^{n_k}\Delta_{k,i}$. Let $C\coloneqq\sum_{k=1}^n\Delta_k$ and let $\delta\coloneqq\max_{k=1}^n\Delta_k$. Then for any lower transition rate operator $\lrate$ such that either $\norm{\lrate}=0$ or $\delta\leq\nicefrac{1}{\norm{\lrate}}$:
\begin{equation*}
...
\begin{proof}
To avoid trivialities, we will assume that $\norm{\lrate}\neq 0$.
Fix any $k\in\{1,\dots,n\}$. Then for all $i\in\{1,\dots,n_k\}$, since $\Delta_{k,i}\leq\Delta_k\leq\delta\leq\nicefrac{1}{\norm{\lrate}}$, it follows from Propositions~\ref{lemma:normQsmallenough} and~\ref{lemma:compositioncoherence} that $\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)$ and $(I+\Delta_k\lrate)$ are lower transition operators. Therefore,
\begin{align*}
\norm{\prod_{k=1}^n\left(\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)\right)
-
\prod_{k=1}^n(I+\Delta_k\lrate)
...
This implies that, for all $k\in\{1,\dots,n\}$, there is some sequence $\Delta_{k,i}\geq 0$, $i\in\{1,\dots,n_k\}$, such that $\Delta_k=\sum_{i=1}^{n_k}\Delta_{k,i}$ and
\begin{equation*}
...
Similarly, for all $k\in\{1,\dots,n^*\}$, there is some sequence $\Delta^*_{k,i}\geq 0$, $i\in\{1,\dots,n^*_k\}$, such that $\Delta^*_k=\sum_{i=1}^{n^*_k}\Delta^*_{k,i}$ and
\begin{equation*}
...
Since $\lim_{i\to\infty}\sigma(u_i)=0$, and because of Propositions~\ref{lemma:normQsmallenough} and~\ref{lemma:compositioncoherence}, there is some index $n$ such that the sequence $\Phi_{u_n},\Phi_{u_{n+1}},\ldots$ consists of lower transition operators. Due to Corollary~\ref{corol:cauchy}, this sequence is Cauchy and therefore, because of Proposition~\ref{lemma:completemetricspace}, this sequence has a limit that is also a lower transition operator. Since the limit starting from $n$ and the limit starting from $1$ are identical (initial elements do not influence the limit), we find that the sequence $\left\{\Phi_{u_i}\right\}_{i\in\nats}$ has a limit, and that this limit is a lower transition operator.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{theo:convergencelowerbound}]
To avoid trivialities, we will assume that $t\neq s$ and $\norm{\lrate}\neq 0$.
...
Consider now any $\epsilon>0$ and let $C\coloneqq s-t$ and
\begin{equation*}
...
We will prove that $\frac{\partial}{\partial t}L_t^s=-\lrate L_t^s$ and $\frac{\partial}{\partial s}L_t^s=\lrate L_t^s$, meaning that
...
To prove Equation~\eqref{eq:lower_deriv_forward}, note that for any $\Delta\neq 0$, it holds that
\begin{align*}
\norm{\frac{L_{t}^{s+\Delta}-L_t^s}{\Delta}-\lrate\lbound_t^s} &= \norm{\frac{L_{t}^{s+\Delta}-L_t^s}{-\Delta}+\lrate\lbound_t^s} = \norm{\frac{L_{t-\Delta}^{s}-L_t^s}{-\Delta}+\lrate\lbound_t^s}\,,
\end{align*}
where the second equality used Proposition~\ref{prop:lower_transition_is_homogeneous}. Therefore, the proof of Equation~\eqref{eq:lower_deriv_forward} follows immediately from Equation~\eqref{eq:lower_deriv_backward}, by simply flipping the sign of $\Delta$.

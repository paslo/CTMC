...
In this work, we formalised the concept of \emph{imprecise continuous-time Markov chains}; a generalisation of continuous-time Markov chains that robustifies this model class with respect to often-made simplifying assumptions. We derived tractable algorithms to compute lower expectations for such models, which work for arbitrary functions that depend on the state at any finite number of time points. In particular, we have seen that these algorithms are most generally applicable when we relax the often-made assumptions of time-homogeneity and Markovian probabilistic independence. One of our most important conclusions, therefore, is that dropping these assumptions in this context actually makes it \emph{easier} to compute the quantities that we are interested in---rather than harder, which is what one might intuitively expect.

In the remainder of this section, we briefly summarise the most important technical results and concepts that we provided in this work. We then finally close with some pointers to future lines of research.

...
Fix any $\epsilon\in\realspos$, and define $C\coloneqq (s-t)$. We will start by constructing $C_\epsilon$. First, choose any $\epsilon'\in\realspos$ such that
\begin{equation*}
\epsilon' < \frac{\epsilon}{3C}\,.
\end{equation*}
...
\norm{T_{t_{i-1}}^r - S_{t_{i-1}}^r} < \frac{\Delta_i\epsilon}{3C} + \frac{\epsilon}{6}\,.
\end{equation}
...
**** Deze laatste case pakt eigenlijk tegelijk links-rechts-midden. beter even versimpelen nog
...
\norm{T_q^r - S_q^r} &= \norm{T_q^{t_i}\left(\prod_{k=i+1}^{j-1}T_{t_{k-1}}^{t_k}\right)T_{t_{j-1}}^r - S_q^{t_i}\left(\prod_{k=i+1}^{j-1}S_{t_{k-1}}^{t_k}\right)S_{t_{j-1}}^r} \\
 &< \quad \frac{\Delta_i\epsilon}{3C} + \frac{\epsilon}{6} \\
 &\quad + \sum_{k=i+1}^{j-1}\cdot\frac{\Delta_k\epsilon}{3C} + \Delta_k\epsilon' \\
 &\quad + \frac{\Delta_{j}\epsilon}{3C} + \frac{\epsilon}{6} \\
...
\end{equation*}
with $T_q^r\in\mathcal{T}$ and $S_q^r\in\mathcal{S}$. Because this holds for all $q$ and $r$, it follows that *** need to replace $\epsilon$ with some $\epsilon^*<\epsilon$ throughout to get it to be an open cover. do we need this? ***
\begin{equation*}
d(\mathcal{T},\mathcal{S}) = \sup\{\norm{T_q^r - S_q^r}\,:\,q,r\in[t,s], q\leq r\} \leq \epsilon\,.
\end{equation*}
Because this holds for any $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$, it follows that $C_\epsilon$ is a finite, open $\epsilon$-cover of $\mathcal{T}_\rateset^{[t,s]}$. Because the $\epsilon\in\realspos$ was arbitrary, we conclude that $\mathcal{T}_\rateset^{[t,s]}$ is totally bounded.
\end{proof}

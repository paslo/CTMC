...
\end{align*}
Therefore, computing the term $\smash{\underline{\mathbb{E}}_{\rateset,\,\mathcal{M}}^{\mathrm{WHM}}[f(X_t)\,\vert\,X_0]}$ is at its core a non-linear, constrained optimization problem over the set $\rateset$---the non-linearity stemming from the term $e^{Qt}$, and the specific form of the constraints depending on the choice of $\rateset$. While numerically solving this optimization problem is tractable for sufficiently ``nice'' $\rateset$ and in low dimensions, the computational complexity will in general quickly explode as the size of $\states$ increases.

...
\end{array}\right]\,,
\end{equation*}
with the unit-time being one week. That is, the person on average becomes sick once every 52 weeks, and becomes better after one week on average.
...
We will now focus on how to compute lower expectations with respect to imprecise continuous-time Markov chains induced by the set $\rateset$ from Example {\bf REF}. To this end, consider the lower transition rate operator $\lrate$ corresponding to this $\rateset$. It should be clear that, for any function $f\in\gamblesX$, computing the quantity $\lrate f$ is relatively easy.
...
\begin{equation*}
\norm{L_t^sf - \prod_{i=1}^n(I + \Delta\lrate)f} < \epsilon\,.
\end{equation*}
\end{proposition}

Simply put, this result tells us that if we can compute the quantity $\lrate f$, we can always approximate the quantity $L_t^sf$ to an arbitrary precision. Due to the correspondence between $L_t^sf$ and lower expectations $\underline{\mathbb{E}}_\rateset^\mathrm{WM}[f(X_s)\vert X_t]$ and $\underline{\mathbb{E}}_\rateset^\mathrm{W}[f(X_s)\vert X_t]$, this also tells us how to compute---or at least approximate to arbitrary precision---these latter quantities. The following provides a worked numerical example.

\begin{exmp}
{\bf TODO} numeriek voorbeeld voor simpele $f\in\gamblesX$.
\exampleend
\end{exmp}

\begin{exmp}
{\bf TODO} numeriek voorbeeld voor complexere $f\in\gambles(\states^{u\cup v})$, voor de set $\wprocesses_\rateset$.
\exampleend
\end{exmp}
...
We have that
\begin{align*}
\Delta &= \frac{(s-t)}{n} < (s-t)\frac{\epsilon}{(s-t)^2\norm{\lrate}^2\norm{f}}  = \frac{\epsilon}{(s-t)\norm{\lrate}^2\norm{f}}.
\end{align*}

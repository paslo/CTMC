...
Therefore, $\lrate$ is a map from $\gamblesX$ to $\gamblesX$. It is a matter of straightforward verification to see that $\lrate$ is a lower transition operator.
...
\noindent
Because of this result, it makes sense to call $\lrate$, as defined by Equation~\eqref{eq:correspondinglowertrans}, the \emph{lower transition operator that corresponds to $\rateset$}. %As we have just seen, every non-empty bounded set $\rateset\subseteq\mathcal{R}$ of rate matrices has such a corresponding lower transition rate operator $\lrate$. 
However, this correspondence is not one-to-one. As the following example establishes, multiple non-empty bounded sets of rate matrices may have the same corresponding lower transition rate operator.
...
Conversely, with any lower transition rate operator $\lrate$, we can associate the following set of dominating rate matrices:
...
\right\}
\end{equation}
As we will show in proposition~\ref{prop:dominatingproperties} below, this set of dominating rate matrices satisfies some very nice properties: it is non-empty, bounded, closed and convex, and it has \emph{separately specified rows}.
...
$\rateset$ needs to have tree additional properties: it must to be \emph{bounded} and \emph{convex}, and it must have \emph{separately specified rows}. The first two properties require no further explanation

\begin{corollary}
Consider a closed, bounded and convex set $\rateset$ of rate matrices that has separately specified rows. If $\lrate_\rateset$
\end{corollary}

...
For all $k\in\{1,\dots,m-1\}$, we now let $s_{2k}\coloneqq q_k$ and, for all $k\in\{1,\dots,m\}$, we let $s_{2k-1}\coloneqq r_k$. For the resulting sequence $s_1<s_2<\dots<s_{2m-2}<s_{2m-1}$, it then follows from Equation~\eqref{eq:epsilonboundsforqk} that, for all $i\in\{1,\dots,2m-2\}$:
\begin{equation*}
(\exists Q\in\rateset)
~
\norm{
...
If $s_1\neq t$, we also define $s_0\coloneqq t$ and $\Delta_1\coloneqq s_1-s_0=r_1-t>0$. We then have that $t<s_1=r_1$ and, since $C^*$ is a minimal cover and because of Equation~\eqref{eq:orderingofbounds}, it also follows that $r_1-\delta_{r_1}<t$

*** add argument with norm ***

If $s_{2m-1}\neq s$, we also define $s_{2m}\coloneqq s$. *** add argument with norm ***

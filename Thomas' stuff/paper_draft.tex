...
Because these restricted transition matrix systems are only defined on some interval $[t,s]$, it will be useful to define a product operator between two such systems $\mathcal{T}^{[t,r]}$ and $\mathcal{T}^{[r,s]}$. To his end, let
\begin{equation*}
\mathcal{T}^{[t,r]} \otimes \mathcal{T}^{[r,s]} \coloneqq \mathcal{T}^{[t,r]} \cup \mathcal{T}^{[r,s]} \cup \left\{T_\tau^rT_r^{\tau'}\,\Big\vert\,T_{\tau}^r\in\mathcal{T}^{[t,r]},\,T_r^{\tau'}\in\mathcal{T}^{[r,s]},\,\forall\tau\in[t,r]\forall\tau'\in[r,s]\right\}\,.
\end{equation*}
\begin{proposition}
...
\end{equation*}
is a restricted transition matrix system defined on $[t,s]$.
\end{proposition}
\begin{proof}
*** TODO
\end{proof}
...
\end{equation*}
We next endow this space $\mathbb{T}_\rateset^{[t,s]}$ with the following metric $d$,  defined for all $\mathcal{T}^{[t,s]},\mathcal{S}^{[t,s]}\in\mathbb{T}_\rateset^{[t,s]}$ as
...
\end{lemma}
\begin{proof}
We need to show that for any Cauchy sequence $\left\{\mathcal{T}_i\right\}_{i\in\nats}$ in $\mathbb{T}_\rateset^{[t,s]}$, it holds that $\mathcal{T}_*\coloneqq\lim_{i\to\infty}\mathcal{T}_i$ exists and satisfies $\mathcal{T}_*\in\mathbb{T}_\rateset^{[t,s]}$. Therefore, consider any Cauchy sequence $\left\{\mathcal{T}_i\right\}_{i\in\nats}$ in  $\mathbb{T}_\rateset^{[t,s]}$, meaning that
\begin{equation*}
...
\begin{equation*}
\mathbb{C}_\epsilon\coloneqq \left\{\mathcal{T}_i\in\mathbb{T}_\rateset^{[t,s]}\,:\,i\in\{1,\ldots,N\}\right\}\,,
\end{equation*}
with $N\in\nats$, such that
\begin{equation*}
(\forall \mathcal{T}\in\mathbb{T}_\rateset^{[t,s]})(\exists \mathcal{S}\in \mathbb{C}_\epsilon)\,:\,d(\mathcal{T},\mathcal{S}) < \epsilon\,.
\end{equation*}
...
It remains to show that $\mathbb{C}_\epsilon$ generates an open $\epsilon$-cover of $\mathbb{T}_\rateset^{[t,s]}$. To this end, consider any $\mathcal{T}\in\mathbb{T}_\rateset^{[t,s]}$. We will show that there is some $\mathcal{S}\in \mathbb{C}_{\epsilon}$ such that $d(\mathcal{T},\mathcal{S})<\epsilon$. We start by constructing this $\mathcal{S}$.

Because $\mathcal{T}\in\mathbb{T}_\rateset^{[t,s]}$, it follows that there is some corresponding $P\in\wmprocesses_\rateset$ which agrees with $\mathcal{T}$ on all transition matrices $T_q^r\in\mathcal{T}$. 
...
\end{equation*}
Because this holds for any $\mathcal{T}\in\mathbb{T}_\rateset^{[t,s]}$, it follows that $\mathbb{C}_\epsilon$ is a finite, open $\epsilon$-cover of $\mathbb{T}_\rateset^{[t,s]}$. Because the $\epsilon\in\realspos$ was arbitrary, we conclude that $\mathbb{T}_\rateset^{[t,s]}$ is totally bounded.
\end{proof}
...
By Lemma~{\bf REF}, $\mathbb{T}_{\widetilde{\rateset}}^{[t,s]}$ is totally bounded. Therefore, and because any subset of a totally bounded set is itself totally bounded~{\bf CITE}, it follows that $\mathbb{T}_\rateset^{[t,s]}$ is totally bounded as well.
\end{proof}

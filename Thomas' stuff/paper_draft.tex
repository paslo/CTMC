...
\end{equation*}
Consider this $\widetilde{Q}_i$. Then, because $\widetilde{Q}_i\in\rateset_{\epsilon'}$, we have that $\mathcal{T}_{\widetilde{Q_i}}^{[t_{i-1},t_i]}\in\widetilde{\mathcal{T}}_i$.

If we now repeat this selection for all $i\in\{1,\ldots,n\}$, we end up with a collection $\mathcal{T}_{\widetilde{Q_1}}^{[t_{0},t_1]},\ldots,\mathcal{T}_{\widetilde{Q_n}}^{[t_{n-1},t_n]}$ such that $\mathcal{T}_{\widetilde{Q_i}}^{[t_{i-1},t_i]}\in \widetilde{\mathcal{T}}_i$ for all $i\in\{1,\ldots,n\}$. Hence, by the construction of $C_{\epsilon'}$, we have that
\begin{equation*}
...
\end{equation*}
It remains to show that $d(\mathcal{T},\mathcal{S})<\epsilon$. We make use of the following two claims, which are proved below.
\begin{claim}\label{claim:lemma_total_bounded_full_interval_error}
Consider any $i\in\{1,\ldots,n\}$. Then, with $T_{t_{i-1}}^{t_i}\in\mathcal{T}$ and $S_{t_{i-1}}^{t_i}\in\mathcal{S}$, it holds that
\begin{equation*}
...
For the second case, suppose that $q\in[t_{i-1},t_i]$ and $r\in[t_i,t_{i+1}]$ for some $i\in\{1,\ldots,n-1\}$. Then, because $T_q^r$ and $S_q^r$ satisfy Equation~\eqref{eq:transmatrixproduct}, it follows from Lemma~{\bf RECURSIVE} and Claim~\ref{claim:lemma_total_bounded_sub_interval_error} that
\begin{align*}
...
\norm{T_q^r - S_q^r} &= \norm{T_q^{t_i}\left(\prod_{k=i+1}^{j-1}T_{t_{k-1}}^{t_k}\right)T_{t_{j-1}}^r - S_q^{t_i}\left(\prod_{k=i+1}^{j-1}S_{t_{k-1}}^{t_k}\right)S_{t_{j-1}}^r} \\
 &< \frac{\Delta_i\epsilon}{3C} + \frac{\epsilon}{6} + \left(\sum_{k=i+1}^{j-1}\cdot\frac{\Delta_k\epsilon}{3C} + \Delta_k\epsilon'\right) + \frac{\Delta_{j}\epsilon}{3C} + \frac{\epsilon}{6} \\
...
Consider any $i\in\{1,\ldots,n\}$. Then, with $T_{t_{i-1}}^{t_i}\in\mathcal{T}$ and $S_{t_{i-1}}^{t_i}\in\mathcal{S}$, it follows from Equation~\eqref{eq:lemma_total_bounded_full_interval_error_linear} that there is some $Q_i\in\rateset$ such that
\begin{align*}

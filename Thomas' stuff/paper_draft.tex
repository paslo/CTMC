...
Choose now any $u\in\mathcal{U}_{[t,s]}$ with $\sigma(u)<\delta^*$, such that $u=t_0,\ldots,t_n$. Consider any $i\in\{1,\ldots,n\}$. For all $Q_j\in\rateset_{\epsilon'}$, we then let $\mathcal{T}_{Q_j}^{[t_{i-1},t_i]}$ be the restriction of the transition matrix system $\mathcal{T}_{Q_j}$ to the interval $[t_{i-1},t_i]$.

We now consider the set of these restricted transition matrix systems. For all $i\in\{1,\ldots,n\}$, let $\mathbb{T}_i$ be the set of families of transition matrices defined by
\begin{equation*}

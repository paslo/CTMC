...
In order to avoid having to use a too fine grid, as his uniform discretisation method tends to require, \v{S}kulj also considers another computational method, where the size of discretisation steps is not kept constant, but is adapted in such a way as to try and reduce the total number of steps required. *** he also combines his two methods *** we don't do this, nor consider a combination, but it could be a good idea to do this (although perhaps linear rather than exponential) ***

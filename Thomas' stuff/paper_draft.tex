...
On the one hand, {\v{S}kulj}'s first definition of an imprecise continuous-time Markov chain suggests that $\mathcal{P}$ is the set of all continuous-time Markov chains whose time-dependent transition rate matrix $Q_s$ takes values in $\rateset$. Since $Q_s$ is typically defined as a derivative, it furthermore suggests that $T_0^s$ should be differentiable in some way---or, at the very least, that $Q_s$ should be integrable. However, this does not seem compatible with Equation~\eqref{eq:constraintsDamjan}. First of all, the use of the Dini derivatives suggests that $T_0^s$ is not required to be differentiable. Secondly, Equation~\eqref{eq:constraintsDamjan} does not impose a Markov condition. So, in summary, it seems to us that {\v{S}kulj} clearly intends for $\mathcal{P}$ to be a set of stochastic processes of which the infinitesimal differences are compatible with $\rateset$, but that he does not make it explicit whether these processes should be Markov chains, nor whether their transition operators $T_0^s$ should be differentiable.

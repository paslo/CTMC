...
\begin{corollary}
Consider any $P\in\wprocesses$. If $\smash{\overline{\partial}_{+}
{T^t_{t,\,x_u}}}$ is a singleton, then $\smash{\partial_{+}
{T^t_{t,\,x_u}}}$ exists and is equal to the unique element of $\smash{\overline{\partial}_{+}
...
\begin{theorem} *** only to be touched by Jasper ***
...
(T^{t}_{t-\Delta,\,x_u}-I)-Q}<\epsilon.
\end{equation*}
Let $U_r\coloneqq(r-\delta_r,r+\delta_r)$. Then the set $C\coloneqq\{U_r\colon r\in[t,s]\}$ is an open cover of $[t,s]$. By the Heine-Borel theorem, $C$ contains a finite subcover $C^*$ of $[t,s]$. Without loss of generality, we can take this subcover to be minimal, in the sense that if we remove any of its elements, it is no longer a cover. Let $n$ be the cardinality of $C^*$ and let $r_1<r_2<\dots<r_n$ be the ordered sequence of the midpoints of the intervals in $C^*$.

Next, we prove that
\begin{equation}\label{eq:orderingofbounds}
r_i-\delta_{r_i}\leq r_j-\delta_{r_j}
\text{~~and~~}
r_i+\delta_{r_i}\leq r_j+\delta_{r_j}
\text{~~for all $1\leq i<j\leq n$.}
\end{equation}
Assume \emph{ex absurdo} that this statement is not true. Then this implies that there are $1\leq i<j\leq n$ such that either $r_i-\delta_{r_i}>r_j-\delta_{r_j}$ or $r_i+\delta_{r_i}>r_j+\delta_{r_j}$. If $r_i-\delta_{r_i}>r_j-\delta_{r_j}$, then since $i<j$ implies that $r_i<r_j$, it follows that $\delta_{r_j}>\delta_{r_i}+r_j-r_i>\delta_{r_i}$ and therefore, that $r_j+\delta_{r_j}>r_i+\delta_{r_i}$. 
Hence, we find that $U_{r_i}\subseteq U_{r_j}$. Since $C^*$ was taken to be a minimal cover, this is a contradiction.
Similarly, If $r_i+\delta_{r_i}>r_j+\delta_{r_j}$, then since $i<j$ implies that $r_i<r_j$, it follows that $\delta_{r_i}>\delta_{r_j}+r_j-r_i>\delta_{r_j}$ and therefore, that $r_i-\delta_{r_i}<r_j-\delta_{r_i}$. Hence, we find that $U_{r_j}\subseteq U_{r_i}$. Since $C^*$ was taken to be a minimal cover, this is again a contradiction. This allows us to conclude that Equation~\eqref{eq:orderingofbounds} is indeed true.

\end{proof}

\begin{theorem}
*** only to be touched by Jasper ***
*** je kan dingen aan elkaar plakken ***
\end{theorem}
\begin{proof}

...
{T^t_{t,\,x_u}}}$. Since $P$ is well-behaved, it follows from Definition~\ref{def:well-behaved} that there is some $B>0$ and $\epsilon>0$ such that
...
{T^t_{t,\,x_u}}}$. Because of Equation~\eqref{eq:rightouterderivative}, $Q$ is the limit of a sequence of matrices $Q_k$, $i\in\nats$, defined by
...
We only give the proof for $\smash{\overline{\partial}_{+}
{T^t_{t,\,x_u}}}$. The proof for $\smash{\overline{\partial}_{-}
{T^t_{t,\,x_u}}}$ is completely analogous.
Assume \emph{ex absurdo} that there is some $\epsilon$ for which the statement does not hold, meaning that
\begin{equation*}
...
{T^t_{t,\,x_u}}$, it follows that $\norm{Q^*-Q^*}\geq\epsilon$, a contradiction.
\end{proof}

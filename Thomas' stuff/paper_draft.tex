...
*** introduce state space, notation for naturals, reals, ... notation and conventions for matrices ***
...
*** nog wat notatie nodig voor arbitraire (eindige) reeks tijdpunten $<t$, die 0 wel mogen, maar niet hoeven te bevatten.
...
A real-valued $m\times m$ matrix $Q$ is said to be a \emph{transition rate matrix}, or sometimes simply \emph{rate matrix}, if
...
A real-valued $m\times m$ matrix $T$ is said to be a \emph{transition matrix} if
\vspace{5pt}
...
\noindent We start by showing some properties of the norm of these matrices.

\begin{proposition}
Let $Q$ be an arbitrary transition rate matrix. Then, $0\leq\norm{Q}<+\infty$.
\end{proposition}
\begin{proof}
Trivial consequence of the fact that $Q$ is a real-valued matrix.
\end{proof}
...
\end{proposition}
\begin{proof}
This follows directly from Definition~\ref{def:stoch_matrix}.
\end{proof}

*** tralala, rate matrices and transition matrices zijn sterk gelinkt:

\begin{proposition}\label{prop:stochastic_from_rate_matrix}
Consider any $m\times m$ rate matrix $Q\in\mathcal{R}$, and any $0\leq \Delta \leq \nicefrac{1}{\norm{Q}}$. Let $I$ denote the $m\times m$ identity matrix. Then, the matrix $[I+\Delta Q]$ is a transition matrix.
\end{proposition}
\begin{proof}
...
Consider any $m\times m$ transition matrix $T$, and any $\Delta\in\realspos$. Let $I$ denote the $m\times m$ identity matrix. Then, $[\nicefrac{1}{\Delta}\cdot(T-I)]$ is a transition rate matrix.
\end{proposition}
\begin{proof}
...
\begin{exmp}
*** voorbeeld van wat we dus niet toestaan (wortels bijvoorbeeld)
\exampleend
\end{exmp}

*** tralala, we kunnen eigenschappen van stochastische processen uitdrukken met transitie matrices

\begin{definition}[Corresponding Transition Matrix]\label{def:trans_matrix}
Consider any stochastic process $P\in\processes$. Then, for any $0\leq t\leq s$, the \emph{corresponding transition matrix} $T_t^s$ is a $m\times m$ matrix that is defined by
\begin{equation*}
...
Obviously, for a given $P\in\processes$, a corresponding transition matrix $T_t^s$ can also be regarded as a map from $\gamblesX$ to $\gamblesX$, defined for all $f\in\gamblesX$ by $T_t^s(f)\coloneqq T_t^sf$. Note that for all $x_t\in\states$, we have that
\begin{align*}
...
*** tralala, dit heeft een generalisatie, waarbij we voor elke geschiedenis $x_u$ die zich afspeelt \emph{voor} tijdstip $t$, een aparte transitie matrix $T_{t,x_u}^s$ van $t$ naar $s$ pakken:
...
***
show that $T_{t,x_u}^s$ is also a transition matrix (and possibly well-behaved) by including this case in proposition below.
***

\begin{proposition}\label{prop:stochasticprocess:simpleproperties}
Let $P\in\processes$ be a stochastic process. Then for any $0\leq t\leq s$, $T_t^s$ is a transition matrix and $T_t^t=I$ and, if $P$ is well-behaved, then also
...
*** tralala, we kunnen eigenschappen van transitie matrices van een gegeven process uitdrukken aan de hand van (directionele en partieele) afgeleides. Als ze bestaan, zijn dat rate matrices:
...
*** tralala, ze bestaan echter niet altijd, dus het is prettiger om deze generalisatie te beschouwen:
...
*** deze generalisatie bestaat wel altijd (tenminste voor well-behaved processen):
...
*** tralala, de inhoud van deze outer partial derivs zijn intuitief te beschouwen als limieten:
...
*** tralala, als de directionele partiele afgeleides bestaan, komen ze overeen met de generalisatie:
...
*** dit beest moet maar even naar een appendix:
...
\end{lemma}
\begin{proof}
Just check each of the three defining properties. C2 and C3 are trivial. C1 requires a bit more work. *** {\bf TODO of REF}
\end{proof}

\begin{lemma}\label{lemma:lower_trans_to_lower_rate}
Consider any lower transition operator $\lt$, and any $\Delta\in\realspos$. Then, $\nicefrac{1}{\Delta}(\lt - I)$ is a lower transition rate operator.
\end{lemma}
\begin{proof}
Just check each of the three defining properties. *** {\bf TODO or REF}
\end{proof}

We next have two results about the set $\underline{\mathcal{T}}$ of all lower transition operators. As the first result shows, this set is closed under composition.
...
\end{equation*}
By Proposition~\ref{prop:rate_from_stochastic_matrix}, we have that $\nicefrac{1}{\Delta}(T_t^{t+\Delta}-I)$ is a transition rate matrix, and hence by Definition~\ref{def:rate_matrix} we must have that $\nicefrac{1}{\Delta}\left[(T_t^{t+\Delta}-I)\ind{x}\right](x)\leq 0$, and $\nicefrac{1}{\Delta}\abs{\left[(T_t^{t+\Delta}-I)\ind{y}\right](x)}\leq\nicefrac{1}{\Delta}\abs{\left[(T_t^{t+\Delta}-I)\ind{x}\right](x)}$. It therefore follows that also
\begin{equation}\label{eq:nonwellbehaved_limit_diagonal}
\limsup_{\Delta\to0^+}\frac{1}{\Delta}\abs{\left[(T_t^{t+\Delta}-I)\ind{x}\right](x)} \nless +\infty\,.
\end{equation}
...
Because we assumed that the statement is false, and because $\ind{x}\in\gamblesX$, it must hold for all $\Delta\in\realspos$ that $\left[L_{t}^{t+\Delta}\ind{x}\right](x)\leq\left[T_{t}^{t+\Delta}\ind{x}\right](x)$, or equivalently, that
\begin{equation*}
\frac{1}{\Delta}\left[(L_{t}^{t+\Delta} - I)\ind{x}\right](x) \leq \frac{1}{\Delta}\left[(T_{t}^{t+\Delta} - I)\ind{x}\right](x)\,.
\end{equation*}
From our earlier observations, we know that both sides of this inequality are non-positive. Hence, we must have that
\begin{equation*}
\abs{\frac{1}{\Delta}\left[(T_{t}^{t+\Delta} - I)\ind{x}\right](x)} \leq \abs{\frac{1}{\Delta}\left[(L_{t}^{t+\Delta} - I)\ind{x}\right](x)}\,.
\end{equation*}
...
\begin{equation*}
\abs{\frac{1}{\Delta}\left[(T_{t}^{t+\Delta} - I)\ind{x}\right](x)} \leq \norm{\frac{1}{\Delta}(L_{t}^{t+\Delta} - I)}\,.
\end{equation*}
...
\begin{equation*}
\limsup_{\Delta\to0^+}\abs{\frac{1}{\Delta}\left[(T_{t}^{t+\Delta} - I)\ind{x}\right](x)} \leq \limsup_{\Delta\to0^+}\norm{\frac{1}{\Delta}(L_{t}^{t+\Delta} - I)} = \norm{\lrate} < +\infty\,,
\end{equation*}
which, by Equation~\eqref{eq:nonwellbehaved_limit_diagonal}, is a contradiction. Therefore, if the statement is indeed false, we must instead have that for some (other) $t\in\realsnonneg$ and some (other) $x,y\in\states$,
...
\begin{equation*}
\abs{\frac{1}{\Delta}\left[(T_{t-\Delta}^t - I)\ind{x}\right](x)} \leq \norm{\frac{1}{\Delta}(L_{t-\Delta}^t - I)}\,.
\end{equation*}
...
\begin{equation*}
\limsup_{\Delta\to0^+}\abs{\frac{1}{\Delta}\left[(T_{t-\Delta}^t - I)\ind{x}\right](x)} \leq \limsup_{\Delta\to0^+}\norm{\frac{1}{\Delta}(L_{t}^{t+\Delta} - I)} = \norm{\lrate} < +\infty\,,
\end{equation*}
...
*** Neem aan dat we een methode hebben om voor gegeven $f\in\gamblesX$, de term $\hat{f}_t^s :\approx L_t^sf$ numeriek uit te rekenen, of te benaderen tot arbitraire precisie. Bijvoorbeeld, gebruik de methode van Damjan. Noteer de complexiteit van deze methode als $\mathcal{O}(c)$.

\begin{exmp}
*** Voorbeeld dat uitrekenen voor arbitraire $f\in\gambles(\states^{u\cup v})$ exponentieel moeilijk is. \exampleend
\end{exmp}

*** Introductie subclasse van $f\in\gambles(\states^{u\cup v})$: sommen van een-variabele functies

*** Introductie subclasse van $f\in\gambles(\states^{u\cup v})$: producten van een-variabele functies

*** methode 1: uitrekenen van som-klasse

*** laten zien dat methode inderdaad correct antwoord geeft (misschien als methode om $\hat{f}_t^s$ uit te rekenen een benadering is, de fout van de methode afschatten?)

*** methode 2: uitrekenen van product-klasse
...
\section{Proofs of Results from Section~\ref{sec:lowertrans}}\label{sec:proof_appendix}

...
Consider any non-empty, bounded and convex set of rate matrices $\rateset$, and any $t,s\in\realsnonneg$ such that $t\leq s$. Then, the metric space $\smash{(\mathbb{T}_\rateset^{[t,s]},d)}$ is totally bounded under the metric $d$, defined as in Equation~\eqref{eq:trans_mat_system_metric}.
\end{lemma}
\begin{proof}
In order to prove that $(\mathbb{T}_\rateset^{[t,s]},d)$ is totally bounded, it suffices to construct, for all $\epsilon\in\realspos$, a finite collection of open $\epsilon$-balls centered on elements of $\smash{\mathbb{T}_\rateset^{[t,s]}}$, such that this collection covers $\smash{\mathbb{T}_\rateset^{[t,s]}}$ *** REF ***. Fix any $\epsilon>0$. We will now construct such a finite collection of open $\epsilon$-balls.
...
Because $\rateset$ is non-empty, bounded and convex, we know from Lemma~\ref{lemma:uniform_delta_for_convex_markov_set} that there is some $\delta_1\in\realspos$ such that for all $P\in\wmprocesses_\rateset$, all $t\in\realsnonneg$, and all $\Delta\in\realsnonneg$ such that $\Delta<\delta_1$:
...
Let $\delta\coloneqq\min\{\delta_1,\delta_3,\delta_4\}$. Then for all $P\in\wmprocesses_\rateset$, all $t\in\realsnonneg$, and all $\Delta\in\realsnonneg$ such that $\Delta<\delta$, it follows from Equations~\eqref{lemma:restricted_trans_mat_system_totally_bounded_if_Q_bounded_convex:epsilon1} and~\eqref{lemma:restricted_trans_mat_system_totally_bounded_if_Q_bounded_convex:epsilon2} and Lemma~\ref{lemma:linearpartofexponential} that there are $Q\in\rateset$ and $\tilde{Q}\in\rateset_{\epsilon_2}$ such that 
\begin{align*}
\norm{T_t^{t+\Delta}-e^{\tilde{Q}\Delta}}
&\leq
\norm{T_t^{t+\Delta}-(I+\Delta Q)}
+\norm{\Delta(Q-\tilde{Q})}
+\norm{I+\Delta\tilde{Q}-e^{\tilde{Q}\Delta}}\\
&\leq
\Delta\epsilon_1
+\Delta\epsilon_2
+\Delta^2\norm{\rateset}^2
\leq\Delta(\epsilon_1+\epsilon_2+\epsilon_3),
\end{align*}
where the final inequality holds because $\Delta\norm{\rateset}^2\leq\delta\norm{\rateset}^2\leq\delta_3\norm{\rateset}^2<\epsilon_3$. Hence, for all $P\in\wmprocesses_\rateset$, all $t\in\realsnonneg$, and all $\Delta\in\realsnonneg$ such that $\Delta<\delta$:
\begin{equation}\label{lemma:restricted_trans_mat_system_totally_bounded_if_Q_bounded_convex:epsilon123}
(\exists \tilde{Q}\in\rateset_{\epsilon_2})~
\norm{T_t^{t+\Delta}-e^{\tilde{Q}\Delta}}
\leq
...
Consider now any $u\in\mathcal{U}_{[t,s]}$ such that $\sigma(u)<\delta$. Then, without loss of generality, there is some $n\in\nats$ such that $u=t_0,\ldots,t_n$. 
...
Let
\begin{equation*}
\mathbb{C}_\epsilon \coloneqq \left\{\mathcal{T}_{\tilde{Q}_1}^{[t_{0},t_1]} \otimes \mathcal{T}_{\tilde{Q}_2}^{[t_{1},t_2]} \cdots \otimes \mathcal{T}_{\tilde{Q}_n}^{[t_{n-1},t_n]}\,:\,\left(\forall i\in\{1,\ldots,n\}\right)\,\tilde{Q}_i\in\rateset_{\epsilon_2}\right\},
\end{equation*}
...
Since $n$ and $\abs{\rateset_{\epsilon_2}}$ are finite, this set $\mathbb{C}_\epsilon$ is clearly also finite and, because of Claim~\ref{claim:composition_restricted_trans_mat_sys_in_set}, it is a subset of $\mathbb{T}_\rateset^{[t,s]}$.
...
\begin{equation*}
\norm{T_{t_{i-1}}^{t_i} - S_{t_{i-1}}^{t_i}} \leq \Delta_i(\epsilon_1+\epsilon_2+\epsilon_3).
\end{equation*}
\end{claim}
...
\begin{equation*}
\norm{T_q^r - S_q^r} \leq (r-q)(\epsilon_1+\epsilon_2+\epsilon_3).
\end{equation*}
\end{claim}

We first give a bound on the difference between two transition matrices in $\mathcal{T}^{[t,s]}$ and $\mathcal{S}^{[t,s]}$, for any given time points. To this end, consider any $q,r\in[t,s]$ such that $q\leq r$, and the transition matrices $T_q^r\in\mathcal{T}^{[t,s]}$ and $S_q^r\in\mathcal{S}^{[t,s]}$. 
...
<\epsilon_4
\leq 
2\epsilon_4+(s-t)(\epsilon_1+\epsilon_2+\epsilon_3)<\frac{\epsilon}{2}.
\end{equation*}
In any other case, there must be some $k,\ell\in\{1,\ldots,n\}$ such that $k\leq\ell$, $q\in[t_{k-1},t_k]$ and $r\in[t_{\ell},t_{\ell+1}]$. Therefore, and because of Definition~\ref{def:trans_mat_system}, it follows from Lemma~\ref{lemma:recursive} that
\begin{align*}
...
&\leq
2\epsilon_4+\sum_{i=k+1}^\ell(t_i-t_{i-1})(\epsilon_1+\epsilon_2+\epsilon_3)\\
&\leq 
2\epsilon_4+(s-t)(\epsilon_1+\epsilon_2+\epsilon_3)<\frac{\epsilon}{2}.
\end{align*}
Hence, in all cases: $\norm{T_q^r - S_q^r} < \nicefrac{\epsilon}{2}$.

Because this is true for all $q,r\in[t,s]$ such that $q\leq r$, it follows that
\begin{equation*}

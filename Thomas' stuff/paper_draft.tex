...
\end{equation}
with $C_0\coloneqq\cup_{i=1}^nC_i$. Clearly, without loss of generality, we may assume that for any $i,i'\in\{1,\dots,n\}$ such that $i\neq i'$, $(A_i,C_i)\neq(A_{i'},C_{i'})$.

For any $i\in\{1,\dots,n\}$, since $(A_i,C_i)\in\mathcal{C}_w$, there are $j_i\in\{0,\dots,m\}$ and, for all $\ell\in\{0,\dots,j_i\}$, $z_{\ell,i}\in\states$ such that
\begin{equation*}
...
We now let $C^*\coloneqq\cap_{1\leq \ell<m}(X_{w_\ell}=\omega(w_\ell))$ and $S^*\coloneqq\{i\in\{1,\dots,n\}\colon C_i=C^*\}$.

$\states^*\coloneqq\{x\in\states\colon (\exists i\in S^*) z_{m,i}=x\}$



 and $S^{**}\coloneqq\{i\in\{1,\dots,n\}\setminus(S\cup S^*)$, then for all $i\in S^{**}$ and $\omega\in C^*$
...
=\sum_{i\in S}\lambda_i\ind{C_i}(\omega^*)\bigl(P_w(A_i\vert C_i)-\ind{A_i}(\omega^*)\bigr)\geq0,
\end{equation*}
where the inequality follows from Equation~\eqref{eq:lemma:simplechaincoherence:supremumreached}. Hence, since $\omega^*\in C_0$, Equation~\eqref{eq:lemma:simplechaincoherence:TB} holds.

*** still have to finish this ***

\begin{align*}
&\sup\left\{\sum_{i=1}^n\lambda_i\ind{C_i}(\omega)\bigl(P_w(A_i\vert C_i)-\ind{A_i}(\omega)\bigr)~\Bigg\vert~\omega\in C^*\right\}\\
&=
\sup\left\{
\sum_{i\in S\cup S^*}\lambda_i\ind{C_i}(\omega)\bigl(P_w(A_i\vert C_i)-\ind{A_i}(\omega)\bigr)
~\Bigg\vert~\omega\in C^*\right\}\\
&\geq
\sup\left\{
\sum_{i\in S^*}\lambda_i\ind{C_i}(\omega)\bigl(P_w(A_i\vert C_i)-\ind{A_i}(\omega)\bigr)
~\Bigg\vert~\omega\in C^*\right\}\\
&=
\sup\left\{
...
~\Bigg\vert~\omega\in C^*\right\}\\
&=
\sum_{i\in S^*}\lambda_iP_w(A_i\vert C^*)
-
\inf\left\{
...
\sum_{i\in S^*}\lambda_iP_w(A_i\vert C^*)
-
\min_{x\in\states}\lambda_x\\
&=
\sum_{i\in S^*}\lambda_i p(z_{m,i})
-
\min_{x\in\states}\lambda_x\\
&=
\sum_{x\in \states^*}\lambda_{i_x} p(z_{m,i_x})
-
\min_{x\in\states}\lambda_x\\
&=
\sum_{x\in \states^*}\lambda_{x} p(x)
-
\min_{x\in\states}\lambda_x
\end{align*}

By the assumptions of this lemma, there is some probability mass function $p$ on $\states$ such that, for all $y\in\states$, $P_w(X_{w_m}=y\vert C^*)=p(y)$.

Since $(A_i,C_i)\neq(A_{i'},C_{i'})$ for $i\neq i'$, it follows that for any $x\in\states^*$, there is only one $i_x\in S^*$ such that $z_{m,i_x}=x$. Let $\lambda_x\coloneqq\lambda_{i_x}$.

For all $x\in\states\setminus\states^*$, let $\lambda_x\coloneqq 0$. Now let $\lambda^*\coloneqq\min_{x\in\states}\lambda_x$. Then
\begin{equation*}
\sum_{x\in\states^*}\lambda_x p(x)
=
\sum_{x\in\states}\lambda_x p(x)
\geq
\min_{x\in\states}\lambda_x
...
\end{lemma}
\begin{proof}
Let $\mathcal{T}\coloneqq\mathcal{T}_{P_1}=\mathcal{T}_{P_2}$ be the common transition matrix system of $P_1$ and $P_2$ and let $p$ be their common initial probability mass function, as defined by $p(y)\coloneqq P_1(X_0=y)=P_2(X_0=y)$ for all $y\in\states$. Let $\tilde{P}$ be a real-valued function on $\mathcal{C}$, with $\mathcal{C}$ and $\tilde{P}$ defined as in the proof of Theorem~\ref{theo:uniqueMarkovchain}. It then follows from Equation~\eqref{def:markov_property} that the restriction of $P_1$ and $P_2$ to $\mathcal{C}$ is equal to $\tilde{P}$. Furthermore, for any $s>0$, $y\in\states$ and $j\in\{1,2\}$, we find that
\begin{align*}
P_j(X_s=y)

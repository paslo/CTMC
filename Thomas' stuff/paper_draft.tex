...
\end{cases}
\end{equation}
where the maximum error is now $2\epsilon=0.002$, which is the sum of our previous error bound on $L_1^2f$, which was $\epsilon$, and the maximum extra error that may have arisen while computing Equation~\eqref{eq:secondpass}; the additivity of the two error bounds is a result of~\ref{LT:constantadditivity} and the fact that $L_t^s$ is a lower transition operator.
...
%		\Statex
		\For{$i\in\{m,m-1,\ldots,0\}$}	\Comment{Iterate backward over $v=s_0,\ldots,s_m$}
			...
			\State $g_{(i-1)}\gets ${\textsc{ComputeLuf}}($\lrate,u\cup w,s_i,g_i,\nicefrac{\epsilon}{m}$)  \Comment{Run Algorithm~\ref{alg:compute_singlevar_multiple_past}}\vspace{3pt}
%			\Statex
		\EndFor
%		\Statex

...
*** where $\underline{\mathbb{E}}$ can be either $\underline{\mathbb{E}}_{\rateset\,\mathcal{M}}^{\mathrm{WM}}$ or $\underline{\mathbb{E}}_{\rateset\,\mathcal{M}}^{\mathrm{W}}$. ***
...
\end{equation*}
resulting in a step size of $\Delta=\nicefrac{(s-t)}{n}=1.25\times 10^{-4}$. *** hier stond oorspronkelijk $1.6\times 10^{-4}$, wat denk ik mis was (zelfs met de originele norm) *** Note that this choice of $n$ here also satisfies the requirement $n\geq (s-t)\norm{\lrate}$ from Proposition~\ref{prop:approximation_error_bound}. 

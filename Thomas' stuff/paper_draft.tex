...
Recall from Proposition~\ref{prop:Markovhassystem} that for all $P\in\wmprocesses$, there is a well-behaved transition matrix system $\mathcal{T}_P$ corresponding to this $P$. For any $t,s\in\realsnonneg$ such that $t\leq s$, let $\mathcal{T}_P^{[t,s]}$ be the restriction of $\mathcal{T}_P$ to the interval $[t,s]$. That is, $\mathcal{T}_P^{[t,s]}$ contains all transition matrices $T_q^r\in\mathcal{T}_P$ such that $q,r\in[t,s]$ with $q\leq r$.

For any non-empty bounded set $\rateset\subset\mathcal{R}$ and any $t,s\in\realsnonneg$ such that $t\leq s$, let now
\begin{equation*}
\mathcal{T}_\rateset^{[t,s]} \coloneqq \left\{\mathcal{T}_P^{[t,s]}\,:\,P\in\wmprocesses_\rateset\right\}\,.
\end{equation*}
We next endow this space $\mathcal{T}_\rateset^{[t,s]}$ with the following metric $d$,  defined for all $\mathcal{T},\mathcal{S}\in\mathcal{T}_\rateset^{[t,s]}$ as
\begin{equation}\label{eq:trans_mat_system_metric}
d(\mathcal{T},\mathcal{S}) \coloneqq \sup\left\{\norm{T_q^r - S_q^r}\,:\,q,r\in[t,s], q\leq r\right\}\,,
\end{equation}
where, for all $q,r\in[t,s]$, it is understood that $T_q^r\in\mathcal{T}$ and $S_q^r\in\mathcal{S}$.

We have the following results.

\begin{lemma}
Consider any non-empty, bounded and closed set of rate matrices $\rateset$, and any $t,s\in\realsnonneg$ such that $t\leq s$. Then, the metric space $(\mathcal{T}_\rateset^{[t,s]},d)$ is complete under the metric $d$, defined as in Equation~\eqref{eq:trans_mat_system_metric}.
\end{lemma}
\begin{proof}
We need to show that for any Cauchy sequence $\left\{\mathcal{T}_i\right\}_{i\in\nats}$ in $\mathcal{T}_\rateset^{[t,s]}$, it holds that $\mathcal{T}_*\coloneqq\lim_{i\to\infty}\mathcal{T}_i$ exists and satisfies $\mathcal{T}_*\in\mathcal{T}_\rateset^{[t,s]}$. Therefore, consider any Cauchy sequence $\left\{\mathcal{T}_i\right\}_{i\in\nats}$ in  $\mathcal{T}_\rateset^{[t,s]}$, meaning that
\begin{equation*}
(\forall\epsilon\in\realspos)(\exists N\in\nats)(\forall i,j>N)\,:\,d(\mathcal{T}_i,\mathcal{T}_j) < \epsilon\,.
\end{equation*}

We will first show that $\mathcal{T}_*\coloneqq\lim_{i\to\infty}\mathcal{T}_i$ exists.

*** {\bf TODO} finish this
\end{proof}

\begin{lemma}
Consider any non-empty, bounded and convex set of rate matrices $\rateset$, and any $t,s\in\realsnonneg$ such that $t\leq s$. Then, the metric space $(\mathcal{T}_\rateset^{[t,s]},d)$ is totally bounded under the metric $d$, defined as in Equation~\eqref{eq:trans_mat_system_metric}.
\end{lemma}
\begin{proof}
We have to show that $(\mathcal{T}_\rateset^{[t,s]},d)$ is totally bounded, meaning that for all $\epsilon\in\realspos$, there exists a finite collection of open $\epsilon$-balls centered on elements of $\mathcal{T}_\rateset^{[t,s]}$, such that this collection covers $\mathcal{T}_\rateset^{[t,s]}$. 

Equivalently, we need to show that for all $\epsilon\in\realspos$, there is some set
\begin{equation*}
C_\epsilon\coloneqq \left\{\mathcal{T}_i\in\mathcal{T}_\rateset^{[t,s]}\,:\,i\in\{1,\ldots,N\}\right\}\,,
\end{equation*}
with $N\in\nats$, such that
\begin{equation*}
(\forall \mathcal{T}\in\mathcal{T}_\rateset^{[t,s]})(\exists \mathcal{T}_i\in C_\epsilon)\,:\,d(\mathcal{T},\mathcal{T}_i) < \epsilon\,.
\end{equation*}

Fix any $\epsilon\in\realspos$, and define $C\coloneqq (s-t)$. We will start by constructing $C_\epsilon$.

**** find $\epsilon'$.

Because $\rateset$ is closed and bounded, it is compact under the metric induced by our usual norm $\norm{\cdot}$. Therefore, $\rateset$ admits a finite cover of open balls with radius $\epsilon'$. In other words, there is some set
\begin{equation*}
...
Now, due to Lemma~{\bf REF}, there is some $\delta\in\realspos$ such that, for all $P\in\wmprocesses_\rateset$, all $t\in\realsnonneg$, and all $\Delta\in\realspos$ with $\Delta<\delta$,
\begin{equation}\label{eq:lemma_total_bounded_uniform_delta}
(\exists Q\in\rateset)\,\norm{T_t^{t+\Delta} - (I+\Delta Q)} < \frac{\Delta\epsilon}{XX\cdot C}\,.
\end{equation}
Consider this $\delta$. Let now
\begin{equation*}
\delta^* \coloneqq \min\left\{\frac{\epsilon}{\norm{\rateset}^2\cdot XX\cdot C}, \delta\right\}\,.
\end{equation*}
...
We now consider the set of these families of transition matrices. For all $i\in\{1,\ldots,n\}$, let $\widetilde{\mathcal{T}}_i$ be the set of families of transition matrices defined by
\begin{equation*}
\widetilde{\mathcal{T}}_i \coloneqq \left\{ \mathcal{T}_{Q_j}^{[t_{i-1},t_i]} \,:\,Q_j\in\rateset_{\epsilon'} \right\}\,.
\end{equation*}
Because $\rateset_{\epsilon'}$ is finite, $\widetilde{\mathcal{T}}_i$ is also clearly finite.

For all $i\in\{1,\ldots,n\}$, choose some $\mathcal{T}_i^{[t_{i-1},t_i]}\in\widetilde{\mathcal{T}}_i$. Let $\mathcal{T}^{[t,s]}$ be defined as (INCOMING HACKY NOTATION--- plak shit aan elkaar).
\begin{equation*}
...
\begin{equation*}
\mathcal{T}^{[t,s]} \in \mathcal{T}_\rateset^{[t,s]}\,.
\end{equation*}
\end{claim}
Because each $\widetilde{\mathcal{T}}_i$ is finite, there are only finitely many ways in which $\mathcal{T}^{[t,s]}$ can be constructed. Therefore, if we now let
\begin{equation*}
C_\epsilon \coloneqq \left\{\mathcal{T}_1^{[t_{0},t_1]} \otimes \mathcal{T}_2^{[t_{1},t_2]} \cdots \otimes \mathcal{T}_n^{[t_{n-1},t_n]}\,:\,\left(\forall i\in\{1,\ldots,n\} \,:\,\mathcal{T}_i^{[t_{i-1},t_i]}\in\widetilde{\mathcal{T}}_i\right) \right\}\,,
\end{equation*}
then $C_\epsilon$ contains a finite number of elements, and, due to Claim~\ref{claim:composition_restricted_trans_mat_sys_in_set}, $C_\epsilon\subset \mathcal{T}_\rateset^{[t,s]}$.

It remains to show that $C_\epsilon$ generates an open $\epsilon$-cover of $\mathcal{T}_\rateset^{[t,s]}$. To this end, consider any $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$. We will show that there is some $\mathcal{S}\in C_{\epsilon}$ such that $d(\mathcal{T},\mathcal{S})<\epsilon$. We start by constructing this $\mathcal{S}$.

Because $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$, it follows that there is some corresponding $P\in\wmprocesses_\rateset$ which agrees with $\mathcal{T}$ on all transition matrices $T_q^r\in\mathcal{T}$. 

Consider any $i\in\{1,\ldots,n\}$, and the corresponding transition matrix $T_{t_{i-1}}^{t_i}\in\mathcal{T}$. Clearly, we have that $T_{t_{i-1}}^{t_i}=T_{t_{i-1}}^{(t_{i-1}+\Delta_i)}$. Therefore, and because $\Delta_i\leq\sigma(u)<\delta^*\leq\delta$, it follows from Equation~\eqref{eq:lemma_total_bounded_uniform_delta} that there is some $Q_i\in\rateset$ such that
\begin{equation}\label{eq:lemma_total_bounded_full_interval_error_linear}
\norm{T_{t_{i-1}}^{t_i} - (I+\Delta_i Q_i)} < \frac{\Delta_i\epsilon}{XX\cdot C}\,.
\end{equation}
...
Hence, we have that
\begin{equation*}
\norm{(I + \Delta_iQ_i) - (I + \Delta_i\widetilde{Q}_i)} < \Delta_i\epsilon'\,.
\end{equation*}
Furthermore, because $\widetilde{Q}_i\in\rateset_{\epsilon'}$, we have that $\mathcal{T}_{\widetilde{Q_i}}^{[t_{i-1},t_i]}\in\widetilde{\mathcal{T}}_i$.

If we now repeat this selection for all $i\in\{1,\ldots,n\}$, we end up with a collection $\mathcal{T}_{\widetilde{Q_1}}^{[t_{0},t_1]},\ldots,\mathcal{T}_{\widetilde{Q_n}}^{[t_{n-1},t_n]}$. Furthermore, by the construction of $C_{\epsilon'}$, we have that
\begin{equation*}
\mathcal{S}\coloneqq \mathcal{T}_{\widetilde{Q_1}}^{[t_{0},t_1]} \otimes \cdots \otimes \mathcal{T}_{\widetilde{Q_n}}^{[t_{n-1},t_n]} \in C_\epsilon\,.
\end{equation*}
It remains to show that $d(\mathcal{T},\mathcal{S})<\epsilon$.

To this end, consider first any $i\in\{1,\ldots,n\}$. Then, with $T_{t_{i-1}}^{t_i}\in\mathcal{T}$ and $S_{t_{i-1}}^{t_i}\in\mathcal{S}$, it follows from Equation~\eqref{eq:lemma_total_bounded_full_interval_error_linear} that there is some $Q_i\in\rateset$ such that
\begin{align*}
\norm{T_{t_{i-1}}^{t_i} - S_{t_{i-1}}^{t_i}} &\leq \norm{T_{t_{i-1}}^{t_i} - (I+\Delta_iQ_i)} + \norm{(I+\Delta_iQ_i) - S_{t_{i-1}}^{t_i}} \\
 ...
 &< \frac{\Delta_i\epsilon}{XX\cdot C} + \Delta_i\epsilon' + \norm{(I+\Delta_i\widetilde{Q}_i) - S_{t_{i-1}}^{t_i}} \\
 &= \frac{\Delta_i\epsilon}{XX\cdot C} + \Delta_i\epsilon' + \norm{(I+\Delta_i\widetilde{Q}_i) - e^{\widetilde{Q}_i\cdot\Delta_i}} \\
 &\leq \frac{\Delta_i\epsilon}{XX\cdot C} + \Delta_i\epsilon' + \Delta_i^2\norm{\rateset}^2\,,
\end{align*}
where the strict equality follows from the definition of $\mathcal{S}$, and the final inequality follows from Lemma~\ref{lemma:linearpartofexponential}. Furthermore, because $\Delta_i\leq\sigma(u)<\delta^*\leq\nicefrac{\epsilon}{(\norm{\rateset}^2XX\cdot C)}$, we have that
\begin{equation}\label{eq:lemma_total_bounded_full_interval_error}
\norm{T_{t_{i-1}}^{t_i} - S_{t_{i-1}}^{t_i}} < \frac{\Delta_i\epsilon}{XX\cdot C} + \Delta_i\epsilon' + \Delta_i^2\norm{\rateset}^2 < 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + \Delta_i\epsilon'\,.
\end{equation}

Next, consider any $i\in\{1,\ldots,n\}$, and any $q\in[t_{i-1},t_i]$. Consider the transition matrices $T_q^{t_i}\in\mathcal{T}$ and $S_q^{t_i}\in\mathcal{S}$. Then, because $(t_i-q)\leq\Delta_i\leq\sigma(u)<\delta^*\leq\delta$, it follows from Equation~\ref{eq:lemma_total_bounded_uniform_delta} that there is some $Q\in\rateset$ such that
\begin{align*}
\norm{T_q^{t_i} - S_q^{t_i}} &< \frac{(t_i-q)\epsilon}{XX\cdot C} + \norm{(I+(t_i-q)Q) - S_q^{t_i}} \\
 &\leq \frac{(t_i-q)\epsilon}{XX\cdot C} + \norm{(I+(t_i-q)Q) - (I+(t_i-q)\widetilde{Q}_i)} + \norm{(I+(t_i-q)\widetilde{Q}_i) - S_q^{t_i}} \\
 &\leq \frac{(t_i-q)\epsilon}{XX\cdot C} + 2(t_i-q)\norm{\rateset} + \norm{(I+(t_i-q)\widetilde{Q}_i) - S_q^{t_i}} \\
 &= \frac{(t_i-q)\epsilon}{XX\cdot C} + 2(t_i-q)\norm{\rateset} + \norm{(I+(t_i-q)\widetilde{Q}_i) - e^{\widetilde{Q}_i\cdot(t_i-q)}} \\
 &\leq \frac{(t_i-q)\epsilon}{XX\cdot C} + 2(t_i-q)\norm{\rateset} + (t_i-q)^2\norm{\rateset}^2 \\
 &\leq \frac{\Delta_i\epsilon}{XX\cdot C} + 2(t_i-q)\norm{\rateset} + \Delta_i^2\norm{\rateset}^2 \\
 &\leq \frac{\Delta_i\epsilon}{XX\cdot C} + 2(t_i-q)\norm{\rateset} + \frac{\Delta_i\epsilon}{XX\cdot C}\,,
\end{align*}
where the last inequality follows from the fact that $\Delta_i\leq\sigma(u)<\delta^*\leq\nicefrac{\epsilon}{(\norm{\rateset}^2XX\cdot C)}$. Hence, and because $(t_i-q)\leq\Delta_i$, we have
\begin{equation}\label{eq:lemma_total_bounded_left_interval_error}
\norm{T_q^{t_i} - S_q^{t_i}} < 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset}\,.
\end{equation}

Next, consider any $i\in\{1,\ldots,n\}$, and any $r\in[t_{i-1},t_i]$. Consider the transition matrices $T_{t_{i-1}}^r\in\mathcal{T}$ and $S_{t_{i-1}}^r\in\mathcal{S}$. Using a completely analogous argument as above, we find that
\begin{equation}\label{eq:lemma_total_bounded_right_interval_error}
\norm{T_{t_{i-1}}^r - S_{t_{i-1}}^r} < 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset}\,.
\end{equation}

Finally, consider any $i\in\{1,\ldots,n\}$, and any $q,r\in[t_{i-1},t_i]$ with $q\leq r$. Consider the transition matrices $T_q^r\in\mathcal{T}$ and $S_q^r\in\mathcal{S}$. Again using the same argument, and the fact that $(r-q)\leq\Delta_i$, we have
\begin{equation}\label{eq:lemma_total_bounded_middle_interval_error}
\norm{T_q^r - S_q^r} < 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset}\,.
\end{equation}

We are now ready to prove the general case. Consider any $q,r\in[t,s]$ such that $q\leq r$, and the transition matrices $T_q^r\in\mathcal{T}$ and $S_q^r\in\mathcal{S}$. We consider three different cases. If $q,r\in[t_{i-1},t_i]$ for some $i\in\{1,\ldots,n\}$, then it follows from Equation~\eqref{eq:lemma_total_bounded_middle_interval_error} that
\begin{equation*}
\norm{T_q^r - S_q^r} < 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset} < \cdots < \epsilon TODO
\end{equation*}

For the second case, suppose that $q\in[t_{i-1},t_i]$ and $r\in[t_i,t_{i+1}]$ for some $i\in\{1,\ldots,n-1\}$. Then, because $T_q^r$ and $S_q^r$ satisfy Equation~\eqref{eq:transmatrixproduct}, it follows from Lemma~{\bf RECURSIVE} and Equations~\eqref{eq:lemma_total_bounded_left_interval_error} and~\eqref{eq:lemma_total_bounded_right_interval_error} that
\begin{align*}
\norm{T_q^r - S_q^r} &= \norm{T_q^{t_i}T_{t_i}^r - S_q^{t_i}S_{t_i}^r} \\
 &< \quad 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + 2\cdot\frac{\Delta_{i+1}\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset} + 2\Delta_{i+1}\norm{\rateset}
\end{align*}

In any other case, there must be some $i,j\in\{1,\ldots,n\}$ such that $i+1<j$, and for which $q\in[t_{i-1},t_i]$ and $r\in[t_{j-1},t_j]$. Therefore, and because $T_q^r$ and $S_q^r$ satisfy Equation~\eqref{eq:transmatrixproduct}, it follows from Lemma~{\bf RECURSIVE} and Equations~\ref{eq:lemma_total_bounded_full_interval_error}, \eqref{eq:lemma_total_bounded_left_interval_error} and~\eqref{eq:lemma_total_bounded_right_interval_error} that
\begin{align*}
\norm{T_q^r - S_q^r} &= \norm{T_q^{t_i}\left(\prod_{k=i+1}^{j-1}T_{t_{k-1}}^{t_k}\right)T_{t_{j-1}}^r - S_q^{t_i}\left(\prod_{k=i+1}^{j-1}S_{t_{k-1}}^{t_k}\right)S_{t_{j-1}}^r} \\
 &< \quad 2\cdot\frac{\Delta_i\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset} \\
 &\quad + \sum_{k=i+1}^{j-1}2\cdot\frac{\Delta_k\epsilon}{XX\cdot C} + \Delta_k\epsilon' \\
 &\quad + 2\cdot\frac{\Delta_{j}\epsilon}{XX\cdot C} + 2\Delta_{j}\norm{\rateset} \\
 &= 2\cdot\sum_{k=i}^j \frac{\Delta_k\epsilon}{XX\cdot C} + 2\Delta_i\norm{\rateset} + 2\Delta_j\norm{\rateset} + \sum_{k=i+1}^{j-1}{\Delta_k\epsilon'}
\end{align*}
\end{proof}
...
The claim follows immediately from the fact that the inequality
\begin{equation*}
e^a - (1 + a) < a^2
\end{equation*}
holds for all $a\in(0,1.79328)$, up to some numerical error on the upper bound.

Unfortunately, solving this problem involves finding the roots of a function with quadratic and exponential terms, which has no algebraic solution. However, the claim is easily verified using graphical methods, or checked with automated theorem provers like Mathematica~{\bf REF}.

***** IF YOU HAVE ANY IDEA HOW TO SOLVE THIS NICELY, PLEASE LET ME KNOW :)
\end{proof}
...
Let $\epsilon'\coloneqq \nicefrac{\epsilon}{2}$. Then, by Lemma~{\bf REF}, there is some $u\in\mathcal{U}_{[t,t+\Delta]}$ with $u=t_0,\ldots,t_n$, such that, for all $i\in\{1,\ldots,n\}$,
\begin{equation*}
...
\end{equation*}
Therefore, and due to **** SOME LEMMA WITH A GENERAL VERSION OF THE RECURSIVELY PROVEN NORM INEQUALITY THAT WE USE A LOT IN THIS PAPER ***
we find using the Markov property of $P$ that
\begin{align*}
...
Q\coloneqq \sum_{i=1}^n \frac{\Delta_i}{\Delta} Q_i\,.
\end{equation*}
Then, by Lemma~{\bf REF}, we have that $Q\in\rateset$, and because $\Delta<\delta\leq\nicefrac{1}{\norm{\rateset}}$,
\begin{equation*}
\norm{\prod_{i=1}^n(I+\Delta_iQ_i) - (I+\Delta Q)} < \Delta^2\norm{\rateset}^2\,.
\end{equation*}
...
\begin{lemma}
Consider any non-empty bounded set of rate matrices $\rateset$, and any $t,s\in\realsnonneg$ such that $t\leq s$. Then, the metric space $(\mathcal{T}_\rateset^{[t,s]},d)$ is totally bounded under the metric $d$, defined as in Equation~\eqref{eq:trans_mat_system_metric}.
\end{lemma}
\begin{proof}
Let $\widetilde{\rateset}$ denote the convex closure of $\rateset$. Then, clearly, $\widetilde{\rateset}$ is a non-empty, bounded, and convex set of rate matrices, and furthermore, $\mathcal{T}_\rateset^{[t,s]}\subseteq \mathcal{T}_{\widetilde{\rateset}}^{[t,s]}$.

By Lemma~{\bf REF}, $\mathcal{T}_{\widetilde{\rateset}}^{[t,s]}$ is totally bounded. Therefore, and because any subset of a totally bounded set is itself totally bounded~{\bf CITE}, it follows that $\mathcal{T}_\rateset^{[t,s]}$ is totally bounded as well.
\end{proof}

\begin{theorem}
Consider any non-empty, bounded and closed set of rate matrices $\rateset$, and any $t,s\in\realsnonneg$ such that $t\leq s$. Then, the metric space $(\mathcal{T}_\rateset^{[t,s]},d)$ is compact under the topology generated by the metric $d$, defined as in Equation~\eqref{eq:trans_mat_system_metric}.
\end{theorem}
\begin{proof}
**** Direct consequence of the lemma X and Y.
\end{proof}

\begin{proposition}
Consider any $t,s\in\realsnonneg$ such that $t\leq s$, and any non-empty bounded set of rate matrices $\rateset$. Then, for all $\mathcal{T}\in\mathcal{T}_\rateset^{[t,s]}$, there is a $P\in\wmprocesses_\rateset$ for which it holds that $\mathcal{T}_P^{[t,s]}=\mathcal{T}$.
\end{proposition}
\begin{proof}
*** rewrite below

Consider any $t,s\in\realsnonneg$ such that $t\leq s$, any non-empty bounded set of rate matrices $\rateset$, any $Q\in\rateset$, and any $\mathcal{S}\in\mathcal{T}_\rateset^{[t,s]}$. Consider the family of transition matrices $\mathcal{T}$, defined for all $q,r\in\realsnonneg$ with $q\leq r$, as
\begin{equation*}
T_q^r \coloneqq \left\{\begin{array}{ll}
e^{Q(r-q)} & \quad\text{if $q,r\leq t$ or $s\leq q,r$} \\
T_q^tT_t^r & \quad\text{if $q< t$ and $r\in[t,s]$} \\
T_q^sT_s^r & \quad\text{if $q\in[t,s]$ and $s< r$} \\
S_q^r\in\mathcal{S} & \quad\text{if $q,r\in[t,s]$}
\end{array}
\right.
\end{equation*}
Then, $\mathcal{T}$ is a well-behaved transition matrix system. 

Furthermore, for any probability mass function $p$ on $\states$, there is a well-behaved Markov chain $P$ such that $\mathcal{T}_P=\mathcal{T}$ and $P(X_0=x)=p(x)$ for all $x\in\states$, and for which it holds that $P\in\wmprocesses_\rateset$.
...
Let $S_\emptyset\coloneqq\left\{i\in\{1,\dots,n\}\colon(A_i,C_i)\in\mathcal{C}_\emptyset\right\}$. Since $P_\emptyset$ is a stochastic process, it follows from Corollary~\ref{corol:processiffcoherent} and Equation~\eqref{eq:theo:aanelkaarplakken:defPtilde} that *** still need to tackle the $S_\emptyset=\emptyset$ case separately ***
\begin{equation*}%\label{eq:theo:aanelkaarplakken:cohfirstpart}
\sup\left\{\sum_{i\in S_\emptyset}\lambda_i\ind{C_i}(\omega)\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega)\bigr)~\Bigg\vert~\omega\in C_\emptyset\right\}\geq0,
\end{equation*}
with $C_\emptyset\coloneqq\cup_{i\in S_\emptyset}C_i$. Since $S_\emptyset$ is finite and because, for every $i\in S_\emptyset$, $\ind{A_i}$ and $\ind{C_i}$ can only take two values---$0$ or $1$---it follows that this supremum is taken over a finite set of real numbers, which implies that it is actually a maximum. Therefore, there is some $\omega^*\in C_\emptyset$ such that
\begin{equation}\label{eq:theo:aanelkaarplakken:geqfirstpart}
\sum_{i\in S_\emptyset}\lambda_i\ind{C_i}(\omega^*)\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^*)\bigr)\geq0.
\end{equation}
Let $x_u^*\in\states^u$ be defined by $x_t^*\coloneqq \omega^*(t)$ for all $t\in u$, and let
\begin{equation*}
E\coloneqq
\{
\omega\in\Omega
...
\}.
\end{equation*}
Then for all $i\in S^*$, we have that $(A_i,C_i)\in\mathcal{C}_{x_u^*}$ and therefore, there are $u\subseteq v_i\in\mathcal{U}$ and $x_{v_i\setminus u}\in\states^{v_i\setminus u}$ such that $C_i=(X_u=x_u^*)\cap(X_{v_i\setminus u}=x_{v_i\setminus u})$, which we use to define
\begin{equation*}
...
Since $P_{x_u^*}$ is a stochastic process, it follows from Corollary~\ref{corol:processiffcoherent} and Equation~\eqref{eq:theo:aanelkaarplakken:defPtilde} that
*** still need to tackle the $S^*=\emptyset$ case seperately ***

\begin{equation*}%\label{eq:theo:aanelkaarplakken:secondpart}
\sup\left\{\sum_{i\in S^*}\lambda_i\ind{C_i^*}(\omega)\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega)\bigr)~\Bigg\vert~\omega\in C^*\right\}\geq0,
\end{equation*}
with $C^*\coloneqq\cup_{i\in S^*}C_i^*$.
Since $S^*$ is finite and because, for every $i\in S^*$, $\ind{A_i}$ and $\ind{C_i^*}$ can only take two values---$0$ or $1$---it follows that this supremum is taken over a finite set of real numbers, which implies that it is actually a maximum. Therefore, there is some $\omega^{**}\in C^*$ such that
...
\end{equation}
Now let $\omega^{***}\in\Omega$ be defined by
\begin{equation*}
\omega^{***}(t)\coloneqq
\begin{cases}
\omega^{*}(t) & \text{if $t\leq \max u$}\\
\omega^{**}(t) & \text{if $t\geq \max u$}
\end{cases}
\text{~~~for all $t\in\reals_{\geq0}$.}
\end{equation*}
Then *** still need to add some extra explanations for the equations below ***
\begin{equation*}
...
\end{equation*}
\begin{equation*}
\sum_{i\in S_\emptyset}\lambda_i\ind{C_i}(\omega^{***})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{***})\bigr)
=
\sum_{i\in S\emptyset}\lambda_i\ind{C_i}(\omega^{*})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{*})\bigr)\geq0,
\end{equation*}
and, for all $i\in\{1,\dots,n\}\setminus\{S_\emptyset\cup S^*\}$, $\ind{C_i}(\omega^{***})=0$. Therefore, we find that
\begin{equation*}
\sum_{i=1}^n\lambda_i\ind{C_i}(\omega^{***})\bigl(\tilde{P}(A_i\vert C_i)-\ind{A_i}(\omega^{***})\geq0,
\end{equation*}
which, since $\omega^{***}\in C_0$, implies that Equation~\eqref{eq:theo:aanelkaarplakken:coh} holds. Hence, $\tilde{P}$ is coherent.

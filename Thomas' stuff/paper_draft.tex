...
Consider any $t,s\in\realsnonneg$ such that $t<s$, and any $u,u^*\in\mathcal{U}_{[t,s]}$ such that $\sigma(u)<\delta$ and $\sigma(u^*)<\delta$, with $0<\delta\leq\nicefrac{1}{\norm{\lrate}}$. Let $C\coloneqq s-t$. Then,
\begin{equation*}
...
Another conclusion that we can draw from this, is that $L_t^s$ can be expressed as the solution of the differential equation
\begin{equation*}
...
For all $i\in\{2,\dots,n\}$, since $\Delta_i\leq\nicefrac{1}{\norm{\lrate}}$, it follows from Proposition~\ref{lemma:normQsmallenough} that $(I+\Delta_i\lrate)$ and $I$ are lower transition operators, which implies that they are non-negatively homogeneous and, because of Proposition~\ref{lemma:normofcoherenttrans}, that $\norm{I+\Delta_i\lrate}\leq1$ and $\norm{I}\leq1$. Therefore,
\begin{multline*}
...
\end{aligned}
\end{multline*}\\[3pt]
where the second inequality follows from the induction hypothesis and Lemma~\ref{lemma:differencenormofcoherenttransrate}, and the third inequality follows from Lemma~\ref{lemma:recursive}.
\end{proof}
...
\end{lemma}
\begin{proof}
Fix any $k\in\{1,\dots,n\}$. Then for all $i\in\{1,\dots,n_k\}$, since $\Delta_{k,i}\leq\Delta_k\leq\delta\leq\nicefrac{1}{\norm{\lrate}}$, it follows from Propositions~\ref{lemma:normQsmallenough} and~\ref{lemma:compositioncoherence} that $\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)$ and $(I+\Delta_k\lrate)$ are lower transition operators, which implies that they are non-negatively homogeneous and, because of Proposition~\ref{lemma:normofcoherenttrans}, that $\norm{\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)}\leq1$ and $\norm{(I+\Delta_k\lrate)}\leq1$. Therefore,
\begin{align*}
\norm{\prod_{k=1}^n\left(\prod_{i=1}^{n_k}(I+\Delta_{k,i}\lrate)\right)
-
\prod_{k=1}^n(I+\Delta_k\lrate)
...
=\delta C\norm{\lrate}^2,
\end{align*}
where the first inequality follows from Lemma~\ref{lemma:recursive} and the second inequality follows from Lemma~\ref{lemma:justthelinearpart}.
\end{proof}

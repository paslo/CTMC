...
However, this connection becomes less obvious for the notion of well-behaved stochastic processes (rather than well-behaved transition matrix systems), because the ``Lipschitz constant'' may there depend not only on $t$, but also on the specific history $x_u$ before time $t$. Due to how Lipschitz continuity is typically defined---on domains that are metric spaces and, for local Lipschitz continuity, on neighbourhoods in those domains---formalising this connection is rather non-trivial. For instance, since stochastic processes are defined on sets of events, even the appropriate metric for this domain seems non-obvious to us.

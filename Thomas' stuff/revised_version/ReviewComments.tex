...
We would very much like to thank you for the time and effort that you spent in reviewing our paper ``Imprecise Continuous-Time Markov Chains'' for the International Journal of Approximate Reasoning. Your comments and suggestions were plentiful, constructive, detailed, and very useful. Clearly, you have done a very thorough job and, given the length of the paper, that must have been quite a task. It's too bad this process is anonymous; we would very much like to return the favour. 

Below, we provide a detailed reply to each of your comments, in which we answer your questions and explain how your suggestions have been incorporated into the text or, in some cases, why we decided not to.

Once more, thank you very much for your efforts.
...
First of all, thank you very much for passing on this paper to the reviewers and our apologies for its length. We realise this creates some challenges, and we are grateful that you are allowing us to publish it anyway.

In order to try and alleviate this concern, we have addressed the comments aimed at reducing parts of the paper and, in particular, have removed the proofs of some of our examples and the appendix with the gambling interpretation of coherence (these parts have now been put in an extended on-line report). We have not, however, followed the third reviewer's suggestion of splitting the paper in two. While he or she is correct that the main text of the paper could be split in such a way, this is not the case for the proofs. In particular, the proofs for the second part of the paper rely heavily on the technical concepts in the first part. For that reason, splitting the paper would result in two papers that are both rather lengthy, and both still rather technical.

Replies to all of the reviewer's comments are given below. Most of them have been incorporated; in case we did not, our reply motivates this choice.
...
Our claim here is not that these two ways of interpreting the derivative do not lead to the same solution. In fact, in this case, as we show, they do. Our point is merely that Equation (57)---Equation (46) in the new version---trivially implies (56)---now Equation (45)---and that, in that sense,  Equation (57)---(46)---is stronger than (56)---(45). In order to try and clarify this, we have slightly reformulated the text.
...
The implication in the third line (i.e. that Equation (132)---Equation (F.7) in the new version---holds) follows from the fact that we use the \emph{maximum} norm, rather than e.g. the Euclidean norm.
...
We unfortunately do not really see an easier argument than the one that was given; note that we cannot directly apply Property LT5 to all $n$ steps simultaneously, nor is Equation (49)---Equation (38) in the new version---sufficient.

Regarding the direct use of Equation (49)---now (39)---to try to construct an easier argument, note that for functions $f \leq g$ it does not in general hold that also $\underline{Q}f \leq \underline{Q}g$. For example, consider $(-1, 0) \leq (0, 0)$, and the transition rate matrix $Q$ (which is trivially a lower transition rate operator $\underline{Q}$), given by
\begin{equation}
...
In any case, the induction argument that we give in Lemma G.4---Lemma F.4 in the new version---is the simplest that we have been able to find. If you know of an easier argument, we would of course be willing to incorporate that instead.
...
To answer the question, this was due to a stylistic preference of one of the authors (we had some discussion about this in the past, and eventually decided to go with this enumeration system).

That said, we have now changed the enumeration. We cannot find an explicit guide for IJAR papers, but went with the following system:
\begin{itemize}
\item The counters of all environments reset per section (and the numbering is prefixed by the section number, so e.g. Definition 3.2).
\item Theorem, Proposition, Corollary, and Lemma environments share one common counter. So, we might get Proposition 3.1, followed by Theorem 3.2, and then Corollary 3.3.
\item Definitions, Examples, and Remarks each have their own counter, distinct from the one that is shared by the various proposition environments mentioned above.
\end{itemize}

As an aside, the {\tt elsarticle} class somehow uses consecutive equation numbering in the main text (so, Equation (1), then Equation (2) and so on), but starts prefixing the equations with section numbers only in the appendix (so Equation (64) in Section 10 is followed by Equation (A.1) in Appendix A). While we also do not like this inconsistency, we have decided to keep it since it seems to be preferred/imposed by the Elsevier article class.
...
Yes, our notion of separately specified rows can rather directly be understood as the continuous-time analogue of the separately specified local models that are often adopted in credal networks. We now explain this in a footnote at the end of Definition 24---Definition 7.3 in the new version.
...
The reason why they do not coincide is indeed, essentially, due to the difference between epistemic irrelevance and strong independence, be it in a continuous-time setting. Following your suggestion, we now provide some intuition on this in a footnote (which is situated a couple of lines before Example 22---Example 9.2 in the new version).
...
We have incorporated this suggestion by adding a ``Finding Your Way Around in This Paper'' section to the introduction, which includes a list of notations.
...
These methods are superficially related to ours, in the sense that they consider a set of parameter vectors, but there does not seem to be a mathematical connection that can be used to perform our computations. First of all, these references deal with observations, which we do not. Secondly, even if we were to include observations (and consider a hidden imprecise continuous-time Markov chain), then still, there are a number of crucial differences. For example, their observations are real-valued, whereas ours are categorical. Also, since they consider a deterministic---rather than probabilistic---system, there is no need to estimate the parameters; it is just a matter of being able to compute them (or rather the bounds on them). Finally, in our case, at least in the case of sensitivity analysis, the system for which we have set-valued parameters is linear, whereas theirs is non-linear.
...
Regarding experiments to provide a ``practical proof-of-concept'', we would like to refer to the paper ``Modelling spectrum assignment in a two-service flexi-grid optical link with imprecise continuous-time Markov chains'' by Rottondi \emph{et al.}, for a successful application of the model. This reference has now been added to the---final paragraph of the---introduction of the paper.

%*** jasper: voegt die ref ook bij voorbeeld toe
%Heb besloten dit uiteindelijk niet te doen, aangezien het in die toepassing niet echt expliciet gemaakt wordt of het over de WM of WHM versie gaat...
...
If $j$ is odd, it follows from Equations~(11)---now Equation (10)---and (12)---now (11)---that
\begin{equation*}
...
We do not understand this comment. In fact, our Example 15---Example 6.2 in the new version---demonstrates that one \emph{cannot} (in general) compute the lower expectation by computing the precise expectation at the vertices of the parameter set, because the optimum is---in this example---obtained at an interior point of the parameter set.
...
The text has also been slightly reformulated, and some of our claims have been weakened.

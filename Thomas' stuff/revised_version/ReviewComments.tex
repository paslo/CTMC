...
This is indeed a very interesting question, and something that might be worth looking into in future work. The obvious subtlety is in the kind of continuity assumptions that should be imposed on these changing parameter-sets. However, for such time-dependent sets that are, say, piecewise-constant, we do indeed expect that our methods should translate fairly naturally to this more general setting.

We will not add any in-depth research on this to the present paper, but have added a remark to Section 11.
...
We have removed this part of the appendix and replaced its mentioning in the main text with pointers to other work from the literature.
...
The current definition is correct for our present purposes, and is purposefully phrased like this to cover some edge-cases. Note that we also call the singleton sequence $u=t$ a sequence, and that this is the unique element of the set $\mathcal{U}_{[t,t]}$. 

***** We could add the qualifier that the inequality $t=t_0<t_n=s$ should only hold for $n\geq 1$, but we worry that this might come across as somewhat pedantic.

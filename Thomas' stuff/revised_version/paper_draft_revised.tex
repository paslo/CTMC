...
$\norm{\cdot}$ & the maximum ($L_\infty$) norm of a function or operator & \placeref{notation:norm} \\
...
$\sigma(u)$ & the maximum sequential difference of $u=t_0,\ldots,t_n$; $\sigma(u)=\max\{t_i-{t_{i-1}}\,:\,i\in\{1,\ldots,n\}\}$ & TODO \\
$\states$ & the finite state-space of a stochastic process & TODO \\
$\states_t$, $\states_u$ & the state-space $\states$ at an explicit time $t$, resp. the joint state-space at (multiple) time points $u\in\mathcal{U}$ & TODO \\
$\gamblesX$ & the set of all functions $f: \states\to\reals$ & TODO \\
$\gambles(\states_u)$ & the set of all functions $f:\states_u\to\reals$ & TODO \\
$\mathcal{T}$ & a transition matrix system; a family of transition matrices satisfying certain properties & TODO \\
$\mathcal{T}_Q$ & the \emph{exponential transition matrix system} for the transition rate matrix $Q$ & TODO \\
$\mathcal{T}^\mathbf{I}$ & the restriction of the transition matrix system $\mathcal{T}$ to the closed time interval $\mathbf{I}\subseteq\realsnonneg$ & TODO \\
$\otimes$ & the concatenation operator between two restricted transition matrix systems & TODO \\
``well-behaved'' & roughly, ``with bounded rate-of-change'' & TODO \\
$\processes$ & the set of all continuous-time stochastic processes & TODO \\
$\wprocesses$ & the set of all well-behaved continuous-time stochastic processes & TODO \\
$\wmprocesses$ & the set of all well-behaved continuous-time Markov chains & TODO \\
$\whmprocesses$ & the set of all well-behaved, homogeneous continuous-time Markov chains & TODO \\
$\overline{\partial}_+T_{t,x_u}^t$, $\overline{\partial}_-T_{t,x_u}^t$, $\overline{\partial}T_{t,x_u}^t$ & (resp. right-sided, left-sided) outer partial derivative of the transition matrix $T_{t,x_u}^t$ of a stochastic process, for time $t$ and history $x_u$ & TODO \\
$\rateset$ & a set of transition rate matrices & TODO \\
$\mathcal{M}$ & a set of probability mass functions on $\states$ & TODO \\
$\mathcal{P}$ & a set of stochastic processes, $\mathcal{P}\subseteq\processes$ & TODO \\
$\mathcal{P}_{\rateset,\mathcal{M}}$ & the subset of $\mathcal{P}$ \emph{consistent} with $\rateset$ and $\mathcal{M}$ & TODO \\
$\wprocesses_{\rateset,\mathcal{M}}$, $\wmprocesses_{\rateset,\mathcal{M}}$, $\whmprocesses_{\rateset,\mathcal{M}}$ & different types of imprecise continuous-time Markov chains; also see $\mathcal{P}_{\rateset,\mathcal{M}}$ and $\wprocesses$, $\wmprocesses$, and $\whmprocesses$ & TODO \\
$\underline{\mathbb{E}}$ & lower expectation operator & TODO \\
$\underline{\mathbb{E}}^\mathrm{W}_{\rateset,\mathcal{M}}$, $\underline{\mathbb{E}}^\mathrm{WM}_{\rateset,\mathcal{M}}$, $\underline{\mathbb{E}}^\mathrm{WHM}_{\rateset,\mathcal{M}}$ & lower expectation operator for the different ICTMC's $\wprocesses_{\rateset,\mathcal{M}}$, $\wmprocesses_{\rateset,\mathcal{M}}$, $\whmprocesses_{\rateset,\mathcal{M}}$ & TODO \\
$\lrate$ & lower transition rate operator, $\lrate: \gamblesX\to\gamblesX$ & TODO \\
$L_t^s$ & lower transition operator corresponding to some $\lrate$, for time points $t,s$; $L_t^s:\gambles(\states_s)\to\gambles(\states_t)$ & TODO \\
$\underline{\mathcal{T}}_{\lrate}$ & family of lower transition operators $L_t^s$ corresponding to $\lrate$ & TODO
\end{tabularx}

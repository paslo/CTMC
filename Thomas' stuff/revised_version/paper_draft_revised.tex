...
\end{equation*}
where, for every $x\in\states$, $\rateset_x\coloneqq\{Q(x,\cdot)\colon Q\in\rateset\}$ is some given set of rows from which the $x$-row $Q(x,\cdot)$ of the rate matrices $Q$ in $\rateset$ is selected, independently of the other rows.\footnote{This concept is related to that of \emph{separately specified credal networks}---see, e.g., Reference {\bf XXXXX}---and, for readers that are familiar with the literature on this latter subject, can be understood as the continuous-time analogue of this notion.}
\end{definition}

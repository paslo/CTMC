...
Of course, skipping over parts of the paper will introduce some ambiguity about specific technical concepts that we refer to throughout. In order to alleviate this as much as possible, we end this section with a short list of the most important notation and terminology.

{\bf ****** BIG LIST GOES HERE *******}
...
As is proved in \exampleproofref, it then holds that
...
\vspace{7pt}
\end{equation}
Furthermore, it can be shown that the transition matrix system $\mathcal{T}$ is well-behaved---again, see \exampleproofref~for the formal proof.
\exampleend
\end{exmp}
...
As is proved in~\exampleproofref, despite the fact that we know from Example~\ref{exmp:limit_trans_mat_system} that each of the transition matrix systems $\mathcal{T}_i$, $i\in\nats_0$, is well-behaved, the limit $\mathcal{T}$ itself is not well-behaved.
...
%The interested reader is invited to take a look at Appendix~\ref{app:coherence}, where we provide this abstract concept with an intuitive gambling interpretation. 

However, for our present purposes, this interpretation is not required. Instead, our motivation for introducing coherence stems from the following two results. First, if $\mathcal{C}=\power\times\nonemptypower$, then coherence is equivalent to the axioms of probability, that is, properties~\ref{def:coh_prob_2}--\ref{def:coh_prob_6}.%conditional probability is the same as a full conditional probability.
...
As is proved in~\exampleproofref, it then holds for this particular process that
\begin{equation*}
...
As is proved in~\exampleproofref, for such a stochastic process $P$, the right-sided outer partial derivative $\smash{\overline{\partial}}_+T_0^0$ is then empty.
\exampleend
\end{exmp}
...
\section{Proofs of Examples}\label{app:example_proofs}
...
We start by proving that Equation~\eqref{eq:ex4:def} indeed holds. To this end, we first establish some properties. 
Consider any $t\in(0,1]$ and let $j$ be the unique element of $\nats_0$ such that $\delta_{j+1}< t\leq\delta_j$. Since $Q_1$ and $Q_2$ commute, it then follows from Equations~\eqref{eq:ex4:def}--\eqref{eq:ex4:phi2} that
\vspace{3pt}
...
\leq\Delta M,
\end{align*}
where the second and third inequality follows from Lemma~\ref{lemma:recursive} and~\ref{lemma:linearboundonexponential} in Appendix~\ref{app:systems}, respectively.
\end{proof}
